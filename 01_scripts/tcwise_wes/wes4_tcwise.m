function tc=wes4_tcwise(trackinput,format,spwinput,outputfile,varargin)
...
if ~isempty(outputfile)
    ...
    switch lower(spw.cs.type(1:3))
        ...
        otherwise
            ...
            % convert lat/lon input of TCWiSE to projected coordinate system:
            for it=1:length(tc.track)
                csold.name='WGS 84';
                csold.type='geographic';
                csnew.name=spw.cs.name;
                csnew.type=spw.cs.type;
                x0=tc.track(it).x;
                y0=tc.track(it).y;
...
for it=1:nt
    ...
    if dx==0 && dy ==0
       disp(['dx and dy became 0, timesteps it+1 and it-1 have exactly the same coordinate for it= ',num2str(it)]) 
       disp('dx and dy have been given small values to prevent further issues')
       dx = 1000; %m
       dy = 1000;
    end
...
for it=1:nt
    ...
    if isnan(tc.track(it).vmax_rel)
        tc.track(it).vmax_rel = tc.track(it).vmax;
    end
