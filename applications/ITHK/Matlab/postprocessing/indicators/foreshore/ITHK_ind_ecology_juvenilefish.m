...
ShoreWidth  = 10000.0;                                                     % Default initial shoreface width is 10km.
if isfield(S.settings.indicators.foreshore,'ShoreWidth')
...
PLOTscale1  = 10;                                                          % PLOT setting : scale magintude of plot results
PLOTscale2  = 0.9;                                                         % PLOT setting : subtract this part (e.g. 0.9 means that plot runs from 90% to 100% of initial shorewidth)
PLOToffset  = 10;                                                          % PLOT setting : plot bar at this distance offshore [m]
if isfield(S.settings.indicators.foreshore,'PLOTscale1') && isfield(S.settings.indicators.foreshore,'PLOTscale2') && isfield(S.settings.indicators.foreshore,'PLOToffset')
    PLOTscale1 = str2double(S.settings.indicators.foreshore.PLOTscale1);         % Default initial value can be replaced by setting in ITHK_settings.xml
    PLOTscale2 = str2double(S.settings.indicators.foreshore.PLOTscale2);         % Default initial value can be replaced by setting in ITHK_settings.xml
    PLOToffset = str2double(S.settings.indicators.foreshore.PLOToffset);         % Default initial value can be replaced by setting in ITHK_settings.xml
end
colour       = {[0.3 0.2 0.8]};
fillalpha    = 0.7;
