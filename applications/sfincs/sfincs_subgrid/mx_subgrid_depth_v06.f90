#include "fintrf.h"
#if 0
! Generate with :  mex mx_subgrid_depth_v06.f90
      ...
      do n = 1, nmax
         do m = 1, mmax
            ...
            do ibin = 1, nbin                   
               ...
               do j1 = 1, npreal_a
                  !
                  h = max(zb - max(zmin(n, m), dd_a(j1)), 0.0)
               ...
               do j1 = 1, npreal_b
                  !
                  h = max(zb - max(zmin(n, m), dd_b(j1)), 0.0)
               ...
               !               
               ! Now take average value of cells A and B
               !               
               hrep(n,m,ibin) = 0.5*(hrep_a + hrep_b)
               manning_avg((n,m,ibin) = 0.5*(n_avg_a + n_avg_b)
               !
