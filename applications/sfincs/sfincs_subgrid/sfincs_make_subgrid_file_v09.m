function subgrd=sfincs_make_subgrid_file(dr,subgridfile,bathy,manning_input,cs,nbin,refi,refj,uopt,maxdzdv,varargin)
...
if isempty(inp)
    ...
    inp=sfincs_read_input([dr 'sfincs.inp'],inp);
end
mmax=inp.mmax;
nmax=inp.nmax;
dx=inp.dx;
...
%        [zmin,zmax,volmax,ddd]=subgrid_volumes_v06(d, dxmp, dymp, nbin, zvolmin, maxdzdv);
        ...
        % No longer use mex files
        [ddd_u,navg_u,pwet_u,fnfit_u,navg_w_u,zmin_u,zmax_u]=subgrid_depth_v09d(d,manning1,nbin,huthresh);
...
%        subgrd.u_dhdz(ic1:ic2,jc1:jc2)=dhdz_u;
        ...
        subgrd.u_navg_w(ic1:ic2,jc1:jc2)=navg_w_u;
        subgrd.u_fnfit(ic1:ic2,jc1:jc2)=fnfit_u;
        subgrd.u_pwet(ic1:ic2,jc1:jc2,:)=pwet_u;
        ...
        % No longer use mex files
        [ddd_v,navg_v,pwet_v,fnfit_v,navg_w_v,zmin_v,zmax_v]=subgrid_depth_v09d(d,manning1,nbin,huthresh);
...
%        subgrd.v_dhdz(ic1:ic2,jc1:jc2)=dhdz_v;
        ...
        subgrd.v_navg_w(ic1:ic2,jc1:jc2)=navg_w_v;
        subgrd.v_fnfit(ic1:ic2,jc1:jc2)=fnfit_v;
        subgrd.v_pwet(ic1:ic2,jc1:jc2,:)=pwet_v;
...
subgrd.z_volmax = subgrd.z_volmax(1:nmax,1:mmax,:);
subgrd.z_depth  = subgrd.z_depth(1:nmax,1:mmax,:);

subgrd.u_zmin   = subgrd.u_zmin(1:nmax,1:mmax);
subgrd.u_zmax   = subgrd.u_zmax(1:nmax,1:mmax);
subgrd.u_hrep   = subgrd.u_hrep(1:nmax,1:mmax,:);
subgrd.u_navg   = subgrd.u_navg(1:nmax,1:mmax,:);
subgrd.u_pwet   = subgrd.u_pwet(1:nmax,1:mmax,:);
subgrd.u_navg_w = subgrd.u_navg_w(1:nmax,1:mmax);
subgrd.u_fnfit  = subgrd.u_fnfit(1:nmax,1:mmax);
subgrd.v_zmin   = subgrd.v_zmin(1:nmax,1:mmax);
subgrd.v_zmax   = subgrd.v_zmax(1:nmax,1:mmax);
subgrd.v_hrep   = subgrd.v_hrep(1:nmax,1:mmax,:);
subgrd.v_navg   = subgrd.v_navg(1:nmax,1:mmax,:);
subgrd.v_pwet   = subgrd.v_pwet(1:nmax,1:mmax,:);
subgrd.v_navg_w = subgrd.v_navg_w(1:nmax,1:mmax);
subgrd.v_fnfit  = subgrd.v_fnfit(1:nmax,1:mmax);

% Write file
if ~isempty(subgridfile)
%    sfincs_write_binary_subgrid_tables_v09(subgrd,msk,nbin,subgridfile,uopt);
    sfincs_write_binary_subgrid_tables_netcdf(subgrd,msk,nbin,subgridfile,uopt);
end
