...
input.mdv.Tstop=3600*2; %simulation time [s]; [1x1 double]; e.g. [3600]
input.mdv.dt=0.1; %time step [s]; [1x1 double]; e.g. [2]
...
input.mdv.chk.flow=0; %Froude and CFL check [-]; [1x1 double]; e.g. [1]
...
input.mor.total_ThUnLyr=0.3; %thickness of the entire bed [m]; [1x1 double]; e.g. [2]
...
input.bch.timeQ0=[0;input.mdv.Tstop/2;input.mdv.Tstop]; %time at which the water discharge is specified [s]; [nix1 double]; e.g. [1800;3600]
input.bch.Q0=[0.2;0.3;0.2]; %water discharge at the specified times [m^3/s]; [ntx1 double]; e.g. [1;2]
