...
%$HeadURL: https://svn.oss.deltares.nl/repos/openearthtools/trunk/matlab/applications/ELV/test/reference/006/input_ELV.m $
%
...
input.mdv.dt = 24*3600; % time step 
...
input.mdv.Tstop = 3600*24*365*1000; %simulation time [s]; [1x1 double]; e.g. [3600]
input.mdv.Flmap_dt = 3600*24*365*1;
...
input.mdv.output_var = {'u','h','etab','Mak','La','msk','Ls','qbk','Cf','ell_idx'}; %variables name to output; 
...
input.grd.L = 300000; %domain length [m]; [1x1 double]; e.g. [100]
input.grd.dx = 1000; %streamwise discretizations [m]; [1x1 double]; e.g. [0.1]
input.grd.B = [ones(100,1)*600; ones(100,1)*150; ones(100,1)*300]';
input.grd.crt = 1;
%% 
%% MORPHOLOGY
%% 
...
input.sed.dk =  0.008; % characteristic grain sizes [m]; [nfx1 double]; e.g. [0.0005;0.003;0.005]
...
input.bch.uptype=12; %type of hydrodynamic boundary condition at the upstream end: 1x= water discharge (Q); [-] x1 = from input; x2 = from file; 1 =water discharge from file [1x1 double]; e.g. [1]
tpath = pwd;
input.bch.path_file_Q =fullfile(paths_run_folder,'Q.mat'); %pathname where data is located [-]
input.bch.dotype=1; %type of hydrodynamic boundary condition at the downstream end: 1=water level 2=water depth [-]; x1 = from input; x2 = from file; [1x1 double]; e.g. [1]
