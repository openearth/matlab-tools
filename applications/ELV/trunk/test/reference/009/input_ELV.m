...
input.mdv.Tstop=60*5; %simulation time [s]; [1x1 double]; e.g. [3600]
input.mdv.dt=0.10; %time step [s]; [1x1 double]; e.g. [2]
input.mdv.Flmap_dt=10; %printing map-file interval time [s]; [1x1 double]; e.g. [60]  
...
input.mdv.Cf=0.0104; %friction coefficient [-]; [1x1 double]; e.g. [0.008]
...
input.mdv.output_var={'u','h','etab','Mak','La','msk','Ls','qbk','Cf','ell_idx','time_loop','pmm'}; %map variables name to output; 
...
input.frc.wall_corr=3; %flume wall correction: 0=NO; 1=Johnson (1942); 2=Nikuradse; 3=imposed bed friction coefficient
input.frc.Cfb=0.0084; %nondimensional bed friction coeddicient 
...
input.grd.dx=0.15; %streamwise discretizations [m]; [1x1 double]; e.g. [0.1]
...
input.mor.gsdupdate=7; %update grain size distribution 0=NO; 1=Hirano ;2=eli 1 (max. La); 3=eli 1 (min. La); 4=pmm Mak(0,1); 5=pmm etab(0,1); 6=frozen Mak(0,1); 7=frozen Mak>1 [-]; e.g. 1
...
input.mor.La=0.010; %active layer thickness [m]; [1x1 double]; e.g. [0.1]
input.mor.ThUnLyr=0.01; %thickness of each underlayer [m]; [1x1 double]; e.g. [0.15]
...
input.sed.dk=[0.002108,0.005496]; %characteristic grain sizes [m]; [nfx1 double]; e.g. [0.0005;0.003;0.005]
...
input.tra.cr=3; %sediment transport closure relation= %1=Meyer-Peter-Muller; 2=Engelund-Hansen; 3=Ashida-Michiue; 4=Wilcock-Crowe; [1x1 double]; e.g. [1]
input.tra.param=[17,0.05]; %sediment transport parameters (depending on the closure relation)
input.tra.hid=3; %hiding function= %0=NO function; 1=Egiazaroff; 2=Power-Law; 3=Ashida-Mishihue;
...
input.ini.subs.patch.x=[4.90-0.25,4.90+0.25]; %x coordinate of the upper corners of the patch [m]; [1x2 double]; e.g. [5,7.5]
input.ini.subs.patch.releta=0.01; %substrate depth of the upper corners of the patch [m]; [1x1 double]; e.g. [0.2,0.2]
...
input.bch.Q0=[0.150;0.150]; %water discharge at the specified times [m^3/s]; [ntx1 double]; e.g. [1;2]
...
input.bch.etaw0=[0;-0.005]; %downstream water level at the specified times [m]; [nix1 double]; e.g. [1;1.5]
...
input.bcm.timeQbk0=[0;1;input.mdv.Tstop]; %time at which the input is specified [s]; [nix1 double]; e.g. [1800;3600]
input.bcm.Qbk0=[0,7.861635220125786e-06;0,7.861635220125786e-06;0,7.861635220125786e-06]; %volume of sediment transported excluding pores per unit time, and per size fraction at the specified times [m^3/s]; [ntxnf double]; e.g. [2e-4,4-4;3e-4,5e-4]
