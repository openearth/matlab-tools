...
input.mdv.steady_solver=3; %(if input.mdv.flowtype=1); steady solver: 1=energy-Euler; 2=energy-RK4; 3=depth-Euler; 4=depth-RK4; [1x1 double]; default=1
...
input.mdv.Tstop=3600*24*302; %simulation time [s]; [1x1 double]; e.g. [3600]
input.mdv.dt_type=2; %type of time step method [-]: 1=fixed time step; 2=fixed CFL number; [1x1 double]; e.g. [1]  
input.mdv.dt=3600; %time step [s]; [1x1 double]; e.g. [2] When using input.mdv.dt_type=2 this is the first time step.
input.mdv.cfl=0.7; %CFL number to compute time step [-]; [1x1 double]; e.g. [0.9] 
...
input.mdv.Flmap_dt=3600*24*15; %printing map-file interval time [s]; [1x1 double]; e.g. [60]  
% input.mdv.Flmap_dt=3600; %printing map-file interval time [s]; [1x1 double]; e.g. [60]  
% input.mdv.disp_t_nt=1000; %number of time steps to average the time needed to finish the simulatin [-]; [1x1 double]; e.g. [100]  
...
input.mdv.output_var={'u','h','etab','qbk','ell_idx'}; %map variables name to output; 
...
input.frc.Cf=0.00; %friction coefficient [-]; [1x1 double]; e.g. [0.008]
input.frc.wall_corr=0; %flume wall correction: 0=NO; 1=Johnson (1942); 
input.frc.Cfb_model=3; %skin friction model: 0=plane bed (Cf_b=Cf); 2=Nikuradse (1933); 3=imposed bed friction coefficient; 4=related to active layer thickness; 5=Engelund-Hansen (1967); 6=Wright and Parker (2004); 7=Smith and McLean (1977); 8=Haque (1983)
%input.frc.Cfb=(1e-3/0.05*(9.81*1.65*0.001)^(5/2)/sqrt(9.81*1.65*0.001^3))^(2/3); %nondimensional bed friction coefficient 
input.frc.Cfb=0.01; %nondimensional bed friction coefficient 
...
input.grd.L=2100; %domain length [m]; [1x1 double]; e.g. [100]
input.grd.dx=30; %streamwise discretizations [m]; [1x1 double]; e.g. [0.1]
...
input.mor.ellcheck=1; %ellipticity check 0=NO; 1=YES (full model); 2=YES (approximate model) [-]; e.g. 1
...
input.mor.La=1.00; %active layer thickness [m]; [1x1 double] | [nix1 double]; e.g. [0.1];
...
input.mor.Tstart=3600*24*2; %start time of morphology update [s]; [1x1 double]; e.g. [600]
% input.mor.Tstart=0; %start time of morphology update [s]; [1x1 double]; e.g. [600]
...
input.sed.dk=0.001; %characteristic grain sizes [m]; [nfx1 double]; e.g. [0.0005;0.003;0.005]
...
input.tra.param=[0.05,5]; %sediment transport parameters (depending on the closure relation) 1=[8,1.5,0.047] (MPM) [5.7,1.5,0.047] (FLvB); 2=[0.05,5] (EH); 3=[17,0.05] (AM)
...
x=input.grd.dx-input.grd.dx/2:input.grd.dx:input.grd.L-input.grd.dx/2;
etab=etab_max*exp(-(x-mu).^2/sig^2);
h=etaw-etab;
...
input.ini.u=q./h; %flow velocity [m/s]; [1x1 double] | [1xnx double]; e.g. [1]
input.ini.h=h; %flow depth [m]; [1x1 double] | [1xnx double]; e.g. [1.5]
% input.ini.slopeb=0; %bed slope [-]; [1x1 double] | [1xnx double]; e.g. [1e-3]
% input.ini.etab0=-1; %downstream bed elevation [m]; [1x1 double]; e.g. [0]
input.ini.etab=etab; %bed level [-]; [1x1 double] | [1xnx double]; e.g. [1e-3]
% input.ini.Fak=1; %effective fractions at the active layer [-]; [(nf-1)x1 double] | [(nf-1)xnx double]; e.g. [0.2,0.3]
...
input.bch.Q0=[q;q]; %water discharge at the specified times [m^3/s]; [ntx1 double]; e.g. [1;2]
...
input.bch.etaw0=[etaw;etaw]; %downstream water level at the specified times [m]; [nix1 double]; e.g. [1;1.5]

%% 
%% MORPHODYNAMIC BOUNDARY CONDITIONS
%% 

input.bcm.type=4; %type of morphodynamic boundary condition:  [-] [1x1 double] e.g. [1]
