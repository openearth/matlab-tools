...
input.mdv.flowtype=4; %flow assumption: 0=NO update 1=steady; 2=quasi-steady; 3=unsteady (explicit); 4=unsteady (implicit); [-]; [1x1 double]; e.g. [1]
...
input.mdv.Tstop=365*24*3600; %simulation time [s]; [1x1 double]; e.g. [3600]
input.mdv.dt=600; %time step [s]; [1x1 double]; e.g. [2]
input.mdv.Flmap_dt=24*3600; %printing map-file interval time [s]; [1x1 double]; e.g. [60]  
...
input.mdv.Cf=0.007; %friction coefficient [-]; [1x1 double]; e.g. [0.008]
...
input.grd.L=200000; %domain length [m]; [1x1 double]; e.g. [100]
input.grd.dx=1000; %streamwise discretizations [m]; [1x1 double]; e.g. [0.1]
input.grd.B=300; %width [m]; [1x1 double] | [1xnx double]; e.g. [10]
...
input.mor.La=1.00; %active layer thickness [m]; [1x1 double]; e.g. [0.1]
input.mor.ThUnLyr=1.00; %thickness of each underlayer [m]; [1x1 double]; e.g. [0.15]
input.mor.total_ThUnLyr=5.00; %thickness of the entire bed [m]; [1x1 double]; e.g. [2]
...
input.sed.dk=[0.001,0.010]; %characteristic grain sizes [m]; [nfx1 double]; e.g. [0.0005;0.003;0.005]
...
% input.tra.E_cr=1; %closure relation for entrainment function 1=FLvB [1x1 double]; e.g. [1]
% input.tra.vp_cr=1; %closure relation for paticle velocity function 1=FLvB [1x1 double]; e.g. [1]
% input.tra.param=[5.7,1.5,0.047]; %sediment transport parameters (depending on the closure relation)
% input.tra.E_param=[0.0199,1.5]; %entrainment function paramenters
...
% input.tra.kappa=[0.1;0.1]; %diffusivity [m^2/s]; [(nf)x(1) double]; e.g. [0.1;0.1]
...
input.ini.u=1.00; %flow velocity [m/s]; [1x1 double] | [1xnx double]; e.g. [1]
input.ini.h=6.00; %flow depth [m]; [1x1 double] | [1xnx double]; e.g. [1.5]
input.ini.slopeb=1e-4; %bed slope [-]; [1x1 double] | [1xnx double]; e.g. [1e-3]
...
input.ini.Fak=0.7; %effective fractions at the active layer [-]; [(nf-1)x1 double] | [(nf-1)xnx double]; e.g. [0.2,0.3]
input.ini.fsk=0.1; %effective fractions at the substrate [-]; [(nf-1)x1 double] | [(nf-1)xnx double]; e.g. [0.2,0.3]
...
input.bcm.type=13; %type of morphodynamic boundary condition: 1=sediment discharge [-]; 2=periodic; 3=cyclic hydrograph [1x1 double]; 13=Normal Flow Load Distribution (with Qw(t) we compute Qs(t) for normal flow ATT! more info is needed (input.bcm.NFLtype)); e.g. [1]
...
input.bcm.NFLparam=[input.ini.slopeb,input.ini.Fak,(1-sum(input.ini.Fak))']; %if input.bcm.NFLtype=1: [slope,Fak (for all k)] e.g. [1e-4,0.1,0.9]; if input.bcm.NFLtype=2: [Qb,Fak (for all k)] e.g. [1e-5,0.1,0.9]; if input.bcm.NFLtype=3: [Qbk] e.g. [0.56,0.11]*1e9/2650/31536000
...
% input.bcm.Qbk0=[0.0,1.0;0.0,1.0]*1e-5; %volume of sediment transported excluding pores per unit time, and per size fraction at the specified times [m^3/s]; [ntxnf double]; e.g. [2e-4,4-4;3e-4,5e-4]
% input.bcm.pa_u=3; %particle activity upstream boundary condition type: 1=Dirichlet (Gammak at x=0 specified), 2=Neumann (dGammak/dx at x=0 specified), 3=Robin (Qbk at x=0 specified)
...
% input.bcm.pa_d=3; %particle activity downstream boundary condition type: 1=Dirichlet (Gammak at x=L specified), 2=Neumann (dGammak/dx at x=L specified), 3=Robin (Qbk at x=L specified)
% input.bcm.timeGammakL=[0;input.mdv.Tstop]; %time at which the input is specified [s]; [nix1 double]; e.g. [1800;3600]
% input.bcm.GammakL=NaN; %particle activity at x=0 at the specified times [m]; [(nt)x(nf)]; e.g. [0,0;0,0] if NaN it sets saturation value for all time
