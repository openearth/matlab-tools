...
input.mdv.dt=1800; %time step [s]; [1x1 double]; e.g. [2]
...
input.mdv.output_var={'u','h','etab','Mak','La','msk','Ls','qbk','Cf','ell_idx','time_loop'}; %variables name to output; 
...
input.mor.La=0.015; %active layer thickness [m]; [1x1 double]; e.g. [0.1]
input.mor.ThUnLyr=0.50; %thickness of each underlayer [m]; [1x1 double]; e.g. [0.15]
...
% input.tra.param=[3.97,1.5,0.0495]; %sediment transport parameters (depending on the closure relation)
...
input.ini.u=0.8; %flow velocity [m/s]; [1x1 double] | [1xnx double]; e.g. [1]
input.ini.h=3.5; %flow depth [m]; [1x1 double] | [1xnx double]; e.g. [1.5]
input.ini.slopeb=1.0e-5; %bed slope [-]; [1x1 double] | [1xnx double]; e.g. [1e-3]
...
input.ini.Fak=0.2; %effective fractions at the active layer [-]; [(nf-1)x1 double] | [(nf-1)xnx double]; e.g. [0.2,0.3]
input.ini.fsk=0.8; %effective fractions at the substrate [-]; [(nf-1)x1 double] | [(nf-1)xnx double]; e.g. [0.2,0.3]
...
input.bch.Q0=[2;2]; %water discharge at the specified times [m^3/s]; [ntx1 double]; e.g. [1;2]
...
input.bcm.Qbk0=[1;1]*1e-5; %volume of sediment transported excluding pores per unit time, and per size fraction at the specified times [m^3/s]; [ntxnf double]; e.g. [2e-4,4-4;3e-4,5e-4]
