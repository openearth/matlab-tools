...
% etawobs_fm=NC_read_text(fpath_his_fm,'station_name');
...
for kcs=1:ncs
    for kt=1:nt
        ...
        etaw_v=etaw_fm(idx_ost_ocs(kcs),idx_tim_fm); %y-vector. FM water level for all times.
        
        etaw_fm_mod(idx_ost_ocs(kcs),idx_tim_fm(kt))=interp1(Q_v,etaw_v,Q_q); %interpolated FM water level.
    end
end
...
for kcs=1:ncs
    ...
    in_p.etaw_fm=etaw_fm(idx_ost_ocs(kcs),idx_tim_fm);
    in_p.Q_wa=Q_wa(idx_ocs_wa_v(kcs),idx_tim_wa);
    in_p.etaw_fm_mod=etaw_fm_mod(idx_ost_ocs(kcs),idx_tim_fm);
