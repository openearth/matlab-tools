...
try
    if exist('maprefcells', 'file') == 2
        R = maprefcells(x_limits, y_limits, size(Z), 'ColumnsStartFrom', 'north');
    else
    ...
    geotiffwrite(fpath_output_tif, Z, R, 'CoordRefSysCode', epsg_code);
catch me_map
    try
        option = struct();
        ...
        geotiffwrite(fpath_output_tif, [], single(Z), 32, option);
    catch me_oet
