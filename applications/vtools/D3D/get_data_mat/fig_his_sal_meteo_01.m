...
%
%$Revision: 18074 $
%$Date: 2022-05-24 05:47:55 +0200 (Tue, 24 May 2022) $
%$Author: chavarri $
%$Id: figure_layout.m 18074 2022-05-24 03:47:55Z chavarri $
...
for kobs=1:nobs
    ...
    if vm>500
        ...
        str_obs_1{k1}=data.obs(kobs).name;
    else
        kc=2;
        k2=k2+1;
        ...
        str_obs_2{k2}=data.obs(kobs).name;
    end
end
...
for kobs=1:nobs
han.p(kr,kc,kobs)=plot(data.crs_cum(kobs).tim,data.crs_cum(kobs).val,'parent',han.sfig(kr,kc),'color',cmap(kc,:),'linewidth',prop.lw1,'linestyle',prop.ls1,'marker',prop.m1);
end
...
han.p(kr,kc,1)=fill([xlims(1),xlims(2),xlims(2),xlims(1)],[bf_l,bf_l,bf_h,bf_h],[0.9,0.9,0.9],'parent',han.sfig(kr,kc));
...
han.leg(kr,kc).Position=pos.leg+[-0.2,0.1,0,0];
