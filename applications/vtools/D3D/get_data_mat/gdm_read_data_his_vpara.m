...
function data=gdm_read_data_his_vpara(fdir_mat,fpath_his,flags)

%% PARSE

depth_average=isfield_default(flags,'depth_average',false,'output','array');
projection_angle=isfield_default(flags,'projection_angle',NaN,'output','array');
...
if depth_average
    data_x=gdm_read_data_his(fdir_mat,fpath_his,'depth-averaged_x_velocity',flags);
    data_y=gdm_read_data_his(fdir_mat,fpath_his,'depth-averaged_y_velocity',flags);
    vx=data_x.val;
    vy=data_y.val;
    data=data_x;
else
    data=gdm_read_data_his(fdir_mat,fpath_his,'uv',flags);
    vx=data.vel_x;
    vy=data.vel_y;
end
vx=reshape(vx,1,[]);
