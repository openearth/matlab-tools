...
function fig_fraction_cs_pli(in_p)
...
kr=1; kc=1;    
han.p(kr,kc,1)=plot(sb(:,1),sb(:,2),'parent',han.sfig(kr,kc),'color','k','linewidth',prop.lw1,'linestyle',prop.ls1,'marker',prop.m1);
han.p(kr,kc,1)=plot(wb(:,1),wb(:,2),'parent',han.sfig(kr,kc),'color','c','linewidth',prop.lw1,'linestyle',prop.ls1,'marker',prop.m1);
nrkm=size(x,1);
for krkm=1:nrkm
    for kpli=1:3
        han.p(kr,kc,1)=plot(x{krkm,kpli},y{krkm,kpli},'parent',han.sfig(kr,kc),'color',prop.color(idx_v(kpli),:),'linewidth',prop.lw1,'linestyle',prop.ls1,'marker','.');
    end
end
