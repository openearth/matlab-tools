...
kr=1; kc=1;    
for kls=1:nls
han.p(kls)=plot(ls(kls).data.xg,ls(kls).data.yg,'color',cmap(kls,:),'LineStyle','-','marker','*','parent',han.sfig(kr,kc));
han.p2(kls)=plot(ls(kls).data.xg,ls(kls).data.y_fit,'color',cmap(kls,:),'LineStyle','--','marker','o','parent',han.sfig(kr,kc));
han.p3(kls)=plot(ls(kls).data.xg,ls(kls).data.y_0,'color',cmap(kls,:),'LineStyle','-.','marker','s','parent',han.sfig(kr,kc));
end
