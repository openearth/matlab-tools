...
function fig_map_fraction_cs_01(in_p)
...
if isfield(in_p,'fig_size')==0
    in_p.fig_size=[0,0,14.5,7]; %(1+sqrt(5)/2)
end
...
% Q_s=sum(Q,2);
% bol_out=Q_s==0;
% han.p(kr,kc,:)=area(rkm(~bol_out),val_p(~bol_out,:),'parent',han.sfig(kr,kc));
han.p(kr,kc,:)=area(rkm,val_p,'parent',han.sfig(kr,kc));
