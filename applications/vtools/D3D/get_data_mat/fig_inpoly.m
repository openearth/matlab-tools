...
tol=500;
if any(bol_get)
    lims_y=[min(gridInfo.Ycen( bol_get))-tol,max(gridInfo.Ycen( bol_get))+tol];
    lims_x=[min(gridInfo.Xcen( bol_get))-tol,max(gridInfo.Xcen( bol_get))+tol];
else
    lims_y=[-1,1];
    lims_x=[-1,1];
end
...
kr=1; kc=1;    
han.p(kr,kc,1)=scatter(gridInfo.Xcen(~bol_get),gridInfo.Ycen(~bol_get),10,'r','filled');
han.p(kr,kc,2)=scatter(gridInfo.Xcen( bol_get),gridInfo.Ycen( bol_get),10,'g','filled');
han.p(kr,kc,1)=plot(sb(:,1),sb(:,2),'k');
