...
function fig_his_convergence(in_p)
...
stations_clean=cellfun(@(X)strrep(X,'_','\_'),stations,'UniformOutput',false);
if ns<20 
    han.sfig(kr,kc).XTick=1:1:ns;  
    han.sfig(kr,kc).XTickLabel=stations_clean;
else %otherwise there are too many
    st=round(ns/20);
    han.sfig(kr,kc).XTick=1:st:ns;  
    han.sfig(kr,kc).XTickLabel=stations_clean(1:st:ns);
end
xtickangle(90)
...
% han.sfig(kr,kc).Title.String=sprintf('%f s',;
