...
for krkmv=1:nrkmv %rkm polygons
    ...
    for kt=kt_v %time
        ...
        for kvar=1:nvar %variable
            ...
            for kfn=1:nfn
                in_p.val=data.(fn_data{kfn});
                fname_noext=fig_name(fdir_fig,tag,runid,time_dnum(kt),var_str,fn_data{kfn});
...
function fpath_fig=fig_name(fdir_fig,tag,runid,time_dnum,var_str,fn)
...
fpath_fig=fullfile(fdir_fig,sprintf('%s_%s_%s_%s_%s',tag,runid,datestr(time_dnum,'yyyymmddHHMM'),var_str,fn));
