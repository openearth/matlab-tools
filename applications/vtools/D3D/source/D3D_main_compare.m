...
kf=kf+1; 
dire_sim_v{kf}=fullfile(path_runs_1,'\r_003\dflowfm'); 
desc_sim_v{kf}='straigth';
...
% br_name{kb,1}='Kattendiep';
    ...
    %% time
time_data=1; %0=all times (one time call); 1=several times (several time calls); 
...
% for kb=1:nb
%     for ks=1:ns
%         time_d=datetime(1994,1,1)+seconds(time_r{ks,kb});
%             [s_m,t_m]=meshgrid(SZ{ks,kb},time_d);
%         SZ_p{ks,kb}=s_m;
%         time_p{ks,kb}=t_m;
%         z_p{ks,kb}=z{ks,kb,kt,kv}-repmat(z{ks,kb,1,kv}(:,1),1,size(z{ks,kb,kt,kv},2));
