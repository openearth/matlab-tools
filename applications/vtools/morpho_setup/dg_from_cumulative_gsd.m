...
for kgsd=1:ngsd
    ...
    %rkm
    rkm_all=cat(1,rkm_all,rkm);
    
    %dg
    dg=cumulative_to_dg(cum,dsieve);
    dg_all=cat(1,dg_all,dg);

    %save local
    [fdir,fname]=fileparts(fpath_gsd_loc);
    fpath_mat=fullfile(fdir,sprintf('%s_dg.mat',fname));
...
%sort
if ~mono_increase(rkm_all)
    error('deal with this')
end

%save
fdir=fileparts(fpath_gsd{1});
fpath_mat=fullfile(fdir,'dg.mat');

data=struct();
data.dg.val_mean.val=dg_all;
data.dg.val_mean.tim_dnum=tim_dnum;
data.dg.val_mean.s=rkm_all;
data.dg.val_mean.rkm=rkm_all;
...
kr=1; kc=1;    
han.p(kr,kc,1)=plot(data.rkm,data.dg,'parent',han.sfig(kr,kc),'color',prop.color(1,:),'linewidth',prop.lw1,'linestyle',prop.ls1,'marker',prop.m1);
