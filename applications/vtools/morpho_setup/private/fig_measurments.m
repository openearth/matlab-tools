...
xlabels{kr,kc}=labels4all('rkm',1,lan);
ylabels{kr,kc}=data.unit;
...
str_leg=cell(nt,1);
for kt=1:nt
han.p(kr,kc,kt)=plot(data.rkm,data.val(:,kt),'parent',han.sfig(kr,kc),'color',cmap(kt,:),'linewidth',prop.lw1,'linestyle',prop.ls1,'marker',prop.m1);
str_leg{kt}=datestr(data.tim_dnum(kt),'dd-mm-yy'); %check
end
