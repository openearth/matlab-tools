...
%
%$Revision: 8 $
%$Date: 2021-07-08 10:17:44 +0200 (Thu, 08 Jul 2021) $
%$Author: chavarri $
%$Id: fig_grid_change.m 8 2021-07-08 08:17:44Z chavarri $
%$HeadURL: file:///P:/11206813-007-kpp2021_rmm-3d/E_Software_Scripts/00_svn/modify_bed_elevation_grid/fig_grid_change.m $
%
%MATLAB BUGS:
...
%colorbar
kr=1; kc=1;
cbar(kr,kc).displacement=[0.15,0.12,0.2,0]; 
cbar(kr,kc).location='northoutside';
cbar(kr,kc).label='bed elevation [m]';
...
xlabels{kr,kc}='x coordinate [m]';
ylabels{kr,kc}='y coordinate [m]';
...
%figure initialize
han.fig=figure('name',prnt.filename,'Visible','off');
...
han.sfig(kr,kc).Title.String='original';
...
% han.sfig(kr,kc).XScale='log';
% han.sfig(kr,kc).YScale='log';
han.sfig(kr,kc).Title.String='modified';
% han.sfig(kr,kc).XColor='r';
% han.sfig(kr,kc).YColor='k';
...
%plots
kr=1; kc=1;    
han.p(kr,kc,1)=scatter(node_x,node_y,prop.ms1,node_z,'filled','parent',han.sfig(kr,kc));
for kf=1:numel(pol)
han.s(kr,kc,1)=plot(pol{kf}(:,1),pol{kf}(:,2),'color','k','parent',han.sfig(kr,kc));
end

kr=1; kc=2;    
han.p(kr,kc,1)=scatter(node_x,node_y,prop.ms1,node_z_mod,'filled','parent',han.sfig(kr,kc));
for kf=1:numel(pol)
han.s(kr,kc,1)=plot(pol{kf}(:,1),pol{kf}(:,2),'color','k','parent',han.sfig(kr,kc));
end
