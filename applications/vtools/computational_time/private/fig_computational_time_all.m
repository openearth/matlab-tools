...
function fig_computational_time_all(in_p)
...
kr=1; kc=1;    
if do_log
    y=seconds(data_plot.y);
else
    y=data_plot.y;
end
if do_rel
    x=flownodes./data_plot.x;
else
    x=data_plot.x;
end

scatter(x,y,10,[data_plot.nodes],'filled')

%for leg
nodes_u=unique(data_plot.nodes);
for k=1:numel(nodes_u)
    idx=find(data_plot.nodes==nodes_u(k),1,'first');
    han.p(kr,kc,k)=scatter(x(idx),y(idx),10,data_plot.nodes(idx),'filled');
    leg_str{k}=sprintf('# nodes = %2d',nodes_u(k));
end
