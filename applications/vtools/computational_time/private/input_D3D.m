...
simdef.file.software='p:\d-hydro\dimrset\weekly\2.26.14.78891\';
...
simdef.grd.L=               2500; %domain length [m] [10,100]
simdef.grd.B=               simdef.grd.L; %width [m]; [0.01,0.05,0.1,0.2,1.0,10]
simdef.grd.dx=              5; %dx [m]; [0.01,0.05,0.1,0.2,1.0,10]
simdef.grd.dy=              5; %dy [m]; [0.01,0.05,0.1,0.2,1.0,10]
    %type 2
...
simdef.mdf.Tstop=           10*3600; %simulation time [s] 
simdef.mdf.nparts_res=      10;
% simdef.mdf.Dt=              1; %time step [s] [dx,dt]=[10,10]; [1,2.5]; [0.2,0.5]; [0.1,0.2]; [0.05,0.1]; [0.01,0.02];
...
simdef.mdf.CFL=             5; %CFL number [-]
% simdef.mdf.Vicouv=          1/6*1*0.41*sqrt(simdef.mdf.g)/simdef.mdf.C*1; %horizontal eddy viscosity [m^2/s] [double(1,1)] e.g. [5]
simdef.mdf.Vicouv=          0.05; %horizontal eddy viscosity [m^2/s] [double(1,1)] e.g. [5]
simdef.mdf.Dicouv=          0.05; %horizontal eddy diffusivity [m^2/s] [double(1,1)] e.g. [5]
% simdef.mdf.Vicouv=          elder(1,1,0.007); %horizontal eddy viscosity [m^2/s] [double(1,1)] e.g. [5]
% simdef.mdf.Dicouv=          elder(1,1,0.007); %horizontal eddy diffusivity [m^2/s] [double(1,1)] e.g. [5]
simdef.mdf.Vicoww=          0e-6; %vertical eddy viscosity [m^2/s] [double(1,1)] e.g. [1e-6]
simdef.mdf.Dicoww=          0e-6; %vertical eddy diffusivity [m^2/s] [double(1,1)] e.g. [1e-6]
simdef.mdf.Smagorinsky=     0.0; %Smagorinsky factor in horizontal turbulence [-] [double(1,1)] e.g. [0.15]
...
simdef.mdf.filter=          0; %filter the 2dx oscillations due to numerical scheme in FM: 0=NO; 1=YES [-] [double(1,1)] e.g. [0]
    %output
% simdef.mdf.Flmap_dt=        10*60; %printing map-file interval time [s] [double(1,1)] e.g. [60]     
...
% simdef.mdf.Flrst_dt=        0; %restart file time interval [s] [double(1,1)] e.g. [60]
...
simdef.mdf.Removesmalllinkstrsh=0;
...
simdef.mor.ASKLHE=          0.0; %Power Law hiding factor (in case simdef.mor.hiding=4). ATTENTION! in ECT is defined with a change of sign!
simdef.mor.MorStt=          10*3600; %spin-up time [s] [double(1,1)] e.g. [1800]
...
% simdef.ini.etaw_file='etaw.xyz'; %relative to mdu
    %etab
simdef.ini.etab0_type=      1; %type of initial bed elevation: 1=sloping bed; 2=constant bed elevation; 3=xyz 
simdef.ini.u=               1;
simdef.ini.v=               0;
    q=simdef.ini.h.*simdef.ini.u;
simdef.ini.s=               q^2/simdef.ini.h.^3/simdef.mdf.C^2; %(if etab0_type=1) initial slope [-] [double(1,1)] | [double(M,1)] e.g. [3e-3] | linspace(0.001,0.0001,simdef.grd.L/simdef.grd.dx+1)]
...
simdef.ini.etab_noise=      6; %flag for initial noise [double(1,1)] 0=NO; 1=random; 2=alternate bars; 3=random including upstream end; 4 trench
simdef.ini.noise_amp=       0.001; %noise amplitude (also bar height, trench height) [m] In case of trench, positive is trench and negative is hump.
simdef.ini.noise_x0=        [simdef.grd.L/2,simdef.grd.B/2];
simdef.ini.noise_Lb=        50; %(for etab_noise=2) bar length [m]
...
simdef.mor.IBedCond=        1; %flag for upstream boundary bed boundary condition: 
                               %1=bed level fixed; 
...
simdef.bct.Q=               q*simdef.grd.B; %Q [m3/s] (value before width is the value per unit width)
...
% simdef.mdf.Dpuopt='mean_dps'; %bed level interpolation at velocity    points [string]: MIN=minimum of the surrounding point; MEAN=mean of the surrounding points; UPW=upwind; MOR=MIN; mean_dps=average based on cell centre values. 
simdef.mdf.Dpuopt=1; %bed level interpolation at velocity    points [string]: MIN=minimum of the surrounding point; MEAN=mean of the surrounding points; UPW=upwind; MOR=MIN; mean_dps=average based on cell centre values. 
