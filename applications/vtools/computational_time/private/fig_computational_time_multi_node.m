...
function fig_computational_time_multi_node(in_p)
...
xlabels{kr,kc}='# nodes [-]';
if do_rel
    ylabels{kr,kc}='relative time in time loop [-]';
else
    if do_log
        ylabels{kr,kc}='time in time loop [s]';
    else
        ylabels{kr,kc}='time in time loop';
    end
end
...
kk=1;
for kl=1:nl
    if numel(data_plot(kl).x)<2
        continue
    end
    if do_rel
        y=seconds(data_plot(kl).val_mean)./seconds(data_plot(kl).val_mean(1));
        ys=[];
    else
        if do_log
            y=seconds(data_plot(kl).val_mean);
            ys=seconds(data_plot(kl).val_std);
        else
            y=data_plot(kl).val_mean;
            ys=data_plot(kl).val_std;
        end
    end
%     seconds(data_plot(kl).val_mean)./seconds(data_plot(kl).val_mean(1))
    han.p(kr,kc,kk)=errorbar(data_plot(kl).x,y,ys,ys,'parent',han.sfig(kr,kc),'color',cmap(kk,:));
    leg_str{kk}=data_plot(kl).leg;

    kk=kk+1;
%     han.p(kr,kc,kl)=plot(data_plot(kl).x,y,'parent',han.sfig(kr,kc),'color',cmap(kl,:),'linewidth',prop.lw1,'linestyle',prop.ls1,'marker',prop.m1);
end
