...
% $Author: ormondt $
% $Revision: 16899 $
% $HeadURL: https://svn.oss.deltares.nl/repos/openearthtools/trunk/matlab/applications/DelftDashBoard/main/operations/ddcompile4_SFINCS.m $
% $Keywords: $
...
compilefolder='c:\Users\leijnse\Documents\delftdashboard_sfincs_v00_01\'; % Folder where executable and data will be created

% addpath(genpath('d:\checkouts\ShorelineS\trunk\'));

include_additional_toolboxes=0;
include_additional_models=0;
revisionnumber='18504';
...
for j=1:length(flist)
    if flist(j).isdir
        model=flist(j).name;
%         if strcmp('sfincs',model) %only include SFINCS
...
%         if strcmp('ModelMaker',toolbox) || strcmp('Bathymetry',toolbox) || strcmp('Drawing',toolbox)  %only include ModelMaker, Bathymetry  & Drawing
...
ddb_copyAllFilesToDataFolder(inipath,[compilefolder filesep 'data' filesep],[]);
