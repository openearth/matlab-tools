...
%
  if foption(1:1) == 'F'
    ...
    [fxn(1),fyn(1)] = ddb_ddb_det_nxtvrtx(faultX(1)  , faultY(1)  , strike(1)+90., fd);
    for i=1:nseg
      [fxn(i+1),fyn(i+1)] = ddb_ddb_det_nxtvrtx(faultX(i+1)  , faultY(i+1)  , strike(i)+90., fd);
    end
    for i=1:nseg
...
%
     fw = fwidth*cos(dip(1)*degrad);
     [xvtc,yvtc]=ddb_ddb_det_nxtvrtx(lon_epi, lat_epi, strike(1)-90., fw/2);
