...
% subgrd.z_volmax=zeros(ni*nib,nj*njb);
% subgrd.z_dep=zeros(ni*nib,nj*njb,nbin);
...
% subgrd.u_depth=zeros(ni*nib,nj*njb,nbin);
% 
...
% subgrd.v_depth=zeros(ni*nib,nj*njb,nbin);
...
%         yy1=yy1+di; % add extra row cuz we need data in v points
        ...
        if ~isempty(cs)
            ...
            for ibat=1:length(model.bathymetry)
                ...
                %             ddxx=(bboxx(2)-bboxx(1))/20;
                %             ddyy=(bboxy(2)-bboxy(1))/20;
                %             bboxx(1)=bboxx(1)-ddxx;
                %             bboxx(2)=bboxx(2)+ddxx;
                %             bboxy(1)=bboxy(1)-ddyy;
                %             bboxy(2)=bboxy(2)+ddyy;
