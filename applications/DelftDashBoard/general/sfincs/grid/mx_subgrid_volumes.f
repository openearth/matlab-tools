...
C
#if 0
C     generate with :  mex mx_subgrid_volumes.f
C     
C     crvec.f
...
c           Loop through depths
            ...
            do j1 = 2, np
               ...
               vvv = 0.0
               do j2 = 1, j1
                  vvv = vvv + (lev(j1) - dd(j2))*a
               enddo
               vol(j1) = vvv
               vol(j1) = max(vol(j1), vol(j1 - 1) + 1.0e-9)
            enddo
            ...
            do ibin = 1, nbin - 1
               ...
               depth(n,m,ibin) = ddd
            enddo
            depth(n,m,nbin) = zmax(n,m)

         enddo
      enddo
      
c     Now smooth extremely steep slopes

      do n = 1, nmax
         do m = 1, mmax

            volmax1 = volmax(n, m)/dx**2
            dv      = volmax1/nbin
            maxdzdv = 0.0
...
c              need to adjust profile
               ...
               do ibin = 1, nbin
c                 Set vvru in this bin to be higher than in bin below
...
c              Now re-interpolate

               do ibin = 1, nbin + 1
                  vvr(ibin)   = vvr(ibin)*dx**2
               enddo   
               volmax(n,m) = vvr(nbin + 1)
               dv          = volmax(n,m)/nbin

c               do ibin = 1, nbin + 1
c                  write(801,'(3i5,20e14.4)')n,m,ibin,vvr(ibin),dep(ibin),dv
c               enddo
