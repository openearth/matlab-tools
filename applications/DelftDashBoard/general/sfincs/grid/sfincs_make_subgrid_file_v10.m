function subgrd1 = sfincs_make_subgrid_file_v10(dr,subgridfile,bathy,manning_input,cs,nbin,refi,refj,uopt,maxdzdv,usemex,varargin)
...
for ii=1:length(varargin)
    if ischar(varargin{ii})
        switch lower(varargin{ii})
            ...
            case{'depth_factor'}
                depth_factor=varargin{ii+1};
            case{'manning_deep_files'}
                manning_deep_files=varargin{ii+1};
            case{'manning_deep_values'}
                manning_deep_values=varargin{ii+1};
            case{'quiet'}
                quiet=varargin{ii+1};
        end
    end
...
for ii=1:ni
    for jj=1:nj
        ...
        % Set Manning in deep water inside polygons
        if ~isempty(manning_deep_files)
            for jm=1:length(manning_deep_files)
                [xpol,ypol]=landboundary('read',manning_deep_files{jm});
                inpol=inpolygon(xg0,yg0,xpol,ypol);
                manning(zg<manning_level&inpol)=manning_deep_values(jm);
            end
        end
...
%         manning(ideep)=min(manning(ideep), 0.024);
        ...
        if ~isempty(find(isnan(zg), 1)) || ~isempty(find(isnan(manning), 1))
            ...
            if ~isempty(isn)
                if length(isn)<0.1*size(xg,1)*size(xg,2)
                    ...
                    disp('Small number of NaNs found in data. Trying internal diffusion ...');
        ...
        [zmin,zmax,volmax,ddd]=mx_subgrid_volumes_05(d,cellareas,nbin,dx,dy,maxdzdv);
