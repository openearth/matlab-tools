function msk=sfincs_make_mask_flosup(x,y,z,zlev,varargin)
...
% remove parts not in include polygon and above xvel(2)
if ~isempty(xy_in)
    msk(z>zlev(2)& ~inp)=0; % everything in include polygon we want to keep independent of elevation    
    z(z>zlev(2)& ~inp)=NaN;
else
    msk(z>zlev(2))=0;
    z(z>zlev(2))=NaN;    
...
% Set boundary points to closed > overrules msk=2
...
% if ~isempty(xy_in)
%     msk(z>zlev(2)& ~inp & ~onp & msk~=2 & msk~= 3)=0; % everything in include polygon we want to keep
...
msk(isnan(z))=0;
A4fig; axis equal; scatter(x(:),y(:),7,msk(:),'filled'); shading flat; colorbar;xlim([minmin(x),maxmax(x)]);ylim([minmin(y),maxmax(y)]);
% 
% A4fig; axis equal; pcolor(x,y,msk); plot(xp,yp,'r'); shading flat; colorbar;xlim([minmin(x),maxmax(x)]);ylim([minmin(y),maxmax(y)]);
% 
