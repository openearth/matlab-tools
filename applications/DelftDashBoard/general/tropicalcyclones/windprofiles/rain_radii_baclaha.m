function [pmax_out,pr] = rain_radii_bacla_new(meas_vmax, rmax, radius, probability, tp)
...
rng('shuffle') % added by Arish for making it random
...
[id1, id2] = size(radius);
if id1 == 1
    inp.radius  = radius'; %TL temp: check whether this is okay
else
    inp.radius  = radius;
end
inp.rmax    = rmax;
 ...
 for ii = 1:length(pmax_sample)
     ...
     if tp.data == 1 %GPM/TRMM data trained model
         ...
         elseif tp.split ==4 % xn forced to get pmax fit, bs based on area under graph
          ...
          elseif tp.split == 5 %xn forced to get pmax fit, bs based on area under graph 
             if pmax_sample(ii) >= 5.0 %Introduced by Arish the thresold for fit
                bsi = 2 .* rand(1000,1);
                xn = log(inp.pmax)/log(inp.pmax/exp(1));
                area = 13.208 + (19.101 * log(inp.pmax)) +  (19.101 * (log(inp.pmax)^2));
                Ai = [];
                ...
                [I, D] = min(dif);
                betaA       = [xn bsi(D)];
             else %method A for peak amount of rainfall below 5mm/hr
                 bs          = 2.204.* pmax_sample(ii).^-0.231; 
                 xn          = 0.788.*pmax_sample(ii).^0.216; 
                 betaA       = [xn bs];
             end
