...
%% lookat
   ...
   if ~(isempty(OPT.LookAtLon))
      ...
      '  <range>%g</range>\n'...
      '  <tilt>%g</tilt>\n'...
      '  <heading>%g</heading></LookAt>\n'],OPT.LookAtLon,OPT.LookAtLat,OPT.LookAtAltitude,OPT.LookAtrange,OPT.LookAttile,OPT.LookAtheading);
   else
      LookAt = '';
   end
...
%% type HEADER
   ...
   output = sprintf([...
    ...
    '<!-- Created with Matlab (R) googlePlot toolbox $Revision$ $Date$ from OpenEarthTools http://www.OpenEarth.eu-->\n',...
    '<Document>\n'...
    '%s %s'...
    '<name>%s</name>\n'...
    '<snippet>%s</snippet>\n'...
