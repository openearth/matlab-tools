...
%SHAPE_READ  reads an arcview shapefile into a Matlab structure 
%
%    R = shape_read(filename)
%
% where filename is an arcview file name without the extension.
% The struct R has the following field names:
%
%  R.npoly     = a scalar with # of polygon regions or data observations on the map
%  R.nvars     = # of variables from the dbf file [should = the length(vnames)]
%  R.nobs      = # of observations from the dbf file [should = npoly]
%  R.xmin      = an nobs-vector of minimum longitude for each region
%  R.xmax      = an nobs-vector of maximum longitude for each region
%  R.ymin      = an nobs-vector of minimum lattitude for each region
%  R.ymax      = an nobs-vector of maximum lattitude for each region
%  R.nvertices = an nobs-vector of # of vertices for each region
%  R.nparts    = an nobs-vector of # of parts for each region
%  R.xc        = an nobs-vector with x-centroid for each polygon/ region
%  R.yc        = an nobs-vector with y-centroid for each polygon/ region
%  R.data      = (an nobs=npoly by nvars) matrix of sample data observations for each polygon/ region
%  R.vnames    = variable names for data vectors from dbf file
%
%  R.x         = an (n by nvertices) vector of polygon points, with NaN separators
%  R.y         = an (n by nvertices) vector of polygon points, with NaN separators
%               (the same as poly(i).handles(i), but organized a one long vector
%                these can be used as carte argument to the geoxp functions carte = [R.x R.y])
%
% Example: to load and plot a map involving npoly=nobs sample data observations
%
%    R    = shape_read('myarcfile');
%    plot(S.x,S.y)
%
% SHAPE_READ is part of <a href="http://www.spatial-econometrics.com/">arc_mat</a> and has been <a href="http://dx.doi.org/10.1007/s10109-009-0096-6">published</a>.
%
% see also: arc_histmap, arc_moranplot
...
results.npoly    = npoly;
results.xc       = xc;clear xc;
results.yc       = yc;clear yc;
