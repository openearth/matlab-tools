...
%             .SLRfile        (optional) Cell-string with BCI-file (with one filename per phase) {Nx1}
...
if isfield(CL,'SLRfile')
    fldnms=[fldnms,'SLRfile']; 
end
...
for kk=1:CL.no_phases; 
    ...
    if isfield(CL,'SLRfile')
        [pthnm, CL.SLRfile{kk}, extnm] = fileparts(CL.SLRfile{kk});
    end
end
...
for ii=1:CL.no_phases 
    if ~isfield(CL,'SLRfile') 
        ...
        fprintf(fid,'  %4.0f  %8.3f  %8.3f    ''%s''   ''%s''   ''%s''   ''%s''   ''%s''   ''%s''   ''%s''   ''%s''\n',ii,CL.tstart(ii),CL.tend(ii),CL.GKLfile{ii},CL.BCOfile{ii},CL.GROfile{ii},CL.SOSfile{ii},CL.REVfile{ii},CL.OBWfile{ii},CL.BCIfile{ii},CL.SLRfile{ii});
    end 
end 
fprintf(fid,'%s\n','iaant    ifirst    ival'); 
