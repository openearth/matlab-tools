...
xStats.min = nanmin(x,[],dim);
xStats.max = nanmax(x,[],dim);
xStats.n = sum(~isnan(x),dim);
xStats.prc05 = getprctile(x,5,dim);
xStats.prc50 = getprctile(x,50,dim);
xStats.prc95 = getprctile(x,95,dim);
xStats.mean  = nanmean(x,dim);
xStats.var  = nanvar(x,1,dim);
xStats.std  = nanstd(x,1,dim);

if strcmp(statId,'error');
   xStats.mse  = nanmean(x.^2,dim);
...
%--------------------------------------------------------------------------
function xperc = getprctile(x,prc,dim)
...
if dim==0
   xperc = perctile(x,prc);

elseif dim==1
    for i3 = 1:d3
        for i = 1:d2
            xperc(1,i,i3) = perctile(x(:,i,i3),prc);
        end
    end
elseif dim==2
    for i3 = 1:d3
        for i = 1:d1
            xperc(i,1,i3) = perctile(x(i,:,i3),prc);
        end
    end
elseif dim==3
    for i = 1:d3
        xperc(1,1,i) = perctile(x(:,:,i),prc);
    end
end
...
elseif n==1
   xperc = x;
elseif n>=2
    xperc = interp1((1:n)/n,x,(prc/100),'nearest','extrap');
end
