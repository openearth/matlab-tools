...
switch s.selection.opt
    ...
    case 2        
        obs_nr = 0;
        measSelection = find(meas.Y>= min(s.model.data.YAll) & meas.Y<= max(s.model.data.YAll) & meas.X >= min(s.model.data.XAll) & meas.X <= max(s.model.data.XAll));   % first selection of s.meas stations in the area
