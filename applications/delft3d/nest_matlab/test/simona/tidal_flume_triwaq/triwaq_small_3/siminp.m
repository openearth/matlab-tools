function S=siminp(filename,file_waquaref,exclude_timeseries,varargin)
...
%
% Read main SIMINP file
%
[P,N,E] = fileparts(filename);
filename = [N E];
S = readsiminp(P,filename,exclude_timeseries);
...
%
% Exclude timeseries (in include file) to avoid large parsing times
%

if exclude_timeseries

    no_series = strfind(lower(S.File),'timeseries');

    for i_series = 1: length(no_series)
        if ~isempty(no_series{i_series})
            S.File{i_series}     = [];
            i_add = 1;
            while ~isempty(strfind(lower(S.File{i_series + i_add}),'include'))
                S.File{i_series + i_add} = [];
                i_add = i_add + 1;
            end
    ...
    i_new = 0;
    for i_field = 1: length(S.File)
        if ~isempty(S.File{i_field})
            i_new = i_new + 1;
            Hulp{i_new} = S.File{i_field};
        end
    end
