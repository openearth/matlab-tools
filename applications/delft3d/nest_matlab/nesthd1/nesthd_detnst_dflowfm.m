      function [names,weight,x_nes,y_nes] = nesthd_detnst  (x,y,name_coarse,xbnd,ybnd,sphere)

      % detnst_dflowfm determines coordinates nest stations and belonging weight factors
      % x,y, are cell centres (water level points, flow nodes)
      ...
      for i_pnt = 1: no_pnt
          ...
          if ~isnan(xbsp)
              ...
              % create selection of neighbouring flow nodes
              neighbours    = [find(tri(:,1) == idualface); find(tri(:,2) == idualface); find(tri(:,3) == idualface)];
