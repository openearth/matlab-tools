...
dataA       = zeros(1,2);
dataB       = zeros(1,2);
wb          = waitbar(0,'Writing tim-files ...');
for i=1:I;
    tline   = fgetl(fid1);
	if tline<0;
	    break;
            ...
            if strcmp(location,loc{tellerA});
                for j=1:J;
                    ...
                    if strcmp(tline( 1:20),'records-in-table    ');
                        ...
                        dataA(1,:)   = [];
                        dataB(1,:)   = [];
                        nametimA     = [pathout,'/',plibasis,'_',num2str(perm(tellerA,1),'%0.2d'),'_',num2str(perm(tellerA,2),'%0.4d'),'.tim'];
                        nametimB     = [pathout,'/',plibasis,'_',num2str(perm(tellerB,1),'%0.2d'),'_',num2str(perm(tellerB,2),'%0.4d'),'.tim'];
                        ...
                        if tellerA>size(perm,1);
                            close(wb);
                            msgbox('Timeserie files have succesfully been generated.','Message');
                            return;
                        end
                        break;
                    end
