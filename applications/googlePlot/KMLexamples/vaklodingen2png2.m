...
outputDir      = 'D:\repositories\kml\vaklodingen2';
url            = vaklodingen_url;
EPSG           = load('EPSG');

for ii = 145:154%length(url):-1:1;
    [path, fname] = fileparts(url{ii});
    x    = nc_varget(url{ii},   'x');
    y    = nc_varget(url{ii},   'y');
    time = nc_varget(url{ii},'time');KML

    % expand x and y 10 m in each direction
    x = [x(1) + (x(1)-x(2))/1.5; x; x(end) + (x(end)-x(end-1))/1.5];
    y = [y(1) + (y(1)-y(2))/1.5; y; y(end) + (y(end)-y(end-1))/1.5];
    % coordinates:
    [X,Y] = meshgrid(x,y);
    ...
    for jj = size(time,1)%:-1:size(time,1)+1 - min(size(time,1),3)   ;
         if ~exist([outputDir filesep fname '_' date(jj,:) '.kml'],'file')
            ...
            clim([-25 25]);
            KMLfig2png(h,'levels',4,'timeIn',date(jj,:),'timeOut',date(jj+1,:),...
