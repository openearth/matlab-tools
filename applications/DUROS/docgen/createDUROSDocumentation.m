...
close all
mkdir(fullfile(DUROSdocroot,'helpdocs','tocpages'));
copyfile(fullfile(fileparts(mfilename('fullpath')),'TRDA.pdf'),...
    tempdir);
movefile(fullfile(tempdir,'*.*'),...
    fullfile(DUROSdocroot,'helpdocs','tocpages'));
rmdir(tempdir,'s');
...
tbd.listitems(2) = tblistitem(...
    'label','Help',...
    'icon','DeltaresLogo16.PNG',...
    'call','web(fullfile(DUROSdocroot,''helpdocs'',''tocpages'',''getDuneErosion_doc.html''),''-helpbrowser'');');
...
buildhelpjar(fullfile(DUROSdocroot,'helpdocs'));

rmdir(fullfile(DUROSdocroot,'helpdocs','backup'),'s');

builddocsearchdb(fullfile(DUROSdocroot,'helpdocs'));