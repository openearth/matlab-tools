...
tr(7) = addvolumecase7(xInitial,zInitial,x0DUROS,WL_t,precision,maxiter,slope,poslndwrd);
...
figure('Color','w');
title('Testcase 1: Basic calculation of additional volume');
hold on
ylim([0 16]);
xlim([-150, 250]);
...
figure('Color','w');
title('Testcase 2: Additional volume restricted by maximum length');
hold on
ylim([0 16]);
xlim([-150, 250]);
plot(xInitial,zInitial,'DisplayName','Initial profile','Color',[255 222 111]/255,'LineWidth',2);
...
%% $Description (Name = x0 Duros within value, but also restricted in value)
...
function testresult = addvolumecase7(xInitial,zInitial,x0DUROS,WL_t,precision,maxiter,slope,poslndwrd)
