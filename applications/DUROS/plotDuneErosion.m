...
for i = 1 : LastFilledField
    ...
    %     0 :   'Iteration boundaries are non-consistent'(**) ==> -99
    %     2 :   Channel slope zit in de weg ==> This code means shortage of info at landward side now
    %     100 : landwaards transport correctie boven water ==> onzin oplossing ==> -5
    TODO('This is really old. The codes aren''t even correct. Why did we have this?');
    NoMessages = isempty(result(1).info.messages);
    if ~NoMessages
        NaNMessageCodes = ~isempty(find(isnan(vertcat(result(1).info.messages{:,1})), 1));
        IterationBoundariesNonConsistent = any(strcmp(result(1).info.messages(:,2),'Iteration boundaries are non-consistent'));
    end
    if  NoMessages ||...
            ~(...
            NaNMessageCodes ||... % does not exist enymore
            IterationBoundariesNonConsistent ||...
            ~isempty(find(vertcat(result(1).info.messages{:,1})==2, 1))... % Code 2 used to be channel slope. Now it is shortage of landward info... Why don't we do this than?
            )
