function getAdditionalErosion_test
%% Simple profile with valley (DUROS result) and conditions
...
%% case 1 just determin additional volume
TargetVolume = -100.81;
...
writemessage init
fig(1) = figure;
plotDuneErosion(result,fig(1));

%% case 2 determin additional volume restricted by the maximum length
TargetVolume = -100.81;
maxRetreat = 10;
result = getAdditionalErosion(xInitial,zInitial,...
    'TargetVolume',TargetVolume,...
    ...
    'zmin',WL_t);
fig(2) = figure;
plotDuneErosion(result,fig(2));

%% case 3 determine additional volume restricted by the maximum length in a dune valley
TargetVolume = -100.81;
maxRetreat = 20;
result = getAdditionalErosion(xInitial,zInitial,...
    'TargetVolume',TargetVolume,...
    ...
    'zmin',WL_t);
fig(3) = figure;
plotDuneErosion(result,fig(3));

%% case 4 x0 Duros is inside a valley
TargetVolume = -100.81;
maxRetreat = 500;
zTemp = zInitial;
...
result = getAdditionalErosion(xInitial,zTemp,...
    ...
    'zmin',WL_t);
fig(4) = figure;
plotDuneErosion(result,fig(4));

%% case 5 x0 Duros is inside a valley. but also resticted within a valley
TargetVolume = -100.81;
maxRetreat = 30;
zTemp = zInitial;
zTemp(xInitial > -70 & zInitial > WL_t) = WL_t - 1;
...
if result.info.resultinboundaries
    disp('This shouldn''t be true');
end

%% case 6 Complete profile below the water line
TargetVolume = -100.81;
maxRetreat = 30;
zTemp = zInitial;
...
%% case 7 Now try the other direction
xTemp = 0-xInitial;
maxRetreat = 20;
...
...    'poslndwrd',-poslndwrd,...
    ...
    'zmin',WL_t);
fig(7) = figure;
plotDuneErosion(result,fig(7));

%% case 8 try other profile with poslndwrd == 1
xInitial = (0:5:6300)';
