...
if isunix()
    ...
    elseif strcmpi(OPT.ssh_host,'h5')
        if isempty(OPT.queue)
            cmd = sprintf('%s %s@%s -pw %s -batch "qsub -N %s %s/%s"', ...
                exe_path, OPT.ssh_user, OPT.ssh_host, OPT.ssh_pass, OPT.name, rpath, script);
        else
            cmd = sprintf('%s %s@%s -pw %s -batch "qsub -q %s -N %s %s/%s"', ...
                exe_path, OPT.ssh_user, OPT.ssh_host, OPT.ssh_pass, OPT.queue, OPT.name, rpath, script);
        end
    else
