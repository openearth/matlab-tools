function description = sdn_verify(sdn)
%SDN_VERIFY   verifies SeaDataNet vocabulary term on http://vocab.ndg.nerc.ac.uk webservice (BETA)
%
%    xml = sdn_verify(sdn)
%
% verifies SeaDataNet vocabulary term on http://vocab.ndg.nerc.ac.uk webservice
% by requesting description xml.
%
% Example:
% description = sdn_verify('SDN:P061::UPBB')
% description = sdn_verify('SDN:P011::PRESPS01')
%
% Sometime a sterm issues by SDN is not in the vocabulary, e.g.:
...
% http://vocab.ndg.nerc.ac.uk/axis2/services/vocab/getRelatedRecordByTerm?subjectTerm=http://vocab.ndg.nerc.ac.uk/term/P011/current/EWDAZZ01&predicate=1&inferences=true
   ...
   index            = strfind(sdn,':');
   listReference    =  sdn(index(1)+1:index(2)-1);% e.g.'P011';
   listVersion      =  'current';
   entryReference   =  sdn(index(3)+1:end);       % e.g.'EWDAZZ01';
