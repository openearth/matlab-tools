...
%   cellstr_fields = {'longitude','latitude','z','datestring','timestring','variable'}
    ...
    while true
        ...
        if isempty(temp{numcol})
            ...
            %data(:,numcol+3:numcol+5) = [year(data(:,dateCol)+datenum('01-Jan-1970')),month(data(:,dateCol)+datenum('01-Jan-1970')),yearweek(data(:,dateCol)+datenum('01-Jan-1970'))];
    ...
    function [] = make_field(name_of_field)
        ...
        thecompend.(name_of_field).referenceDate = datenum('01-Jan-1970','dd-mmm-yyyy');
        ...
        thecompend.(name_of_field).dimensions(ismember(thecompend.(name_of_field).dimensions(:,1),'datestring'),:) = {'time',['days since ',datestr(datenum('01-Jan-1970'),'yyyy-mmm-dd HH:MM:SS'),' ',timezone]};
