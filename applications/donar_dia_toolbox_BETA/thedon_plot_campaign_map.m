%% Make time plots and histograms.
    ...
    %%
        ...
        donar_plot_campaign_maps(ctd,'CTD','turbidity',[pwd,filesep],20,20);
        title('CTD: Upoly0 [-]','fontweight','bold')
        fileName = 'themap_CTD_turbidity';
        print('-dpng',fileName);
        close;

        donar_plot_campaign_maps(ferry,'FerryBox','turbidity',[pwd,filesep],20,20);
        title('FerryBox: Turbidity [NTU]','fontweight','bold')
        fileName = 'themap_FerryBox_turbidity';
        print('-dpng',fileName);
        close;

        donar_plot_campaign_maps(scan,'ScanFish','turbidity',[pwd,filesep],20,20);
