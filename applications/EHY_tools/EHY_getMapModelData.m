...
if OPT.mergePartitions==1 && strcmp(modelType,'dfm') && strcmp(fileInp(end-6:end),'_map.nc') && ~isempty(str2num(fileInp(end-10:end-7)))
    mapFiles=dir([fileInp(1:end-11) '*' fileInp(end-6:end)]);
    for iM=1:length(mapFiles)
        disp(['Reading and merging map model data from partitions: ' num2str(iM) '/' num2str(length(mapFiles))])
        mapFile=[fileparts(fileInp) filesep mapFiles(iM).name];
        DataPart=EHY_getMapModelData(mapFile,varargin{:},'mergePartitions',0);
        if iM==1
            Data=DataPart;
        else
            Data.value=horzcat(Data.value,DataPart.value);
            if isfield(Data,'ucx')
                Data.ucx=horzcat(Data.ucx,DataPart.ucx);
                Data.ucy=horzcat(Data.ucy,DataPart.ucy);
            end
        end
    end
    Data.OPT.mergePartitions=1;
    modelType = 'Do not load new data, this was a partitioned run';
end
    ...
    case 'dfm'
        ...
        switch OPT.varName
            ...
            case {'mesh2d_Numlimdt','numlimdt'}
                ...
                elseif ismember('numlimdt',{infonc.Variables.Name}) % old format
            ...
            otherwise
                if nc_isvar(fileInp,OPT.varName)
                    ...
                    error(['Could not find variable or group ''' OPT.varName ''' in file.'])
                end
        end
