...
if isempty(Model.FlowRstFile) && Model.makeIniFile
    ...
    makeBctBccIni('ini','nestxml',[tmpdir 'nest.xml'],'inpdir',tmpdir,'runid',Model.Runid,'workdir',tmpdir);
...
if ~strcmpi(Model.UseMeteo,'none')
    ...
    WriteD3DMeteoFile2(meteodir,Model.UseMeteo,tmpdir,'meteo',Model.XLim,Model.YLim,dx,dy,coordsys,coordsystype,Model.RefTime,Model.TFlowStart,Model.TStop,dt,hstr);
end
            ...
            for ip=1:np
                ...
                if ~exist(nstobs,'file')
                    ...
                    if ~strcmpi(hm.Models(i).CoordinateSystem,Model.CoordinateSystem) || ~strcmpi(hm.Models(i).CoordinateSystemType,Model.CoordinateSystemType)
                        % Convert coordinates
                        [xg,yg]=ConvertCoordinates(xg,yg,'CS1.name',hm.Models(i).CoordinateSystem,'CS1.type',hm.Models(i).CoordinateSystemType,'CS2.name',hm.Models(m).CoordinateSystem,'CS2.type',hm.Models(m).CoordinateSystemType);
                    end
