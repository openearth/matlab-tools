...
if ~strcmpi(Model.CoordinateSystemType,'geographic')
    [lon,lat]=ConvertCoordinates(Model.WebSite(1).Location(1),Model.WebSite(1).Location(2),'CS1.name',Model.CoordinateSystem,'CS1.type',Model.CoordinateSystemType,'CS2.name','WGS 84','CS2.type','geographic');
    fprintf(fid,'%s\n',['Anglat= ' num2str(lat)]);
end
...
if Model.includeSalinity || Model.includeTemperature
