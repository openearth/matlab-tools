...
for j=1:n
    ...
    if ~strcmpi(hm.Models(i).Type,'ww3') && hm.Models(i).Priority>0
        ...
        if ~strcmpi(hm.Models(i).CoordinateSystem,Model.CoordinateSystem) || ~strcmpi(hm.Models(i).CoordinateSystemType,Model.CoordinateSystemType)
            ...
            xx=pnts(:,1);
            yy=pnts(:,2);
            [xx,yy]=ConvertCoordinates(xx,yy,'CS1.name',hm.Models(i).CoordinateSystem,'CS1.type',hm.Models(i).CoordinateSystemType,'CS2.name',Model.CoordinateSystem,'CS2.type',Model.CoordinateSystemType);
            pnts(:,1)=xx;
            pnts(:,2)=yy;
        end
