...
%    drawnow;
    ...
    try
        ...
        fh.setFigureIcon(javax.swing.ImageIcon([d3dpath 'muppet\settings\icons\deltares.gif']));
    end
    ...
    d3dpath=[getenv('D3D_HOME') '\' getenv('ARCH') '\'];
    icons=load([d3dpath 'muppet\settings\icons\ico.mat']);
    c=icons.icon;
    ...
    set(h,'ClickedCallback',{@ZoomInOutPan,4}');
    set(h,'Tag','UIToggleToolPan');
    ...
    set(addellipse,'cdata',MakeIcon([d3dpath 'muppet\settings\icons\tool_ellipse.bmp'],30));
    ...
    cda=single(MakeIcon([d3dpath 'muppet\settings\icons\polyline.bmp'],18));
    a=sum(cda,3);
    cda(a>500)=NaN;
    cda=double(cda/255);
    set(h,'cdata',cda);
    ...
    cda=single(MakeIcon([d3dpath 'muppet\settings\icons\spline.bmp'],18));
    a=sum(cda,3);
    cda(a>500)=NaN;
    cda=double(cda/255);
    set(h,'cdata',cda);
    ...
    cda=single(MakeIcon([d3dpath 'muppet\settings\icons\curvec2.bmp'],18));
    a=sum(cda,3);
    cda(a>500)=NaN;
    cda=double(cda/255);
    set(h,'cdata',cda);
