...
%
%    CMStructOut=MD_COLORMAP(CMStructIn)
%    Starts colormap editor with CMStructIn as
%    active colormap (default a structure
%    representation of the colormap JET).
%
%    The colormap structure can be converted into
%    a normal colormap with help of the CLRMAP
%    function.
%
%    CMStructOut=MD_COLORMAP(CMStructIn);
...
switch cmd
   ...
   case {'red','green','blue','select','cyan','magenta','yellow'}
      switch cmd
         ...
         case {'red','green','blue'}
            RGB{3}=str2num(get(uih(3,1),'string'));
            RGB{2}=str2num(get(uih(2,1),'string'));
            RGB{1}=str2num(get(uih(1,1),'string'));
            if isequal(cellfun('prodofsize',RGB),[1 1 1])
               RGB=[RGB{:}]/255;
            else
               RGB=0;
            end
         case {'cyan','magenta','yellow'}
            CMY{3}=str2num(get(uih(3,4),'string'));
            CMY{2}=str2num(get(uih(2,4),'string'));
            CMY{1}=str2num(get(uih(1,4),'string'));
            if isequal(cellfun('prodofsize',CMY),[1 1 1])
               CMY=[CMY{:}]/255;
               RGB=1-CMY;
            else
               RGB=0;
   ...
   case {'hsv-hue','hsv-saturation','value'}
      HSV{3}=str2num(get(uih(3,2),'string'));
      HSV{2}=str2num(get(uih(2,2),'string'));
      HSV{1}=str2num(get(uih(1,2),'string'));
      if isequal(cellfun('prodofsize',HSV),[1 1 1])
         HSV=[HSV{:}]/255;
      else
         HSV=0;
      end
   ...
   case {'hls-hue','lightness','hls-saturation'}
      HLS{3}=str2num(get(uih(3,3),'string'));
      HLS{2}=str2num(get(uih(2,3),'string'));
      HLS{1}=str2num(get(uih(1,3),'string'));
      if isequal(cellfun('prodofsize',HLS),[1 1 1])
         HLS=[HLS{:}]/255;
      else
         HLS=0;
      end
      if ~isequal(HLS,0)
         switch S.Space
            ...
            case 'CMY'
               S.Colors(currentcolor,:)=1-hls2rgb(HSV);
         end
      end    
   case 'name'
   ...
   case 'save'
      ...
      clrmap_path=[d3dpath 'muppet\settings\colormaps\'];

      [f,p] = uiputfile({'*.clrmap','Color Maps (*.clrmap)'},'Save as',[clrmap_path S.Name]);
...
function S1=md_colormap_interface(S,GCF)
if nargin==0
   S.Name='jet';
   S.Space='RGB';
...
if nargin<2
   ...
   elseif isequal(size(S),[1 1]) & ishandle(S)
      GCF=S;
...
elseif ~isequal(size(GCF),[1 1]) | ~ishandle(GCF)
...
uih=cv_edit(h0,Inactive,131,voffset,uih,3,2,'value'         ,'Value');
uih=cv_edit(h0,Inactive,261,voffset,uih,3,3,'HLS-saturation','Saturation');
...
uih=cv_edit(h0,Inactive,131,voffset,uih,2,2,'HSV-saturation','Saturation');
uih=cv_edit(h0,Inactive,261,voffset,uih,2,3,'lightness'     ,'Lightness');
...
uih=cv_edit(h0,Inactive,131,voffset,uih,1,2,'HSV-hue'       ,'Hue');
uih=cv_edit(h0,Inactive,261,voffset,uih,1,3,'HLS-hue'       ,'Hue');
