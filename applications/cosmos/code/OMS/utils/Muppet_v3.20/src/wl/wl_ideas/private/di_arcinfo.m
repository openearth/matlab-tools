function It=di_arcinfo(FileInfo,ax,itoptions)
% DI_ARCINFO is an interface for plotting items based on ARCINFO data
%
...
case 'coverage', % -------------------------------------------------------------------------------
  it=arcinfo('plot',FileData,ax);
  if ~isempty(it),
    itoptions.Name=get(it(1),'tag');
    itoptions.Type='coverage';
    set(it,'tag',Tag);
