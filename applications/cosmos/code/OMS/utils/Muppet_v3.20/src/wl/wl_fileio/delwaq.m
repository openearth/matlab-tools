...
%
if strcmp(Str(4,1:4),'T0: ')
    T0  = Str(4,:)-48;
    DVec= sscanf(Str(4,:),'T0: %d%*c%d%*c%d %d%*c%d%*c%d');
    Year  = DVec(1);
    Month = DVec(2);
    Day   = DVec(3);
    Hour  = DVec(4);
    Minute= DVec(5);
    Second= DVec(6);
    S.T0=datenum(Year,Month,Day,Hour,Minute,Second);
    NClockUnits=sscanf(Str(4,31:38),'%f');
    if isempty(NClockUnits)
        NClockUnits=1;
    end
    switch upper(Str(4,39))
        case {'S',' '}
            ClockUnit=1/(24*3600);
        case 'D'
            ClockUnit=1;
        ...
        case 'M'
            ClockUnit=1/(24*60);
        otherwise
            warning(sprintf('Unknown time unit: %s.',Str(4,39)));
            ClockUnit=0;
    end
    S.TStep=NClockUnits*ClockUnit;
else % default ref.date and timestep according GPP
    S.T0=datenum(1900,1,1,0,0,0);
    S.TStep=1/(24*3600);
end
%
...
if ~DelparPlot
    ...
    switch S.FormatType
        ...
        otherwise
            ...
            if allstr % most certainly this is a DelwaqHIS file ...
                S.FileType='DelwaqHIS';
            end
    end
    fseek(fid,tmploc,-1);
...
if strcmp(S.FileType,'DelwaqHIS')
    ...
    %
    for i=1:Nseg
        S.SegmentID(i)=fread(fid,1,'int32');
        S.SegmentName{i}=deblank(char(fread(fid,[1 20],'uchar')));
    end
    %
    % Skip end block size.
    %
...
if tim==0
    ...
    OTime=fread(fid,[S.NTimes 1],'int32',4*NTot+2*FormRecSize);
    %
    ...
    elseif nsubs>1 & nseg>1
        ...
        Data=fread(fid,[NTot S.NTimes],sprintf('%i*float32',NTot),4+2*FormRecSize);
        Data=reshape(Data,NSubs,NSeg,S.NTimes);
        Data=Data(subs,seg,:);
    elseif nsubs>1
        N=NSubs*(NSeg-1);
        fseek(fid,S.DataStart+FormRecSize+4+4*(seg-1)*NSubs,-1);
        Data=fread(fid,[NSubs S.NTimes],sprintf('%i*float32',NSubs),4*(1+N)+2*FormRecSize);
    ...
    elseif nseg>1
        ...
        for i=seg
            ...
            Data(i,:)=fread(fid,[1 S.NTimes],'float32',4*NTot+2*FormRecSize);
        end
        Data=reshape(Data,1,nseg,S.NTimes);
    else
        fseek(fid,S.DataStart+FormRecSize+4+4*(seg-1)*NSubs+4*(subs-1),-1);
        Data=fread(fid,[S.NTimes 1],'float32',4*NTot+2*FormRecSize);
    end
else
    %
    ...
    for ti=1:ntim
        tim1=tim(ti);
        fseek(fid,S.DataStart+(tim1-1)*S.NBytesBlock+FormRecSize,-1);
        ...
        elseif nseg>1
            ...
            tmpData=fread(fid,[1 NSeg],'float32',4*(NSubs-1));
