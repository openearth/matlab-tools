function It=di_morf(FileInfo,ax,itoptions)
% DI_MORF is an interface for plotting items based on a morf file
%
...
if nargin==2, % Possibility 4: PossibleItems=DI_XXX(FileInfo,Axes)
  ...
  plottypes= ...
    {'simulation process tree'                {'undefined'} };
...
case 'clock', % -------------------------------------------------------------------------------
case 'calendar', % -------------------------------------------------------------------------------
case 'simulation process tree', % -----------------------------------------------------------------
  it=get(morf('plot',FileData,ax),'children');
  itoptions.Name='simulation process tree';
  itoptions.Type='tree';
  set(it,'tag',Tag);
  set(it(1),'userdata',itoptions); % set option only to it(1) and keep the other ones empty
  if strcmp(axtype,'undefined'), xx_setax(ax,'XXX'); end;
otherwise, % ----------------------------------------------------------------------------------
