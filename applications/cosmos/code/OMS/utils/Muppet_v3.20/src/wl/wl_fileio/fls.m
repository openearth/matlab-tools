...
% FLS File operations for Delft-FLS and Sobek Overland files.
%        FileData = fls('open',FileName);
%           where FileName is the name of an mdf file,
%           incremental file, binary/ascii history file,
%           or cross-sections data file.
%           In case of an mdf file, the function reads and
%           checks data from the file and scans the directory
%           for simulation output.
%
%        Data = fls('inc',FileData,Field,Time);
%           determines the field at Time from the
%           incremental data file.
%        [Data,FileData] = fls('inc',FileData,Field,Time);
%           retrieves updates FileData (contains state
%           at retrieved time).
%
%        Data = fls('bin',FileData,'T');
%           reads time from the binary history file
%        Data = fls('bin',FileData,'Field',Station);
%           reads data from the binary history file, where
%           'Field' is one of 'S'(or 'Z'),'U','V','H'
%        Data = fls('bin',...,TimeIndex);
%           reads time/data from the binary history file
%           for specified time indices only
%
%        Data = fls('his',FileData,Station);
%           reads all data for a station from the ascii
%           history file
%
%        Data = fls('cross',FileData,CrossNr,Index);
%           reads data of specified cross-sections
%
%        Data = fls('bottom',FileData,Time);
%           determines bottom data at Time from dam break
%           data.
...
case {'read','open','readbin','readhis','readcross','readinc'},
  ...
  case '.inc',
    Structure=Local_open_incremental(filename);
  case '.bin',
...
case 'inc',
  [Data,FI2]=Local_read_inc(varargin{:});
  if nargout>0,
    Out=Data;
  end;
case 'bin',
...
Path=filename(1:max(findstr(filename,filesep))); % extract path
...
if ~isempty(Line),
  ...
  while 1,
    ...
    case 5,
      ...
      i=i+1;
    case 3, % 
      BDTfile=[Path Line(max(findstr(' ',deblank(Line)))+1:end)];
...
if ~isempty(Line),
  Structure.Map.H=~isempty(findstr(Line(23:end),'H')); % waterdepth
  Structure.Map.U=~isempty(findstr(Line(23:end),'C')); % flow velocity
  Structure.Map.Z=~isempty(findstr(Line(23:end),'Z')); % waterlevel
  Structure.Map.u=~isempty(findstr(Line(23:end),'U')); % u-velocity
  Structure.Map.v=~isempty(findstr(Line(23:end),'V')); % v-velocity
  for c='HUZuv',
...
if ~isempty(Line),
  ...
  while ~isempty(Line) & strcmp(Line(1),' '),
    X=sscanf(Line,'%i',[1 2]);
    if length(X)<2, break; end; % empty line of spaces?
    Loc=Loc+1;
...
if ~isempty(Line),
  ...
  while ~isempty(Line) & strcmp(Line(1),' '),
    X=sscanf(Line,'%f',[1 5]);
    if length(X)<5, break; end; % empty line of spaces?
    if X(1)~=-999, Structure.Class.H(end+1)=X(1); end;
    if X(2)~=-999, Structure.Class.U(end+1)=X(2); end;
    if X(3)~=-999, Structure.Class.Z(end+1)=X(3); end;
    if X(4)~=-999, Structure.Class.u(end+1)=X(4); end;
    if X(5)~=-999, Structure.Class.v(end+1)=X(5); end;
    Line=fgetnel(fid);
  end;
  % The incremental file is '<base>.inc' file for Delft-FLS and
  % it is FLS_H.INC for Sobek overland.
  Structure.Class.FileInfo=Local_open_incremental([Structure.FileBase '.inc']);
  if strcmp(Structure.Class.FileInfo.Check,'NotOK'),
    % Sobek?
    [fp,fn]=fileparts(Structure.FileBase);
    Structure.Class.FileInfo=Local_open_incremental(fullfile(fp,'fls_h.inc'));
...
if ~isempty(Line),
  ...
  while ~isempty(Line) & strcmp(Line(1),' '),
    X=sscanf(Line,'%f',[1 4]);
    if length(X)<4, break; end; % empty line of spaces?
...
while ~feof(fid),
  Line=fgetnel(fid);
  if (length(Line)>length(Keyword)) & (~isempty(findstr(Line,Keyword))),
    return;
  end;
end;
Line='';
...
while ~feof(fid),
  ...
  if ~isempty(findstr(Line,Keyword)),
    % operator specified?
    Op=findstr(Line,'OPERATOR');
    if ~isempty(Op),
      Op=Op+8;
    ...
    if ~isempty(FN),
      ...
      if subfid>0,
        ...
        disp(['Cannot open the file ' FileName '.']);
        disp(['The data will be incomplete.']);
        Array=[];
      end;
    else,
      LUNI=findstr(Line,'UNI'); % UNIFORM
      LCBL=findstr(Line,'CBL'); % CBLOCK
      LVBL=findstr(Line,'VBL'); % VBLOCK
      LPOI=findstr(Line,'POI'); % POINT
      LCPO=findstr(Line,'CPO'); % CPOLYGON
      LVPO=findstr(Line,'VPO'); % VPOLYGON
      LARE=findstr(Line,'ARE'); % AREA
      LARC=findstr(Line,'ARC'); % ARCINFO
      LFRE=findstr(Line,'FRE'); % FREE FORMAT
  ...
  %        array(MN(2),MN(1))=Value;
        ...
        if ischar(Line), fseek(fid,-length(Line)-2,0); end; % unread last line
  ...
  %        array(MN(2),MN(1))=Value;
      ...
      elseif ~isempty(LARC), % ARCINFO
        ...
        Path=filename(1:max(findstr(filename,filesep)));
        FileName=[Path FileName];
...
otherwise,
  fprintf('%s formatted file not supported.\n',OS);
  Error=1;
  return;
end;
switch OS,
...
if ~isequal(N1,N2),
  fprintf('Error reading formatted binary file.\nOpening length of block: %i not equal to closing length of block: %i.\n',N1,N2);
  Error=1;
end;
...
function FileInfo=Local_open_incremental(filename),
...
%  FileInfo.Quant(i).NEntries=0;
  if length(FileInfo.Quant(i).Class)>0,
...
if ~isempty(StartTime),
  LastTime=GetLastTime(fid);
  FileInfo.Begin=StartTime;
  FileInfo.End=LastTime;
...
while t<=Time,
  ...
  elseif X(3)==Field,
    ...
    i=1:floor(nr/3);
    if ~isempty(i),
      Ind=sub2ind(size(FileInfo.Quant(Field).Field{dmn}),X(1,i),X(2,i));
      FileInfo.Quant(Field).Field{dmn}(Ind)=X(3,i);
    end;
...
%for dmn=1:length(FileInfo.Domain),
%  Data{dmn}=fliplr(Data{dmn});
%end;
if length(Data)==1, Data=Data{1}; end; % backward compatibility
if isempty(FI2),
  FI2=FileInfo;
...
Line=fgetnel(fid); % * TIME IN COLUMN 1, CROSS SECTIONS IN COLUMN 2 - N

FileInfo.NumCross=0;
Line=fgetnel(fid);
while length(Line)>2 & strcmp(Line(2),'*'),
  FileInfo.NumCross=FileInfo.NumCross+1;
  X=sscanf(strrep(Line((findstr(Line,'M1N1M2N2')+8):end),',',' '),'%f',4);
  if length(X)==4,
...
function Data=Local_read_cross(FileInfo,CrossNr,Index),
Data=[];
