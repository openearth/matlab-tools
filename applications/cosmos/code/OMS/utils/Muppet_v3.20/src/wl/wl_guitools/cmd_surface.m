function outh=cmd_surface(fig,cmd)
%CMD_SURFACE execute a GUI_SURFACE command
%      CMD_SURFACE(GuiHandle,Command)
...
hname=get(fig,'name');
if ~strcmp(hname(1:4),'surf'),
  fprintf(1,'* The first parameter is not a gui SURFACE handle\n');
  return;
end;

% fig is almost surely a GUITOOLS SURFACE handle
handles=get(fig,'userdata');

h=get(handles{1}(3),'userdata');
h_BackUp=handles{1}(2);
if ~ishandle(h),
  Str='The surface object has been deleted.';
...
case 1,
  ...
  case 1, % face color
    ...
    colspec=deblank(row(coltypes,get(handles{2}(2),'value')));
  ...
  case 6, % new data
    ...
    axes(get(h,'parent'));
    [xdata,ydata,zdata]=ginput3d('xyz');
    cdata=get(h,'cdata');
  ...
  case 10, % meshstyle
    ...
    mst=deblank(row(msttypes,get(handles{2}(18),'value')));
    set(h,'meshstyle',mst);
  otherwise,
    fprintf(1,'* Unknown command.\n');
  end;
