...
while ischar(Line)
    if ~isempty(Line)
        if Line(1)=='0'
            At=min(findstr(Line,' AT '));
            Name=Line(2:(At-1));
            Step=eval(Line(min(findstr(Line,'NST='))+4:min(findstr(Line,' ( ='))-1));
            Time=eval(Line(min(findstr(Line,' ( ='))+4:min(findstr(Line,' ELAPSED'))-1));
...
try
    ...
    for expment=1:Structure.MaxNumExperiments,
        ...
        Str=char(fread(fid,[1 76],'uchar'));
        if strcmp(Structure.Format,'l'), Str=Swap4byte(Str); end
        Structure.Experiment(expment).Name=Str(1:X);
    ...
    for expment=1:Structure2.MaxNumExperiments,
        ...
        Str=char(fread(fid,[1 76],'uchar'));
        if strcmp(Structure.Format,'l'), Str=Swap4byte(Str); end
        Structure2.Experiment(expment).Name=Str(1:X);
        X=fread(fid,[1 7],'int32');
        ...
        if wqdebug
            if expment<=Structure.MaxNumExperiments & ~isequal(Structure.Experiment(expment),Structure2.Experiment(expment))
                ...
                if expment>Structure.MaxNumExperiments
                    fprintf(wqdebug,'    ********* new!\n');
                end
    ...
    fseek(fid,(Structure.OffsetChar1-1)*RecSize,-1);
...
function Data=Local_read_sds(S,Expment,Entry,tsteps,stations)
Data=[];
if nargin<3
    return
elseif nargin==3
...
switch S.Entry(idx).DataType2
    ...
    case 10,
        ...
        if ~isempty(tsteps)
            ...
            Size=Size(tsteps);
            if allequalsize
                if isequal(stations,0)
                    Data.Data=zeros(length(tsteps),Size(1));
                else
                    Data.Data=zeros(length(tsteps),length(stations));
                end
            else
            ...
            for i=1:length(tsteps)
                if Offset(i)<0
                    ...
                    if ~isequal(stations,0), X=X(stations); end
                    if allequalsize
                        Data.Data(i,:)=X;
                    else
                        Data.Data{i}=X;
...
DataTypes={'integer','single precision','unknown','integer coded text'};
InfoTypes={'','mesh, ','problem, ','solution, ','input, ','coefficient, ','boundary conditions, '};
...
function plottree(Tree,i,BaseStr,fid)
n=Tree(i);
V=Tree(i+(1:n));
Cont='| ';
for j=1:n
    ...
    if V(j)<=0
        ...
        plottree(Tree,V(j),[BaseStr Cont],fid);
    end
end
