...
% MORF File operations for a MORSYS morf file.
...
%   LINE : IESP K L M N
    ...
    if ~isequal(X(1),i)
      warning(sprintf('Unexpected element number %i in line %i.',X(1),LineNr(Offset)))
    end
...
for ContrI=Contr,
  ...
  if plottime
    Txt=Local_number(ax,YLim,XX+.5*XTick,YY-1.5*YTick,2,sprintf('%i ',ContrI));
