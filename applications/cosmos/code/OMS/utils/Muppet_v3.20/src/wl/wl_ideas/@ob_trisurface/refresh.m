...
if isempty(ErrorMsg), [U,ErrorMsg]=ds_eval(Info.OpenUStream,Frame); end;
if isempty(ErrorMsg), [V,ErrorMsg]=ds_eval(Info.OpenVStream,Frame); end;
if isempty(ErrorMsg),
...
if isempty(ErrorMsg) & isequal(size(X),size(Y),size(Z),size(C),size(U),size(V)),
  [F,V,C]=trisurface(X,Y,Z,C,U~=0,V~=0);
