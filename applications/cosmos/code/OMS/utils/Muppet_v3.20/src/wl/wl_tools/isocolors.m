function cout=isocolors(varargin)
%ISOCOLORS  Isosurface colors.
%   NC = ISOCOLORS(X,Y,Z,C,VERTICES) computes the colors of isosurface
%   vertices VERTICES by using color values C. Arrays X, Y and Z
%   specify the points at which the data C is given. The colors are
%   returned in NC. C must be 3D (index colors) or 4D (rgb colors).
%   
%   NC = ISOCOLORS(C,VERTICES) assumes [X Y Z] = meshgrid(1:N, 1:M, 1:P)
%          where [M,N,P]=SIZE(C).
%   
%   NC = ISOCOLORS(C,P) or C = ISOCOLORS(X,Y,Z,C,P) uses the vertices 
%          from patch P.
%   
%   ISOCOLORS(C,P) or ISOCOLORS(X,Y,Z,C,P) sets the 'FaceVertexCdata'
...
%      isocolors(x,y,z,cat(4, r/20,g/20,b/20),p);
%      set(p, 'FaceColor', 'interp', 'EdgeColor', 'none')
...
%      isonormals(data,p);
%      [r g b] = meshgrid(20:-1:1, 1:20, 1:20);
%      isocolors(cat(4, r/20,g/20,b/20),p);
%      set(p, 'FaceColor', 'interp', 'EdgeColor', 'none')
%      view(150,30); daspect([1 1 1]);
%      camlight; lighting p; 
%
...
[x y z colors verts] = parseargs(nargin,varargin);

% Take this out when other data types are handled
colors = double(colors);

if ndims(colors)==3
  [msg x y z] = xyzrgbcheck(x,y,z,colors);  error(msg)
else
  [msg x y z] = xyzrgbcheck(x,y,z,...
			    colors(:,:,:,1,    :), ...
			    colors(:,:,:,2,    :), ...
			    colors(:,:,:,3:end,:));  error(msg)
end  
...
if isempty(x)
  if ndims(colors)==3
    c=interp3(colors, verts(:,1), verts(:,2), verts(:,3));
  else
    c(:,1)=interp3(colors(:,:,:,1), verts(:,1), verts(:,2), verts(:,3));
    c(:,2)=interp3(colors(:,:,:,2), verts(:,1), verts(:,2), verts(:,3));
    c(:,3)=interp3(colors(:,:,:,3), verts(:,1), verts(:,2), verts(:,3));
  end
else
  if ndims(colors)==3
    c=interp3(x, y, z, colors, verts(:,1), verts(:,2), verts(:,3));
  else
    c(:,1)=interp3(x, y, z, colors(:,:,:,1), verts(:,1), verts(:,2), verts(:,3));
    c(:,2)=interp3(x, y, z, colors(:,:,:,2), verts(:,1), verts(:,2), verts(:,3));
    c(:,3)=interp3(x, y, z, colors(:,:,:,3), verts(:,1), verts(:,2), verts(:,3));
  end
end
end
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [x, y, z, data, verts] = parseargs(nin, vargin)
...
if nin==2      % isocolors(c, verts)
  data = vargin{1};
  verts = vargin{2};
elseif nin==5  % isocolors(x,y,z,c, verts)
  x = vargin{1};
  y = vargin{2};
  z = vargin{3};
  data = vargin{4};
