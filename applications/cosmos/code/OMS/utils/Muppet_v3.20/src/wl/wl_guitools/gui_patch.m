function outh=gui_patch(h)
%GUI_PATCH displays a graphical user interface for a patch object
%      GUI_PATCH(TextHandle) to edit an existing patch object
%      GUI_PATCH to create a new patch object

%      Copyright (c) H.R.A. Jagers  12-17-1996

frames=str2mat('Patch attributes', ...
               'Lines and markers', ...
               'Lighting', ...
               'Triangular mesh', ...
               'Other attributes');
...
handles{5}(3)=gelm_pushbutton(fig,cmdname,2,2,visible,'on','edit vertex normals',[4 2]);

handles{5}(4)=gelm_pushbutton(fig,cmdname,2,4,visible,'on','edit faces',[4 3]);
handles{5}(5)=gelm_pushbutton(fig,cmdname,2,5,visible,'on','edit vertices',[4 4]);
handles{5}(6)=gelm_pushbutton(fig,cmdname,2,6,visible,'on','edit face vertex data',[4 5]);
