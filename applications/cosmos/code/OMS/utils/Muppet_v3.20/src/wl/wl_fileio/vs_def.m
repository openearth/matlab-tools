...
%        adds a new element, cell, group definition or data group
%        as indicated by the Type: 'elm','cell','grp','data' with
%        the specified Name.
%
%        The further arguments depend on type:
%        * elm : DataType, NBytesPerValue, Size, QuantityName, UnitsName, Description
%                The last three arguments are optional; if not specified then
%                they default to the empty string. The data types can be specified
%                either by their name (character, complex, integer, logical, real)
%                or by the typenumber (1--5 for above mentioned types).
%                Valid values of NBytesPerValue are:
...
%                The cell definition is only added when all elements have been defined.
...
%                contain one element 0 or inf to specify a dimension that may vary
%                in size. The group definition is only added when the cell refered
%                to by CellName exists.
%        * data: GrpDefName, AttribName1, Value1, AttribName2, Value2, ...
%                The AttributeNames and Values are optional. When attributes are
%                specified at most 5 attributes with a scalar real value, 5 attributes
%                with scalar integer, and 5 attributes with string value can be
%                specified. Attributes may be changed later using VS_PUT. The data
%                group is only added when the specified group definition exists.
...
%        In none of the cases will the function overwrite existing data/definitions.
%        Redefining groups, cells and elements is only possible after removing the
%        old definition, using the following command:
%
%        DataInfoOut=vs_def(DataInfo,Command,Type,Name)
%        removes an element, cell, group definition or data group as indicated by
%        the Type: 'elm','cell','grp','data' with the specified Name. The Command
...
%        'purge' : same as remove, but also recursively removes all definitions
%                  below if not used by other data groups, or group or cell
%                  definitions. Purging a data group will result in loss of the
%                  data stored in that group.
%        '*remove' and '*purge':
%                  remove higher levels first if definition is in use. When this
%                  requires removing one (or more) data groups, data stored in
%                  those groups is lost.
%
...
% (c) Copyright 1997-2002, H.R.A.Jagers, Delft Hydraulics, The Netherlands
