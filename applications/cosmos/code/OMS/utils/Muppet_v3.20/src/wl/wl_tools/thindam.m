...
while i<nargin,
  ...
  case 'bottom',
    if length(varargin)==i, error(['Error processing option: ',varargin{i}]); end
    Flag3D=1;
...
if XYW
  ...
  if isempty(dam_M),
    ...
    ind_S1=sub2ind(szxcor,dam_S1_M,dam_S1_N);
  end;
  %
...
%
  if isempty(belev{2}), belev{2}=zeros(0,1); end;
  if isempty(belev{3}), belev{3}=zeros(0,1); end;
%
