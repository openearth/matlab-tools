      subroutine mkcurvec(x2,y2,n2,relwdt,x1,y1,u,v,m1,n1,
     &                    dt,nt,nhead,hdthck,arthck,xp,yp,
     &                    xax,yax)

      integer n2,m1,n1,nt,nhead,np
      ...
      real*8 xp(n2*((nt-nhead)*2+5))
      real*8 yp(n2*((nt-nhead)*2+5))
     ...
     &                  NrIn,W,Iref,iprint) 
         call GRMAP (u,np,u2,n2,iref,w) 
         call GRMAP (v,np,v2,n2,iref,w) 
         x2a=x2+.5*u2*dt
         y2a=y2+.5*v2*dt
...
c
            xar(2*ii-1) = xx1+dy*arthck*relwdt(i)
            yar(2*ii-1) = yy1-dx*arthck*relwdt(i)
            xar(2*ii)   = xx1-dy*arthck*relwdt(i)
            yar(2*ii)   = yy1+dx*arthck*relwdt(i)
...
c
         xar(2*ii-1) = xx1+dy*hdthck*relwdt(i)
         yar(2*ii-1) = yy1-dx*hdthck*relwdt(i)
         xar(2*ii)   = xx1-dy*hdthck*relwdt(i)
         yar(2*ii)   = yy1+dx*hdthck*relwdt(i)         
...
*                 rand
               ...
               Endif
            Endif
         Elseif (ABS (x(i)).LT.1.0E-8.AND.ABS (x(i+1)).LT.1.0E-8) Then
...
c
     ...
     .                code(m1,n1),iprint
      Real*8          X1(M1,N1),Y1(M1,N1),X2(N2),Y2(N2),Xs(N2),Ys(N2),
...
c
      Integer         ier
      Real*8          x,x1,x2,x3,x4,y,y1,y2,y3,y4,a21,a22,a31,a32,a41,
     .                a42,det,xt,yt,x3t,y3t,xi,eta,a,b,c,discr,x0,y0,
     .                xa(4),ya(4),w(4)
c
c     read(12,*)x1,y1,f1
      x1  = xa(1)
...
c and required location (xt,yt)
      ...
      a42 = y4-y1
      det = a21*a42-a22*a41
      If (ABS (det).LT.1e-6) Then
C         Write (*,*) 'surface is zero'
         Goto 999
      Endif
...
C         Write (*,*) 'distorted quadrangle'
         Goto 999
      Endif
      If (ABS (x3t-1.0).LT.1.0e-7) Then
         xi = xt
         If (ABS (y3t-1.0).LT.1.0e-7) Then
            eta = yt
         Else
            If (ABS (1.0+(y3t-1.0)*xt).LT.1.0e-6) Then
C               Write (*,*) 'extrapolation over too large a distance'
               Goto 999
            Else
         ...
         Endif
      Else
         If (ABS (y3t-1.0).LT.1.0e-6) Then
            eta = yt
            If (ABS (1.0+(x3t-1.0)*yt).LT.1.e-6) Then
C               Write (*,*) 'extrapolation over too large a distance'
               Goto 999
            Else
