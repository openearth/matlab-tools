...
% fprintf(fid,'%s\n','');
% fprintf(fid,'%s\n','xbeach_bin_dir=/p/1002266-xbeach/TheDayWeLostEgmond/OMS/OMSJobs/egmond');
% fprintf(fid,'%s\n','');
...
% fprintf(fid,'%s\n','mpdboot -d -n $NHOSTS -f $(pwd)/machinefile');
% fprintf(fid,'%s\n','mpirun -np $NSLOTS $xbeach_bin_dir/xbeach >> output_xbeach 2>&1');
% fprintf(fid,'%s\n','mpdallexit');
% fprintf(fid,'%s\n','date -u ''+%Y%m%d %H%M%S'' >> running.txt');
% fprintf(fid,'%s\n','rm *.sp2');
% %fprintf(fid,'%s\n','StageOut');
