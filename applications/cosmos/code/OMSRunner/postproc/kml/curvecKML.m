    ...
    if ischar(varargin{i})
        switch lower(varargin{i})
            ...
            case {'lookat'}
                lookat=varargin{i+1};
            case {'dxcurvec'}
                Plt.DxCurVec=varargin{i+1};
            case {'dtcurvec'}
                Plt.DtCurVec=varargin{i+1};
            case {'ddtcurvec'}
                Plt.DDtCurVec=varargin{i+1};
        end
    end
end
...
Plt.HeadThickness=0.6;
Plt.ArrowThickness=0.2;
...
Ax.YMin=merc(min(min(y)));
Ax.YMax=merc(max(max(y)));
...
%     end
    ...
    [xp,yp,xax,yax,pos]=omsKMLCurVec(x,ym,uu,vv,pos,Ax,Plt);
    yaxn=invmerc(yax);
