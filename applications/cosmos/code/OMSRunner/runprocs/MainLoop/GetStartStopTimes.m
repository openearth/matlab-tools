function hm=GetStartStopTimes(hm)
        ...
        while nested
            ...
            if hm.Models(m).RunSimulation
                ...
                switch lower(hm.Models(m).Type)
                    case{'ww3'}
                        [rstw,rstfil]=CheckForRestartFile(hm,m,hm.Models(m).TWaveStart,wspinup,'ww3');
                    case{'delft3dflowwave'}
                        [rstw,rstfil]=CheckForRestartFile(hm,m,hm.Models(m).TWaveStart,wspinup,'delft3dwave');
                    otherwise
                        rstw=[];
                        rstfil=[];
                end
...
%                 tfstartnowaves=hm.Models(m).TFlowStart;
                ...
                switch lower(hm.Models(m).Type)
                    case{'delft3dflowwave','delft3dflow'}
                        [rstf,rstfil]=CheckForRestartFile(hm,m,hm.Models(m).TFlowStart,fspinup,'delft3dflow');
                    otherwise
                        rstf=[];
                        rstfil=[];
                end
