...
if strcmp(hObj.XDataMode,'manual')
    ...
    arg = codegen.codeargument('Name',objName,...
        ...
        'IsParameter',true);
    if ~isVectorX
        set(arg,'Value',net_xdata,'Comment','matrix of x data');
    else
        set(arg,'Value',get(hObj,'Xdata'),'Comment','vector of x data');
    end
...
if is3D
    ...
    arg = codegen.codeargument('Name',objName,...
        ...
        'IsParameter',true);
    if ~isVectorZ
        set(arg,'Value',net_zdata,'Comment','matrix of z data');
    else
        set(arg,'Value',get(hObj,'Zdata'),'Comment','vector of z data');
    end
    addConstructorArgin(hCode,arg);
end
...
if isVectorOutput
    ...
    set(hFunc,'Comment',...
        ['% Create multiple lines using matrix input to ',constructor_name]);
    ...
    isManual = strcmp(get(hObj,'CodeGenColorMode'),'manual');
    if isManual && ~hasProperty(hCode,'Color')
