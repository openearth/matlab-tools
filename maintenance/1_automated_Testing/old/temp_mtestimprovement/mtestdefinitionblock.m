classdef mtestdefinitionblock < handle
    ...
    methods % set and get methods
        function set.storedobjname(obj,varargin)
            uniquenames = getappdata(0,'mtest_objectlist');
            if isempty(varargin{1})
                %% Remove from appdata
                % set unique names list
                if ismember(uniquenames,obj.storedobjname)
                    uniquenames(ismember(obj.storedobjname,uniquenames))=[];
                    setappdata(0,'mtest_objectlist',uniquenames);
                end
                try %#ok<TRYNC>
                    rmappdata(0,obj.storedobjname);
                end
                obj.storedobjname = [];
            else
                %% Add to appdata
                % set unique names list
                if isempty(uniquenames) || ~ismember(varargin{1},uniquenames)
                    if isempty(uniquenames)
                        uniquenames = varargin(1);
                    else
                        uniquenames = cat(1,uniquenames,varargin(1));
                    end
                    setappdata(0,'mtest_objectlist',uniquenames);
                end
                setappdata(0,varargin{1},obj);
                obj.storedobjname = varargin{1};
            end
        end
        function delete(obj)
            obj.storedobjname = [];
    ...
    methods (Hidden = true)
        function store(obj)
            ...
            if isempty(obj.functionname)
                tmpname = 'mtestdefinitionblock_0';
            else
                tmpname = ['mtestdefinitionblock_' obj.functionname];
            end
            count = 0;
            while ismember(tmpname,uniquenames)
                count = count+1;
                if isempty(obj.functionname)
                    tmpname = ['mtestdefinitionblock_', num2str(count)];
                else
                    tmpname = ['mtestdefinitionblock_' obj.functionname, '_', num2str(count)];
                end
            end
            obj.storedobjname = tmpname;
        end
    end
end