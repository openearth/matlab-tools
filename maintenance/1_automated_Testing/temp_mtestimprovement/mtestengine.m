classdef mtestengine < handle
    ...
    methods
        ...
        function varargout = runAndPublish(obj,varargin)
            ...
            %% Make sure the current dir is added to the search path
            pt = path;
            addpath(cd);
...
%                     end
            ...
            if min(size(obj.tests))>0
                obj.profInfo = mergeprofileinfo(obj.tests(~[obj.tests.ignore]).profinfo);
            end
                    ...
                    for itest = 1:length(testid)
                        ...
                        if testCaseStringToBeFilled
                            ...
                            for icase = 1:length(obj.tests(id).testcases)
                                ...
                                if ~isempty(obj.tests(id).testcases(icase).name)
                                    tcname = ['Case ' num2str(icase) ' (' obj.tests(id).testcases(icase).name ')'];
                                end
