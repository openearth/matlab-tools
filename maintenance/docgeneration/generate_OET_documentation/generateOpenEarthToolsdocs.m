function generateOpenEarthToolsdocs()
%% documentation generator for OpenEarthTools
...
if ~isdir(fullfile(toolboxdoc.targetdir,toolboxdoc.help_location,'html'))
    mkdir(fullfile(toolboxdoc.targetdir,toolboxdoc.help_location,'html'));
end
copyfile(fullfile(tmp,'*'),...
    fullfile(toolboxdoc.targetdir,toolboxdoc.help_location,'html'),'f');
rmdir(tmp,'s');

%% general tutorials
% create dir_generalstr to build a html page with the contents of the general dir
contentstr = ['% Openearth tutorials:',char(10),...
    '%',char(10)];
...
alldirs = unique(cat(1,applicationfolders(:,1),generalfolders(:,1))); %#ok<NODEF>
alldirs(:,2:3)=cell(size(alldirs,1),2);

contentstr = cat(2,contentstr,...
    '%  General tutorials:',char(10),...
    '%',char(10));

% General tutorials
for igenfile = 1:size(generaltutorials,1) %#ok<USENS>
    if igenfile==1
        toolboxdoc.contentitems(4).children(3).children(1).children = tbcontentitem(...
            'name',strrep(generaltutorials{igenfile,1},'&',' and '),...
    ...
    contentstr = cat(2,contentstr,'%     ',tutorialhref(generaltutorials{igenfile,1},generaltutorials{igenfile,2}),char(10));
end
dir_generalstr = cat(2,dir_generalstr,...
    ['%  </ul>',char(10),...
    '% </html>',char(10),...
    '%',char(10)]);
contentstr = cat(2,contentstr,'%',char(10));
...
for idirs = 1:length(dirnames)
    ...
    toolboxdoc.contentitems(4).children(3).children(1).children(id(idirs)) = tbcontentitem(...
        'name',strrep(strrep(dirnames{idirs},'_',' '),'&',' and '),...
    ...
    dir_generalstr = cat(2,dir_generalstr,...
        ...
        '%   <ul>',char(10)]);
    contentstr = cat(2,contentstr,...
        '%     ',strrep(strrep(dirnames{idirs},'_',' '),'&',' and '),':',char(10));
    ...
    for ifiles = 1:size(fls,1)
        ...
        alldirs{dir_id,3} = cat(2,alldirs{dir_id,3},...
            ['%   <li><a href="',fls{ifiles,3} ,'">',fls{ifiles,2} ,'</a></li>',char(10)]);
        contentstr = cat(2,contentstr,'%       ',tutorialhref(fls{ifiles,2},fls{ifiles,3}),char(10));
    end
    dir_generalstr = cat(2,dir_generalstr,...
        ['%   </ul>',char(10),...
        '% </div>',char(10),...
...
outputhtmldir = fullfile(toolboxdoc.targetdir,toolboxdoc.help_location,'html');
