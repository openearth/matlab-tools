...
elseif iscell(mmat)
    ...
    for ii=1:length(names)
        [dum namesexclpath{ii}] = fileparts(names{ii});
    end
else
...
if nargin == 2
	for i=1:size(hrefs,1)
		...
		m{i} = n;
		for j=1:length(n)
			fprintf(fid,['  ' namesexclpath{i} ' -> ' namesexclpath{n(j)} ';\n']);
		end
	end
	%m = unique([m{:}]);
	fprintf(fid,'\n');
	for i=1:size(hrefs,1)
		fprintf(fid,'%s\n', ['  ' namesexclpath{i} ' [URL="' names{i} options.extension '"];']);
	end
else
	...
	for j=1:length(m)
		if n(j) ~= i
			fprintf(fid,['  ' namesexclpath{m(j)} ' -> ' namesexclpath{i} ';\n']);
		end
	end
	n = unique([n(:)' m(:)']);
	fprintf(fid,'\n');
