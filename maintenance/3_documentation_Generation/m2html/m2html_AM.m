...
%    o excludedFiles - List of files to exclude [ {'exclude.m' '*test.m'} ]
...
%  $Revision$Date: 2005/05/01 16:15:30 $
...
for i=1:2:length(paramlist)
    ...
    elseif length(ind) > 1
        ind = strcmpi(pname,optionsnames);
        if sum(ind)~=1
    ...
    switch(optionsnames{ind})
        ...
        case 'excludedfiles'
            if iscellstr(pvalue)
                options.excludedFiles = pvalue;
            elseif ischar(pvalue)
                options.excludedFiles = cellstr(pvalue);
            else
                error(strrep(msgInvalidPair,'%s',pname));
            end
        case 'ignoreddir'
            if iscellstr(pvalue)
...
if ~options.load
    ...
    for ii=length(mfiles):-1:1
        [mfiledir] = fileparts(mfiles{ii});
        if any(strcmp(mfiles{ii},options.excludedFiles))
            mfiles(ii)=[];
        end
    end
    for iexcl = 1:length(options.excludedFiles)
        if any(strfind(options.excludedFiles{iexcl},'*'))
            exclstr = options.excludedFiles{iexcl}(strfind(options.excludedFiles{iexcl},'*')+1:end);
            ids = ~cellfun(@isempty,strfind(mfiles,exclstr));
            mfiles(ids)=[];
        end
    end
end
