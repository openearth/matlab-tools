...
%    which would typically be called "undock" or have an undocking icon.
...
%
% Change log:
%    2010-05-23: Enabled docking to a figure button (minimized)
...
function reinstrumentDockIcon(hFig1,hSource)
  ...
  jDockIcon = jFrame.fFigureClient.getMenuBar.getParent.getComponent(0).getComponent(1);
...
function animate(hFig1,hSource,dockFlag)
  ...
  for step = 1 : numSteps
      ...
      if dockFlag
          ...
          newPos = startPos + deltaPos*(numSteps+1-step)/numSteps;
      end
      set(hFig,'pos',newPos);
  end
  delete(hFig);
