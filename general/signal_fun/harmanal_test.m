...
%% with noise
   ...
   D.noise = D.A_noise*rand(size(T.t));
   T.h     = T.h + D.noise;
   ...
   FIT = harmanal(T.t,T.h,'omega',D.w,'screenoutput',0,'timeseries',D.noise);
   
   OK = 1;
   for i=1:length(D.A)
      if abs(D.A(i)-FIT.hamplitudes(i)) > OPT.eps & ...
            (FIT.c-1) > OPT.eps % noise is fitted to its exact amplitude
