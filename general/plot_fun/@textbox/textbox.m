...
%   Example:
%     tb = textbox('String','Demonstartion textbox')
%     tb.inspect
%
...
classdef textbox < oop.inspectable
    % Code template generated by calling metaprop.helper.Template(textbox)
    properties (Constant,Hidden)
        metaprops = metaprop.helper.Construct(?textbox,{
    ...
    properties (Hidden)
        ...
        StringSet  = 'String'% stores the string (before wrapping)
    end
    ...
    methods
        ...
        function set.Font               (self,value); self.metaprops.Font               .Check(value);
            self.ht.FontName = char(value.getName);
            self.ht.FontSize = double(value.getSize);
            if value.isItalic
                self.FontAngle = 'italic';
            else
                self.FontAngle = 'normal';
            end
            if value.isBold
    ...
    methods
        ...
        function update(self)
            ...
            if self.FitBoxToText
                extent(1) = (self.Size(1) * self.ht.Extent(3)) + self.MarginInParentUnits(1)*2;
                extent(2) = (self.Size(2) * self.ht.Extent(4)) + self.MarginInParentUnits(2)*2;
                self.ha.Position = self.calculatePosition(extent);
            end
