...
C
      ...
      integer m1, n1, n2, nt, size, nmax, np2, np3, nhead, iopt
...
C     Create matrix for the return argument.
      ...
      relwdt_pr = mxGetPr(prhs(12))
      iopt_pr   = mxGetPr(prhs(13))
...
C     Load the output into a MATLAB array.
     ...
     &                    xax,yax,length,iopt)
      ...
      double precision u(m1,n1)
      double precision v(m1,n1)
...
c      open(800,file='out.txt')
      ...
      if (iopt==1) then
         do i=1,m1
            do j=1,n1
               u(i,j) = u(i,j)/(cos(pi*y1(i,j)/180.0)*geofac)
               v(i,j) = v(i,j)/geofac
            enddo
         enddo
      endif
