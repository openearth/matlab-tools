...
% nhead          : Number of points in arrow head (default 2).
...
if ~isempty(pos)
    ...
    if iopt==1
        % Geographic
        polarea=polyarea(xp*geofac,invmerc(yp)*geofac);
    else
...
[xp,yp,xax,yax,len]=mxcurvec(x2,y2,x1,y1,u,v,tlength,nt,hdthck,arthck,nhead,relwdt,iopt);
...
nn=(nt-1)*(relspeed*timestep/tlength);
nfrac=nn-floor(nn);
nn1=floor(nn)+1;
nn2=floor(nn)+2;
nn2=min(nn2,nt);
