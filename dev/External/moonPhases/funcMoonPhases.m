function tbl = funcMoonPhases(yearStart, yearEnd)
...
% Modifncation of MoonPhases by SDO to produce a longer time series
% By the way : I read simewhere it is only valid between 1949 until 2049
% 
%--------------------------------------------------------------------------
% clc
% clear
...
% Title
fprintf('\n');
fprintf('        PHASES: Phases of the Moon\n');
fprintf('\n');
...
% fprintf('\n');
Year = yearStart; 
lastMonth = ceil((yearEnd-yearStart+1)*(12.3685))+1; 
...
N = M*4; 
Time = NaT(N,1); 
phase = repmat([0:3]', M,1); 
descr = repmat(["New Moon"; "First Quarter"; "Full Moon"; "Last Quarter"], M,1);
tbl = table(Time, phase, descr);
itime = 0; 

% Check 13 months
for Lunation=0:lastMonth
...
%       fprintf('%4d/%2.2d/%2.2d  %2.2d:%2.2d', year, mon, day, hr, min);      
      tbl.Time(itime) = datetime(year, mon, day, hr, min, 0); 
%       tbl.phase(itime) = iPhase; 
%       % Eclipse check
