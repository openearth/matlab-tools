function stat = vtkWriteUnstructered(filename,vtk_title,grid_X,grid_TET,data_struct,flipped)
...
%               transposed; false means not transposed (optional).
...
if isempty(intersect(Tet_Order,[4 5 6 8 10 20]))
    stat = fclose(fid);
    error('Wrong number of nodes in connectivity!');
end
...
if ( Tet_Order == 4 )
    TET_LABEL = 9; % quadrilateral. note that it could also be a tetraeder
elseif ( Tet_Order == 5 )        
    TET_LABEL = 14; % pyramid
elseif ( Tet_Order == 6 )    
    TET_LABEL = 13; % prisms
elseif ( Tet_Order == 8 )    
    TET_LABEL = 12; % hexahedra
elseif ( Tet_Order == 10 )
    TET_LABEL = 24; % quadratic tetrahedra
elseif ( Tet_Order == 20 )
    TET_LABEL = 25; % quadratic hexahedra
    
else
    error('Invalid order!');
end
% note that for the moment all cells must have the same type
