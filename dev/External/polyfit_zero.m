function [p,S,mu] = polyfit_zero(x,y,n,bZero)
%POLYFIT Fit polynomial to data.
%   P = POLYFIT(X,Y,N) finds the coefficients of a polynomial P(X) of
%   degree N that fits the data Y best in a least-squares sense. P is a
%   row vector of length N+1 containing the polynomial coefficients in
%   descending powers, P(1)*X^N + P(2)*X^(N-1) +...+ P(N)*X + P(N+1).
%    P = POLYFIT(X,Y,N,bZero) does the same, except that the constant of the polynome is forced to be zero (so that the curve goes through (0,0)
...
%   $Revision$  $Date$
%   Adapted by ABR (11-1-10), firtthoruigh zero
...
if bZero
    V(:,n+1) = zeros(length(x),1,class(x));
end;
