...
if strcmp(m.type,'seraphin')
    ...
    if m.first||nargin<2 % if this is first one, or if we just want to read next timestep
        ...
        % Time info
        if m.isDouble
            m.AT = fread(m.fid,1,'float64');     % read time (in seconds)
        else
        ...
        for i = 1:m.NBV
            ...
            try
                if m.isDouble
                    m.RESULT(:,i) = fread(m.fid,m.NPOIN,'float64'); % read data
                else
            ...
            tagEnd = fread(m.fid,1,'int32');  %get 2D results end tag
            if tag~=tagEnd
                error('Data corrupt');
            end
        end
    ...
    if nargin == 2
        ...
        if TIMESTEP~=m.timestep % don't need to read again if the current step
            ...
            for i = 1:m.NBV
                ...
                endTag = fread(m.fid,1,'int32');  %get 2D results end tag
                if tag~=endTag
