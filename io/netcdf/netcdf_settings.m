...
if OPT.append
    addpath(ncroot,'-end'); % do not add 'switch' dirs
else
    addpath(ncroot,'-begin'); % do not add 'switch' dirs
end
addpathfast(fullfile(ncroot,'snctools'),'append',OPT.append);
addpathfast(fullfile(ncroot,'nctools'),'append',OPT.append);
addpathfast(fullfile(ncroot,'mexnc'),'append',OPT.append); % snctools needs some mexnc stuff
addpathfast(fullfile(ncroot,'kickstarter'),'append',OPT.append);
...
elseif strcmpi(version('-release'),'2006b') | ... % OK, despite "Exception in thread "Thread-12" java.lang.ArrayIndexOutOfBoundsException: -1"
    ...
    addpathfast(fullfile(ncroot,'mexnc_legacy'),'append',OPT.append);
