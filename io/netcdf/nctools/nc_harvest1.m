function varargout = nc_harvest1(varargin)
%NC_HARVEST1  extract CF + THREDDS meta-data from 1 netCDF/OPeNDAP url
%
%  struct = nc_harvest1(ncfile)
%
...
%
% Use NC_HARVEST to harvest a list of ncfiles (netCDF file/OPeNDAP url).
%
%See also: NC_INFO, nc_dump, NC_ACTUAL_RANGE, NC_HARVEST, OPENDAP_CATALOG
      ...
      for iatt=1:natt
          ...
          if strcmpi(Name,'long_name')
              ...
              if ~any(strfind(ATT.long_name,[' ',Value]))   % remove redudant long_name (can occur with statistics)
                 ATT.long_name = [ATT.long_name Value OPT.separator];  % needs to be char, ; separatred
              end
