...
%
%  where metadatadatabase is the catalog.nc generated by
...
%% get meta data
   ...
   if isstruct(metadatadatabase)
   
      D = metadatadatabase;
      
   elseif strcmpi(fileext(metadatadatabase),'.nc')
   
      D = nc2struct(metadatadatabase);

   elseif strcmpi(fileext(metadatadatabase),'.xls')
...
%% HEADER
   ...
   if isempty(OPT.lat);OPT.lat = mean(D.geospatialCoverage_eastwest  (:,1),1);end
  ...
  'description',OPT.description,...
         'open',0,...
          ...
          'lat',OPT.lat,...
            'z',OPT.z,...
       'timeIn',min(D.datenum_start),...
      'timeOut',max(D.datenum_end ));
...
%  TO DO: pre-allocate output (printing images takes longer, so speed-up will be marginal)?
   ...
   for ii=1:size(D.geospatialCoverage_eastwest,1)
    ...
    %% make table
       ...
       tableContents = [tableContents sprintf([...
        ...
        datestr(D.datenum_start(ii),31),...
        datestr(D.datenum_end  (ii),31),...
   ...
   %  http://code.google.com/apis/kml/documentation/kmlreference.html#timespan
      ...
      if  ~isnan(D.datenum_start(ii))
          OPT.timeSpan = sprintf([...
              '<TimeSpan>'...
              '<begin>%s</begin>'... % OPT.timeIn
              '<end>%s</end>'...     % OPT.timeOut
              '</TimeSpan>'],...
              datestr(D.datenum_start(ii),'yyyy-mm-ddTHH:MM:SS'),...
              datestr(D.datenum_end  (ii),'yyyy-mm-ddTHH:MM:SS'));
      else
          OPT.timeSpan ='';
      end
