function OPT = nc_cf_opendap2catalog2kml(ATT,varargin)
%NC_CF_OPENDAP2CATALOG2KML save harvested THREDDS catalog to catalog.xls
%
% NC_CF_OPENDAP2CATALOG2KML(xlsname,ATT,<keyword,value>)
%
% where ATT = NC_CF_OPENDAP2CATALOG() generates an overview
% for Google Earth of stationtimeseris locations, incl. previews
% of the time series.
%
%See also: nc_cf_opendap2catalog, nc_cf_opendap2catalog2nc, nc_cf_opendap2catalog2xls
%          googleplot, vaklodingen_overview, jarkus_grids_overview
...
%% HEADER
  ...
  'description',OPT.description,...
         'open',0,...
          ...
          'lat',OPT.lat,...
            'z',OPT.z,...
       'timeIn',cellfun(@(x) min(x),D.datenum),...
      'timeOut',cellfun(@(x) max(x),D.datenum));
...
%% marker BallonStyle
   ...
   output = [output ...
    ...
    '  <img src="',filename(oetlogo),'" align="right" width="100"/>]]></text>'...
    '  </BalloonStyle></Style>'...
...
%  TO DO: pre-allocate output (printing images takes longer, so speed-up will be marginal)?
   ...
   for ii=1:length(D.geospatialCoverage_eastwest)
    ...
    %% make table
       ...
       tableContents = [tableContents sprintf([...
        ...
        datestr(D.datenum{ii}(1),31),...
        datestr(D.datenum{ii}(2),31),...
   ...
   %  http://code.google.com/apis/kml/documentation/kmlreference.html#timespan
      ...
      if  ~isnan(D.datenum{ii})
          OPT.timeSpan = sprintf([...
              '<TimeSpan>'...
              '<begin>%s</begin>'... % OPT.timeIn
              '<end>%s</end>'...     % OPT.timeOut
              '</TimeSpan>'],...
              datestr(D.datenum{ii}(1),'yyyy-mm-ddTHH:MM:SS'),...
              datestr(D.datenum{ii}(2),'yyyy-mm-ddTHH:MM:SS'));
      else
          OPT.timeSpan ='';
      end
