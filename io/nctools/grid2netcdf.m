...
%% Define variables
    ...
    s.Attribute = struct('Name', {'long_name', 'comment'}, 'Value', {'identifier', 'sum of area code (x1000000) and alongshore coordinate'});
    nc_addvar(filename, s);
    
    s.Name      = 'areacode';
    s.Nctype    = nc_int;
    s.Dimension = {'alongshore'};
    s.Attribute = struct('Name', {'long_name', 'comment'}, 'Value', {'area code', 'codes for the 15 coastal areas as defined by rijkswaterstaat'});
    nc_addvar(filename, s);

    s.Name      = 'areaname';
    s.Nctype    = nc_char;
    s.Dimension = {'alongshore', 'stringsize'};
    s.Attribute = struct('Name', {'long_name', 'comment'}, 'Value', {'area name', 'names for the 15 coastal areas as defined by rijkswaterstaat'});
    nc_addvar(filename, s);

    s.Name      = 'alongshore';
    s.Nctype    = nc_double;
    s.Dimension = {'alongshore'};
    s.Attribute = struct('Name', {'long_name', 'units', 'comment'}, 'Value', {'alongshore coordinate', 'm', 'alongshore coordinate within the 15 coastal areas as defined by rijkswaterstaat'});
    nc_addvar(filename, s);
    ...
    s.Attribute = struct('Name', {'standard_name', 'units', 'axis'}, 'Value', {'projection_x_coordinate', 'm', 'X'});
    nc_addvar(filename, s);
    
    s.Name      = 'y';
    s.Nctype    = nc_double;
    s.Dimension = {'alongshore', 'cross_shore'};
    s.Attribute = struct('Name', {'standard_name', 'units', 'axis'}, 'Value', {'projection_y_coordinate', 'm', 'Y'});
    nc_addvar(filename, s);
    ...
    s.Nctype    = nc_double;
    s.Dimension = {'alongshore'};
    s.Attribute = struct('Name', {'long_name', 'units', 'comment'}, 'Value', {'mean high water', 'm', 'mean high water relative to nap'});
    nc_addvar(filename, s);
    
    s.Name      = 'mean_low_water';
    s.Nctype    = nc_double;
    s.Dimension = {'alongshore'};
    s.Attribute = struct('Name', {'long_name', 'units', 'comment'}, 'Value', {'mean low water', 'm', 'mean low water relative to nap'});
    nc_addvar(filename, s);
...
%% altitude is big therefor done seperate
