...
!                                                                               
!-------------------------------------------------------------------------------
!  $Id: bedcomposition_module.f90 9563 2013-10-31 17:00:37Z hoonhout $
!  $HeadURL: https://svn.oss.deltares.nl/repos/openearthtools/trunk/programs/SandMudBedModule/02_Matlab/fortran/src/bedcomposition_module.f90 $
!!--module description----------------------------------------------------------
!
...
subroutine bedcomposition_module_info(messages)
    ...
    !
    call addmessage(messages,'$Id: bedcomposition_module.f90 9563 2013-10-31 17:00:37Z hoonhout $')
    call addmessage(messages,'$URL: https://svn.oss.deltares.nl/repos/openearthtools/trunk/programs/SandMudBedModule/02_Matlab/fortran/src/bedcomposition_module.f90 $')
...
!
    ...
    !
    istat = 0
    if (allocwork(this,work) .ne. 0) return
    ! 
    ! update fluff layer            
    !
    ...
    case(2)
       do nm = this%settings%nmlb,this%settings%nmub
          ...
          !  
          dtmorfac = dt*morfac
          if (this%settings%idiffusion>0) call lyrdiffusion(this, nm, dtmorfac, rhosol, messages)
          !
...
!
!==============================================================================
subroutine lyrdiffusion(this, nm, dt, rhosol, messages)
!!--description-----------------------------------------------------------------
!
!    Function:
...
!    write(message,'(a)') 'diffusion coefficients'
!    call addmessage(messages,message)
!    do l=1,ndiff
!        write(message,'(e20.4)') kdiff(l,nm)
!        call addmessage(messages,message)
!    enddo 
    !     
    do l = 1, this%settings%nfrac
    ...
    do k = 2, nlyr
        ...
        if (k==1) then
            thl = thtrlyr(nm)
        elseif (k<keuler)  then
            thl = this%settings%thlalyr
        elseif (k>=keuler) then
            thl = this%settings%theulyr
        endif
        !
        if ( thlyr(k,nm)>0.05_fp*thl )  kx=k
        !
        ...
        do l = 1, this%settings%nfrac
            ...
            ! based on difference in mass or volume fraction
            !
            Fdiff = rhosol(l)*kd*(svfrac(km1, nm) + svfrac(k, nm))/(thlyr(km1, nm)+thlyr(k, nm)) * (pk - pkm1(l))
            !
