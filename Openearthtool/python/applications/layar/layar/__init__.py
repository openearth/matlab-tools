...
def main(global_config, **settings):
    ...
    # use the mailer to mail screenshots....
    ...
    # ** "/robots.txt" from "layar/static/robots.txt" and
    # ** "/images/logo.png" from "layar/static/images/logo.png".
    #
    config.add_static_route("layar", "static", cache_max_age=3600)


    # Configure renderers and event subscribers
    config.add_renderer(".html", "pyramid.mako_templating.renderer_factory")
    config.add_subscriber("layar.subscribers.create_url_generator",
                          "pyramid.events.ContextFound")
    config.add_subscriber("layar.subscribers.add_renderer_globals",
                          "pyramid.events.BeforeRender")

    # Set up view handlers
    config.include("layar.handlers")

    # Set up other routes and views
    # ** If you have non-handler views, create create a ``layar.views``
    # ** module for them and uncomment the next line.
    #
    #config.scan("layar.views")
    ...
    # ** "layar/static" directory. The second serves URLs under 
    # ** "/deform" from the third-party ``deform`` distribution.
    #
    #config.add_static_view("static", "layar:static")
