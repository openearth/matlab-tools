...
def main(global_config, **settings):
    ...
    config.add_subscriber("coastal.subscribers.create_url_generator",
        "pyramid.events.ContextFound")
    config.add_subscriber("coastal.subscribers.add_renderer_globals",
                          "pyramid.events.BeforeRender")

    # Set up view handlers
    config.include("coastal.handlers")

    # Set up other routes and views
    # ** If you have non-handler views, create create a ``coastal.views``
    # ** module for them and uncomment the next line.
    #
    #config.scan("coastal.views")

    # Mount a static view overlay onto "/". This will serve, e.g.:
    # ** "/robots.txt" from "coastal/static/robots.txt" and
    # ** "/images/logo.png" from "coastal/static/images/logo.png".
    #
    config.add_static_route("coastal", "static", cache_max_age=3600)
    ...
    # ** "coastal/static" directory. The second serves URLs under 
    # ** "/deform" from the third-party ``deform`` distribution.
    #
    #config.add_static_view("static", "coastal:static")
