...
def Plot_Crossection(ip,op):
	...
	lineouter=CreateAreaSeries(op.alignment[ip.cross_ind]['dims']['xyouter'])
	...
	linearmour=CreateAreaSeries(op.alignment[ip.cross_ind]['dims']['xyarmour'])
	...
	linefilter=CreateAreaSeries(op.alignment[ip.cross_ind]['dims']['xyfilter'])
	...
	xySWL=np.vstack(([op.alignment[ip.cross_ind]['dims']['xouter'][0],op.alignment[ip.cross_ind]['dims']['xouter'][-1]],[ip.SWL,ip.SWL])).T
	...
	if ip.is2D:
		chart.LeftAxis.Minimum = -ip.profile['mean_z'][ip.cross_ind]
	else:
		chart.LeftAxis.Minimum = -ip.z
	...
	if ip.is2D:
		chart.Title="Breakwater section %d at %.1f m from coastline" %(ip.cross_ind+1,ip.profile['mean_dist'][ip.cross_ind])
	else:
...
def Create_DimensionsText(ip,op):
	...
	if ip.autocrest:
		crestheight_text="%0.2f m (Auto)" % float(ip.crestheight)
	else:
		crestheight_text="%0.2f m" % float(ip.crestheight)
	
	if ip.is2D:
		localdepth = ip.profile['mean_z'][ip.cross_ind]
		breakwatlength = np.max(ip.profile['dist']).Value
	else:
		localdepth = ip.z
		breakwatlength = 1.0
...
def Create_VolumesText(ip,op):
	...
	"Armour:\t\t%.0f m3/m\n" % op.alignment[ip.cross_ind]['area']['armour'] +\
	"Filter:\t\t%.0f m3/m\n" % op.alignment[ip.cross_ind]['area']['filter'] +\
	"Core:\t\t%.0f m3/m\n" % op.alignment[ip.cross_ind]['area']['core'] +\
	"Total:\t\t%.0f m3/m\n" % op.alignment[ip.cross_ind]['area']['outer'] 
