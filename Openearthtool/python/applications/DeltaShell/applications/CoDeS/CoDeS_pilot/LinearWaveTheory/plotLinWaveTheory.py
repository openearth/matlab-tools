...
def plotProfile(profile,crestheight,layer):
    ...
    chart.LeftAxis.Maximum = max([max(depth_or) * 1.1,2])
...
## =============================================================
def addBreakwaterDashes_ind(plot, profile, ip):
    """
    ...
    INPUT:  - plot with chart (where the plot must be placed)
           ...
           (new)* profile['mean_dist']  mean distance between two points [m]
           (new)* profile['mean_z']     mean waterDepth between two points [m minus reference]
             - input class with field 'cross_ind'
    ...
    dashBWLine1 = CreateLineSeries([[meanDistance[ip.cross_ind], meanDepth[ip.cross_ind]],[meanDistance[ip.cross_ind], max([max(depth)*1.1,2])]])
