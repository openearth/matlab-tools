...
from System.Windows.Forms import DateTimePickerFormat, MessageBox, HorizontalAlignment, TabPage, MessageBox
from System.Windows.Forms import DataGridView, FolderBrowserDialog, DialogResult, Padding
...
class BuildInput:
	def __init__(self):
		...
		self.WorkDir = r"..\plugins\DeltaShell.Plugins.Toolbox\Scripts\Scripts\TidalData\WORKING_DIR" # BJ: Adjusted to new map structure
		self.DataSource = ""
...
def PickDataSource(group_IN, SelectedItem, WPScontrols, TScontrols):
...
#get handle to INPUT
def GetProjectName(Change):
	INPUT.ProjectName = Change

def GetWorkDir(Change):
	INPUT.WorkDir = Change

def GetFilename(Change):
	INPUT.Filename = Change

def GetLatitude(Change):
	INPUT.Latitude = Change
	
def GetLongitude(Change):
	INPUT.Longitude = Change

def GetStartTime(Change):
...
def BuildInputBox(INPUT):
	...
	group_IN.Padding = Padding(vert_spacing/1.5)
	...
	L1b.TextChanged += lambda s,e : GetProjectName(L1b.Text)
	#endregion
	...
	L2b.TextChanged += lambda s,e : GetWorkDir(L2b.Text)
	...
	L4a.Location = s.Point(sp_loc,6.5*vert_spacing)
	L4a.Width = label_width

	L4b = TextBox()
	L4b.Text = INPUT.Filename #default name
	L4b.Location = s.Point(label_width+spacer_width,6.5*vert_spacing)
	L4b.Width = label_width
	L4b.TextChanged += lambda s,e : GetFilename(L4b.Text)
	#get choice at end!
	#endregion
	...
	L5b.TextChanged += lambda s,e : GetLatitude(L5b.Text)
	...
	L6b.TextChanged += lambda s,e : GetLongitude(L6b.Text)
	...
	L7b.TextChanged += lambda s,e : GetStartTime(L7b.Text)
	#get choice at end!
	#endregion
	...
	L8b.TextChanged += lambda s,e : GetEndTime(L8b.Text)
	#get choice at end!
	#endregion

	#region build "trigger" button
	B1 = Button()
	B1.Location = s.Point((label_width+spacer_width)/2,14*vert_spacing)
	...
	L3b.SelectedIndexChanged += lambda s,e : PickDataSource(group_IN,L3b.SelectedItem,WPScontrols,TScontrols)
...
def OutputTrigger(sender, e):
	...
	INPUT.OutputNum += 1
	OUTPUT = INPUT.Clone() #clone it
	
	if OUTPUT.DataSource == "WPS":
		plot,B2,STATS,B3 = run_WPS(OUTPUT)
	elif OUTPUT.DataSource == "Timeseries":
		plot,B2,STATS,B3 = run_TS(OUTPUT)
...
def PrintPlot(plot, location):
	...
	MessageBox.Show("Tide Plot Exported!","Confirmation") #alert the user
...
def ExportTideData(location,stats,cons):
	...
	MessageBox.Show("Tide Statistics Exported!","Confirmation") #alert the user
...
def run_WPS(OUTPUT):
	...
	cons = td.extract_TOPEX(float(OUTPUT.Latitude),float(OUTPUT.Longitude))
	...
	plot,B2,STATS,B3 = run_plot(OUTPUT.ProjectName,data,cons,stats)
...
def run_TS(OUTPUT):
	...
	data = td.load_ascii(OUTPUT.WorkDir + os.sep + OUTPUT.Filename,hdr,delimiterChar,dateTimeFormat)
...
def run_plot(location,data,cons,stats):
	...
	#add stats
	STATS,B3 = build_output(location,data,cons,stats)
	return plot,B2,STATS,B3

#"output tidal stats" function
def build_output(location,data,cons,stats):
