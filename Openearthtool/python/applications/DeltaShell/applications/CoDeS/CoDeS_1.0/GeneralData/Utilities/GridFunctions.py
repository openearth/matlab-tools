...
import Libraries.FlowFlexibleMeshFunctions as _FMFunctions
...
import Scripts.BathymetryData.Interpolatie as _Interpolatie
from Scripts.GeneralData.Utilities.Tests import testBathySlope as _SlopeFunctions

import System.Windows.Forms as _swf


def CreateGridFromPath(asciiPath):
...
def CreateGridLayerFromPath(asciiPath):		
	rasterLayer = _RegularGridRasterLayer()
	rasterLayer.Name = os.path.split(asciiPath)[1]
	rasterLayer.DataSource.Path = asciiPath
	return rasterLayer

def SetGradientTheme(layer, attributeName, numClasses, minVal, maxVal):
	"""Sets the theme (coloring) of the layer"""
	
	colorBlend = ReverseColorBlendColors(ColorBlend.Rainbow5)
	
	size = 10
	layer.Theme = ThemeFactory.CreateGradientTheme(attributeName, None, colorBlend, minVal, maxVal, size,size,False,True,numClasses)
	for themeItem in layer.Theme.ThemeItems:
		themeItem.Style.Outline.Width = 1
		themeItem.Style.Outline = themeItem.Style.Outline

def ReverseColorBlendColors(colorBlend):
	colorsReverse = [c for c in reversed(colorBlend.Colors)]
	return ColorBlend(_Array[_Color](colorsReverse), colorBlend.Positions)
	
def SetGradientThemeWavePenetration(layer, attributeName, numClasses, minVal, maxVal):
	"""Sets the theme (coloring) of the layer"""
	
	colorBlend = ColorBlend.GreenToRed
...
def GetProfileFromSlope(LineGeometry, CS_code_ori,NumSteps,CoastlineGeometry,SlopeRatio):
	"""
	Get profile along line geometry based on slope definition
	
	Input arguments
	
	LineGeometry: geometry of profile line (NetTopologySuite.Extensions.Features.Feature.Geometry)	
	CS_code_ori: EPSG code of coordinate system of profile line
	NumSteps: number of points on the line where the Z-value will be calculated
	CoastlineGeometry: geometry of coastline (Z = 0) (NetTopologySuite.Extensions.Features.Feature.Geometry)	
	SlopeRatio: describes slope ratio (4 means slope of 1:4)
	...
	#_swf.MessageBox.Show("Stepping into grid functions")
	LineLength = LineGeometry.Length
	#NumSteps = int(2*LineLength/cellsize)
	
	#	Calculate slope value
	
	slopeValue = float(1)/float(SlopeRatio)
	...
	for PointIndex in range(0,CoastlineGeometry.NumPoints-1):
		coastPoint = CoastlineGeometry.GetPointN(PointIndex)
		CoastlineListX.append(coastPoint.X)
		CoastlineListY.append(coastPoint.Y)
	
	ArrayX = np.array(CoastlineListX)
	ArrayY = np.array(CoastlineListY)
	...
	#_swf.MessageBox.Show("Vectors have been initialised")
	...
	while Distance <= LineLength:
		...
		X = StartPoints[SegmentID].X + diffX * (DistanceInSegment/CurrentSegmentLength)
		Y = StartPoints[SegmentID].Y + diffY * (DistanceInSegment/CurrentSegmentLength)
		#_swf.MessageBox.Show("X: " + str(X))	
		#_swf.MessageBox.Show("Y: " + str(Y))			
		
		#	 Query grid value on point location
		
		#GridValue = ReadValueFromGrid(grid,X,Y)
		#_swf.MessageBox.Show("Looping over number of points" + str(len(ArrayX)))
		
		#_swf.MessageBox.Show("quering distance to coast")
		[distanceToCoast,sideCoast] = _SlopeFunctions.GetDistanceAndSideToCoastLine(ArrayX,ArrayY,np.array([X]),np.array([Y]))
		#_swf.MessageBox.Show("quering done")
		GridValue = _SlopeFunctions.CalculateDepthWithSlope(distanceToCoast,sideCoast,slopeValue)[0]
...
def Convert_XYZ_to_grid(XYZPath,GridPath):	
	...
	ZI = _Interpolatie.invDist(Xvalues,Yvalues,Zvalues,int(minX),int(minY),diffX+1,diffY+1,1,0)
...
def GetAscGridFromGebco(CurrentExtent,AscResultPath):
	...
	GebcoExtract = _FMFunctions.GetGebcoBathymetryData(MinX,MinY,MaxX,MaxY,3857)
	...
	for column in range(0,len(XValues)):
		...
		for row in range(0,len(YValues)):
			...
			ElevationValue = _FMFunctions.GetGebcoBathymetryValueFor(Locatie,3857,GebcoExtract)
