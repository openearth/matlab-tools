...
Puntenlaag = GridFunctions.GetMapLayer("Bathymetry","Depthpoints")
...
offshoreDepth = GridFunctions.ReadGridValue("Bathymetry","elevation_luanda_5m",OffshoreX, OffshoreY)    #Depth [m]

print offshoreDepth

#offshZ = np.array(-500.0)
nearshoreDepth = GridFunctions.ReadGridValue("Bathymetry","elevation_luanda_5m",NearshoreX, NearshoreY) #Depth [m]
#nearshZ = np.array(-2.0)

print nearshoreDepth

NumIntervals = 100
XCoords,YCoords,afstanden,hoogtes = GridFunctions.GetProfile("Bathymetry","elevation_luanda_5m",OffshoreX,OffshoreY,NearshoreX,NearshoreY,NumIntervals)


Profielbestand = open(r"C:\Projecten\Coastal Design Toolbox\Data\Testdata\Bathy\profiel.txt","w")

Profielbestand.write("Distance" + "\t" + "Elevation" + "\n")

for ProfileIndex in range(0,NumIntervals + 1):
	Profielbestand.write(str(XCoords[ProfileIndex]) + "\t" + str(YCoords[ProfileIndex]) + "\t" + str(afstanden[ProfileIndex]) + "\t" + str(hoogtes[ProfileIndex]) + "\n")


Profielbestand.close()
...
(Hsnew, Tpnew, relDirnew) = calcWaveConditions(Hs, Tp, relDir, offshZ, nearshZ)
