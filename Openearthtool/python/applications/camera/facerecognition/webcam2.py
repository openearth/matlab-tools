...
if __name__ == '__main__':
    ...
    # Set game screen
    screen = pygame.display.set_mode(SCREEN, 0)
    ...
    cam = camera.Camera(cams[0], SCREEN)
    cam.start()
    
    image = pygame.surface.Surface(SCREEN, 0, screen)
 
    while 1:  # Ze loop
 
        time.sleep(1 / 2)  # 2 frames per second
 
        image = cam.get_image(image)  # Get current webcam image
