...
class gmi2gmd:
    ...
    def add_fileIdentifier(self, xml, title, xpath_father):
        # xml node
        ins = "<gmd:fileIdentifier xmlns:gmd=\"" + self.namespaces_gmd['gmd'] + "\""">" \
              "<gco:CharacterString xmlns:gco=\"" + self.namespaces_gmd['gco'] + "\""">" \
                "{}" \
              "</gco:CharacterString>" \
              "</gmd:fileIdentifier>".format(title)
    ...
    # Convert iso GMI to
    def convert(self, xmlpath, generateID=False):
        # Read metadata
        ...
        # Add identifier
        if generateID:
            id = xmlpath.replace(":", "").replace(" ", "_").replace("\\", "_").replace("/", "_").replace('.xml', '') # nasty stuff going on here
            print id
            self.remove_tag_gmd(newdom, '//gmd:MD_Metadata/gmd:fileIdentifier')
            self.add_fileIdentifier(newdom, id, '//gmd:MD_Metadata')
...
if __name__ == '__main__':
    # Unit Test
    c = gmi2gmd('http://atsrsensors.org/images/esa%20cci%20aereosol%20icon.gif', 'http://opendap-nhi-data.deltares.nl/thredds/godiva2/godiva2.html')
    xmlcontent=c.convert(r'D:\sala\Documents\Deltares_Data_Portal\TDS_harvesting\iso_nhi_test\Modellen\AMIGO\modelinvoer\starting_heads\stijghoogte_laag1.xml', generateID=True)
    #xmlcontent = c.convert(r'.\test\test1\id54-K13APFM.xml')
    text_file = open(r'D:\sala\Documents\Deltares_Data_Portal\TDS_harvesting\iso_nhi_test\Modellen\AMIGO\modelinvoer\starting_heads\stijghoogte_laag1_GMD.xml', "w")
    text_file.write(xmlcontent)
    text_file.close()