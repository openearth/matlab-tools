...
def readConfig():
    ...
    cfile=os.path.join(os.path.dirname(os.path.realpath(__file__)), 'mepduinen_config.txt')
...
def gettimeseries(engine, x, y, date_from, date_too):
    ...
    epsg = 28992
    strSql = """SELECT gwm_id, meetpuntidentificatie, ST_Distance(st_setsrid(ST_Point({},{}),'{}'), geom) as distance
    ...
    lst = [0,1]
    gwm_id,loc_id = itemgetter(*lst)(a.fetchall()[0])
