...
def readConfig():
    ...
    cfile=os.path.join(os.path.dirname(os.path.realpath(__file__)), 'kleirijperij_config.txt')
...
def queryPostGISClosestPointMonstername(xk, yk, epsg=28992):
    # DB connect
    cf = os.path.join(os.path.dirname(os.path.realpath(__file__)), 'connection.txt')
    credentials = sqlfunctions.get_credentials(cf)
    
    # Look for closest point ()
    sql = """SELECT *, 
    ST_Distance(ST_PointFromText('POINT({} {})','{}'), st_transform(geom,28992)) as distance
             FROM locaties_monsternames
             ORDER BY distance
             LIMIT 1""".format(xk, yk, epsg)     
    res = sqlfunctions.executesqlfetch(sql, credentials)

    # If found call info about point
    if not(len(res)):
        return False
...
# Get values for the different mixed measurements at different depths for a given location
def queryPostGISLocIDMonsternames(locid, param):
    # DB connect
    cf = os.path.join(os.path.dirname(os.path.realpath(__file__)), 'connection.txt')
    credentials = sqlfunctions.get_credentials(cf)
    ...
    sql = """select {pid}, laag_nummer, locatie_nummer
             from krd_monstername_b
             where vak = '{lid}';""".format(pid=param, lid=locid)   
    #logging.info(sql)
    res = sqlfunctions.executesqlfetch(sql, credentials)
