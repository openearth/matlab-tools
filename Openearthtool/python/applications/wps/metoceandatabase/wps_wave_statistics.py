...
http://localhost/cgi-bin/pywps.cgi?&service=wps&version=1.0.0&request=DescribeProcess&identifier=wps_wind_statistics
http://localhost/cgi-bin/pywps.cgi?&service=wps&version=1.0.0&request=Execute&identifier=wps_wave_statistics&datainputs=[locations={"id":"OpenLayers_Geometry_Point_212","x":-1779633.3901414,"y":6836335.5553734,"bounds":{"left":-1779633.3901414,"bottom":6836335.5553734,"right":-1779633.3901414,"top":6836335.5553734}};startdate=19000101;enddate=20180101]
...
class Process(WPSProcess):
    ...
    def __init__(self):
        ...
        WPSProcess.__init__(self,
                            identifier="wps_wave_statistics",
                            title="""Wave statistics""",
                            abstract="""Statistics (Joint Occurence Table, Return Period Hs and Homogeneous check) on Wave direction and speed yearly or monthly from 1979-2015
                            """,
                            version="1.1",
                            storeSupported=True,
        ...
        self.type = self.addLiteralInput(
            identifier="type",
            title="Join Occurence Table (JOT), Return period values Hs (rvpHs), or homogeneous check",
            abstract="input=dropdownmenu",
            type=type(""),
            allowedValues=["Joint Occurrence Table", "Return period values Hs", "Homogeneous check Hs"],
            default="Joint Occurrence Table")

            #Amaury took ths out from allowedValues , "Homogeneous check Tp"
    ...
    def execute(self):
        ...
        mode = []
        if self.type.getValue() == "Joint Occurrence Table":
            mode = ["erai_jot_hsTmm10"]
        elif self.type.getValue() == "Return period values Hs":
            mode = ["rvpHs"]
        elif self.type.getValue() == "Homogeneous check Hs":
            mode = ["check_whether_eraiHs_is_homogeneous"]
        #Amaury took this out as there is not Tp homogeneous check png's
        #elif self.type.getValue() == "Homogeneous check Tp":
        #    mode = ["check_whether_erai_is_homogeneous"]
        logging.info(mode)
        ...
        for i in range(len(lat)):
            ...
            if lon[i] %1.0==0.0:
                lont = int(lon[i])
            filen = mode[0] + "_lon_" + str(lont) + "_lat_" + str(latt) + "*.png"
