...
class Process(WPSProcess):
    def __init__(self):
        ...
        WPSProcess.__init__(self,
                            identifier="blue2_scenario_waste",
                            title="Waste water scenario generation",
                            version="1.0",
                            storeSupported="true",
                            statusSupported="true",
                            abstract="""Please choose a waste water nutrient loads scenario to be generated. Specify at which spatial scale and the type of analysis. Click on Execute to proceed.""",
                            grassLocation=False)


        self.typescenario = self.addLiteralInput(identifier="typescenario",
                                            title="Waste water Nutrient Loads",
                                            abstract="input=dropdownmenu",
                                            type=type(""),
                                            allowedValues=['BAU', 'BAU-FC', '+10%', '+25%', '+50%', '+75%', 'MTFR'],
                                            default='+50%')        
        ...
        self.previewtype = self.addLiteralInput(identifier="previewtype",
                                            ...
                                            abstract="input=dropdownmenu",
                                            type=type(""),
                                            allowedValues=['Wastewater treatment investment distribution', 
                                            'BOD5 load [kg/d]', 'Nitrogen load [kg/d]', 'Phosphorus load [kg/d]',
                                            'BOD5 treatment efficiency [%]', 'Nitrogen treatment efficiency [%]',
                                            'Phosphorus treatment efficiency [%]'])
    ...
    def execute(self):
        ...
        spatialscale = 'NUTS2' #self.spatialscale.getValue() 
        ...
        # Call Ramboll script
        costDistr, nutrientLoads, nu_treatment_efficiency, results = nutient_loads(postgres_db, typescenario, spatialscale)
        ...
        # Add geometry column
        if spatialscale == 'NUTS2':
            gtable = 'nuts_rg_01m_2013_3035_levl_2'
            gcol = 'nu_nuts2_code' # weird but true
        else:
            gtable = 'nuts_rg_01m_2013_3035_levl_0'   
            gcol = 'nu_nuts2_code'
        updateGeometries(engine, tablename, gtable, 'nuts_id', tableId=gcol, geomschema='public')
