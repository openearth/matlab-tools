...
class Process(WPSProcess):
    def __init__(self):
        ...
        WPSProcess.__init__(self,
                            identifier="getlisfloodinfo",
                            title="WFD/LISFLOOD results explorer [maps/tables]",
                            version="1.0",
                            storeSupported="true",
                            statusSupported="true",
                            abstract="""Inspect model results from JRC's model LISFLOOD. Generate maps and tables from the available data.""",
                            grassLocation=False)
    ...
    def execute(self):
        ...
        logging.info('''INPUT [getlisfloodinfo]: table={}'''.format(seltable))        
        ...
        # Send back JSON
        json_str = json.dumps(values)
        #logging.info('''OUTPUT [getlisfloodinfo]: {}'''.format(json_str))
        outdata.write(json_str)
        self.json.setValue(outdata)
