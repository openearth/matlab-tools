...
# $Id: gwvstime.py 14128 2018-01-30 07:30:36Z sala $
...
class Process(WPSProcess):
    def __init__(self):
        ...
        WPSProcess.__init__(self,
                            identifier="gwvstime",
                            title="Ground Water Level vs Time",
                            version="1.0",
                            storeSupported="true",
                            statusSupported="true",
                            abstract="""Plot a time-series of groundwater levels versus time. The closest available location will be selected.""",
                            grassLocation=False)

        self.location = self.addLiteralInput(identifier="locgw",
    ...
    def execute(self):
        ...
        layerWFS = 'Emisk_Locations:Locationkeys_GW_levels'
        ...
        res=sql_gwlevels_vs_time(SQLSERVER_DB, SQLSERVER_HOST, SQLSERVER_USER, SQLSERVER_PASS, properties['locationke'])
        ...
        values['title'] = 'Groundwater level versus time'
        values['zoomx'] = xzoom
        values['zoomy'] = yzoom
        values['dist'] = dist
        json_str = json.dumps(values)
