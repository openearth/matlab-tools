# bbox_plot_timeseries: returns a timeseries plot of one parameter from a remote ODV file as png
...
# $Id: bbox_plot_timeseries.py 10666 2014-05-08 15:17:18Z boer_g $
# $Date: 2014-05-08 17:17:18 +0200 (Thu, 08 May 2014) $
# $Author: boer_g $
# $Revision: 10666 $
# $HeadURL: https://svn.oss.deltares.nl/repos/openearthtools/trunk/python/applications/wps/processes/bbox_plot_timeseries.py $
# $Keywords: $
...
class odvProcess(WPSProcess):
    def __init__(self):
        WPSProcess.__init__(self,
                            identifier="bbox_plot_timeseries", # must be same, as filename
                            ...
                            version="$Id: bbox_plot_timeseries.py 10666 2014-05-08 15:17:18Z boer_g $",
                            storeSupported=True,
...
# choose between different mimeType by padding to url (outside DataInputs=[]): &responsedocument=mapname=@mimetype=application/vnd.google-earth.kmz
    ...
    def execute(self):
        ...
        if dataPath[0:13]=='postgresql://':
            ...
            if len(ODV.data)==0: # in case the query returns empty result
...
# PNG: (clean-up if needed)
        ...
        try: 
            if allinrange == 0:
                edmocdi = pyodv.odv2edmocdi(ODV,self.parameter.getValue(),zname,clims,zlims,times,plot_type='timeseries') # arg plot_type sucks a bit but no time now.
                pyodv.odv2timeseries(tempname_png,ODV,self.parameter.getValue(),zname,clims,zlims,times,cmapstr,self.log10.getValue(),colorvalue,markertype,markersizevalue,alphavalue)
            else:
                edmocdi = pyodv.odv2edmocdi_allinrange(ODV,self.parameter.getValue(),zname,clims,zlims,times,plot_type='timeseries') # arg plot_type sucks a bit but no time now.
                pyodv.odv2timeseries_allinrange(tempname_png,ODV,self.parameter.getValue(),zname,clims,zlims,times,cmapstr,self.log10.getValue(),colorvalue,markertype,markersizevalue,alphavalue)
        except:
                import matplotlib.pyplot as plt
                temp = os.path.join(tempPath,'temp.png')
                tempNoData = os.path.join(tempPath,'NoData.json')
