...
# bbox_plot_profile returns a scatter/profile plot of one parameter from a remote ODV file as png
...
# $Id: bbox_plot_profile.py 10971 2014-07-18 09:39:38Z boer_g $
# $Date: 2014-07-18 11:39:38 +0200 (Fri, 18 Jul 2014) $
# $Author: boer_g $
# $Revision: 10971 $
# $HeadURL: https://svn.oss.deltares.nl/repos/openearthtools/trunk/python/applications/wps/processes/bbox_plot_profile.py $
# $Keywords: $
...
class odvProcess(WPSProcess):
    def __init__(self):
        WPSProcess.__init__(self,
                            identifier="bbox_plot_profile", # must be same, as filename
                            ...
                            version="$Id: bbox_plot_profile.py 10971 2014-07-18 09:39:38Z boer_g $",
                            storeSupported=True,
                            statusSupported=True,
                            abstract="returns a scatter/profile plot of 1 parameter from a bounding box as png",
                            grassLocation=False)
        ...
        self.clim0 = self.addLiteralInput(identifier = "clim0",
                                           title      = "lower x-axis limit for plot, skip keyword for autoscale",
                                           type       = type(0.0),
        ...
        self.clim1 = self.addLiteralInput(identifier = "clim1",
                                           title      = "upper x-axis limit for plot, skip keyword for autoscale",
                                           type       = type(100.0),
        ...
        self.markersize = self.addLiteralInput(identifier = "markersize",
                                           ...
                                           type       = type(1.0),
                                           default    = 6.0)
        self.linestyle  = self.addLiteralInput(identifier = "linestyle",
                                           title      = "linestyle from matplotlib dictionary: http://matplotlib.org/api/pyplot_api.html?highlight=plot#matplotlib.pyplot.plot",
                                           type       = type("--"), # that's a string: abbreviations or complete name
                                           default    = "solid")
        self.linewidth  = self.addLiteralInput(identifier = "linewidth",
                                           title      = "linewidth from matplotlib dictionary: http://matplotlib.org/api/pyplot_api.html?highlight=plot#matplotlib.pyplot.plot",
                                           type       = type(1.0),
                                           default    = 1)
...
# so either do a prior clean-up, or add a unique code and schedule a cleanup batch
    ...
    def execute(self):
        ...
        # logging.info("bbox_plot_profile: tempPath: " + tempPath)
        # logging.info("bbox_plot_profile: dataPath: " + dataPath)
        ...
        if dataPath[0:13]=='postgresql://':
            ...
            #logging.info(self.z.getValue()+' is the z value passed')
            ODV = pyodv.odv2orm_query.orm_from_bbox(dataPath,self.parameter.getValue(),W,E,S,N,self.z.getValue())
            ...
            if len(ODV.data)==0 or self.parameter.getValue() in mytilist: # in case the query returns empty result
            ...
            logging.info("bbox_plot_timeseries: ODV read from PostgreSQL with n=" + str(len(ODV.data)))
...
# PNG: (clean-up if needed)
        ...
        tempname_png = os.path.join(tempPath,str(W) + '_' + str(S) + '_' + str(E) + '_' + str(N) + '_' +
                                             self.parameter.getValue() + '_profile.png')
        ...
        linestylevalue  = self.linestyle.getValue()
        linewidthvalue  = self.linewidth.getValue()
        alphavalue      = self.alpha.getValue()
        
        allinrange      = self.allinrange.getValue()
        
        if allinrange == 0:
            edmocdi = pyodv.odv2edmocdi(ODV,self.parameter.getValue(),zname,clims,zlims,times,plot_type='profile') # arg plot_type sucks a bit but no time now.
            pyodv.odv2profile(tempname_png,ODV,self.parameter.getValue(),zname,clims,zlims,times,cmapstr,self.log10.getValue(),colorvalue,markertype,markersizevalue,linestylevalue,linewidthvalue,alphavalue)
        else:
            edmocdi = pyodv.odv2edmocdi_allinrange(ODV,self.parameter.getValue(),zname,clims,zlims,times,plot_type='profile') # arg plot_type sucks a bit but no time now.
            pyodv.odv2profile_allinrange(tempname_png,ODV,self.parameter.getValue(),zname,clims,zlims,times,cmapstr,self.log10.getValue(),colorvalue,markertype,markersizevalue,linestylevalue,linewidthvalue,alphavalue)
