...
class Process(WPSProcess):
    def __init__(self):
        ...
        WPSProcess.__init__(self,
            ...
            abstract="""Tidal prediction tool can be used for different 
                ...
                Prediction is calculated from Topex/Poseidon dataset. This version calculates
                the water level according to the astronomical tide along a circular path over 
                a certain period.""",
            version = "0.1",
            storeSupported = True,
        ...
        self.radIn = self.addLiteralInput(identifier="rad",
                       title = "Give the rad for the circular motion",
                       type = FloatType, 
                       default = 1)
    ...
    def execute(self):
        ...
        lon = np.cos(np.linspace(0,2*rad*np.pi,self.rangeIn.getValue())) + self.lonIn.getValue()
        lat = np.sin(np.linspace(0,2*rad*np.pi,self.rangeIn.getValue())) + self.latIn.getValue()
        h_t = h(lon, lat, dates)
        
        fig, axes = plt.subplots(1,2, figsize=(10,4))
        sc = axes[0].scatter(lon,lat, s=h_t*50, c=[x.toordinal() for x in dates])
        plt.colorbar(sc, ax=ax)
        axes[1].plot(dates, h_t)
        
        #fig,ax = plt.subplots(1,1)
        #outplot = pandas.Series(data=h_t, index=dates).plot(ax=axes[0])
