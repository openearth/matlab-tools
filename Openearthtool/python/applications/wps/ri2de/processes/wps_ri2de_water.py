...
# $HeadURL: https://svn.oss.deltares.nl/repos/openearthtools/trunk/python/applications/wps/ri2de/processes/wps_ri2de_water.py $
# $Keywords: $
...
class WpsRi2deWater(Process):
	...
	def __init__(self):
		...
		outputs = [ComplexOutput('output_json',
		                         'Ri2DE calculation of a water layer [0,1,2]',
		                         supported_formats=[Format('application/json')])]

		super(WpsRi2deWater, self).__init__(
		    self._handler,
		    identifier='ri2de_calc_water',
		    version='1.0',
		    title='backend process for the RI2DE tool project, calculates water and classifies into classes',
		    abstract='It uses gdal tools to calculate water presence\
		     and sends back a JSON reply wrapped in the xml/wps format with the wmslayer to show',
		    profile='',
		    metadata=[Metadata('WpsRi2deWater'), Metadata('Ri2DE/watercourse')],
		    inputs=inputs,
		    outputs=outputs,
	...
	def _handler(self, request, response):
		...
		try:
			...
			s,w,n,e = get_roads_envelope_geojson(geoFname) #bounding box xmin, ymin, xmax, ymax

			# Data extraction / wcs
			waterfname = os.path.join(self.workdir, 'water_{}.tif'.format(int(1000000*time.time())))
			print(waterfname)
			cut_wcs(s,w,n,e, layer_info['layername'], layer_info['owsurl'], waterfname)

			# Classify water
			classfname = classify_raster(waterfname, self.workdir, layer_info['classes'], 'waterJRC')

			# Apply mask
			apply_road_mask(cf, classfname, geoFname, self.workdir)

			# Upload to GeoServer to TEMP workspace
			layername = os.path.basename(waterfname).replace('.tif', '')
