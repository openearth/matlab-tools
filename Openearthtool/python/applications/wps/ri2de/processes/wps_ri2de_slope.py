...
class WpsRi2deSlope(Process):
	...
	def __init__(self):
		...
		outputs = [ComplexOutput('output_json',
		                         'Ri2DE calculation of a slope layer [0,1,2]',
		                         supported_formats=[Format('application/json')])]

		super(WpsRi2deSlope, self).__init__(
		    self._handler,
		    identifier='ri2de_calc_slope',
		    version='1.0',
		    title='backend process for the RI2DE tool project, calculates slope and classifies into classes',
		    abstract='It uses gdal tools to calculate slope\
		     and sends back a JSON reply wrapped in the xml/wps format with the wmslayer to show',
		    profile='',
		    metadata=[Metadata('WpsRi2deSlope'), Metadata('Ri2DE/slope')],
		    inputs=inputs,
		    outputs=outputs,
	...
	def _handler(self, request, response):
		...
		try:
			...
			# Data extraction [Digital Elevation Model] / wcs						
			demfname = os.path.join(self.workdir, 'dem.tif')			
			cut_wcs(s,w,n,e, layer_info['layername'], layer_info['owsurl'], demfname)			

			# Conversion to slope			
			slopefname = os.path.join(self.workdir, 'slope_{}.tif'.format(int(1000000*time.time())))			
			dem_to_slope(demfname, slopefname)

			# Classify slope			
			classfname = classify_raster(slopefname, self.workdir, layer_info['classes'], '123')
