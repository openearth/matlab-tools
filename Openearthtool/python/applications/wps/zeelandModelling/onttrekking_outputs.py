...
# $Id: onttrekking_outputs.py 13820 2017-10-12 12:36:42Z sala $
# $Date: 2017-10-12 14:36:42 +0200 (Thu, 12 Oct 2017) $
# $Author: sala $
# $Revision: 13820 $
# $HeadURL: https://svn.oss.deltares.nl/repos/openearthtools/trunk/python/applications/wps/zeeland/onttrekking_outputs.py $
# $Keywords: $
...
class onttrekking_IO():

    def __init__(self, c): 
        self.config = c   

    # Write a csv file
    def writeCSV(self, csvfile, res, header='geotop'):
        # Assuming res is a list of lists
        with open(csvfile, "w") as output:
            writer = csv.writer(output, lineterminator='\n')
            writer.writerows(res)
            logging.info('WRITTEN: {}'.format(csvfile))
    ...
    def geoserverUploadShp(self, zippath, tmpdir, sld_style='zeeland_isolines'):
        # Return wms url        
        layername = os.path.basename(tmpdir)+'_isolines'
        wmslay = 'NHIONLINE:'+layername
        logging.info('''GEOSERVER [onttrekking]: wmslayer={}'''.format(wmslay))
    ...
    # Upload file to GeoServer
    def geoserverUploadGtif(self, gtifpath, tmpdir, sld_style='zeeland_raster'):
        # Return wms url
    ...
    def addSLD(self, layername, sld_style):
        ...
        try:
            ...
            logging.info('INFO: Setting style ...')
            layer._set_default_style(sld_style)
            # Update and save layer
            cat.save(layer)
            cat.reload()
            logging.info('INFO: Sytle was fixed OK')
        except:
            logging.info('ERROR while connecting to geoserver to change SLD styling')
            pass        


    # Check on postgres if X,Y is inside layer L [projection rdnew]
    ...
    # Create shapefile zip to upload to geoserver [projection rdnew]
