...
# $Id: transect_nhiflux.py 13812 2017-10-10 18:50:44Z sala $
# $Date: 2017-10-10 20:50:44 +0200 (Tue, 10 Oct 2017) $
# $Author: sala $
# $Revision: 13812 $
# $HeadURL: https://svn.oss.deltares.nl/repos/openearthtools/trunk/python/applications/wps/onlinemodelling/transect_nhiflux.py $
# $Keywords: $
...
class transect_nhiflux(Process):
    ...
    def __init__(self):
        ...
        inputs = [LiteralInput(identifier="transect_nhiflux",
        ...
        super(transect_nhiflux,self).__init__(
                            self._handler,
                            identifier="transect_nhiflux",
                            title="Toon ondergrond volgens LHM3.3 [transect]",
                            version="1.3.3.7",
    ...
    def _handler(self,request,response):
        ...
        linestr_str = request.inputs["transect_nhiflux"][0].data
        logger.info(
            '''INPUT [transect_nhiflux]: location={}'''.format(str(linestr_str)))
        ...
        if len(coords) > nsamplesmax:
            ...
            try:
                for x, y in coords:
                    ...
                    nhi = nhi_invoer(x, y)
                    prev = float(hoogte)
                    nhi_sort = sorted(list(nhi.items()),
                                      key=operator.itemgetter(0))

                    for item in nhi_sort:
                        key, value = item
                        value = [
                            float(x) if x is not None else None for x in value]
        ...
        if data_error:
            ...
            values['title_plot'] = """LHM3.3 (xo={}m, yo={}m, xn={}m, yn={}m)""".format(
