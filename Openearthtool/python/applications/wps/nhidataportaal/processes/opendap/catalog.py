...
# $Date: 2013-07-09 11:51:58 +0200 (Tue, 09 Jul 2013) $
# $Author: boer_g $
# $Revision: 8903 $
...
xpath_catalogs = './/{{{threddsns}}}catalogRef[@{{{xlinkns}}}href]'.format(
    **ns)
...
def getchildren(url):
    ...
    # get the dap service
    services = [service for service in xml.findall(
        xpath_service) if service.attrib['serviceType'].lower() == 'opendap']
    assert len(services) == 1, "%s has %s services: %s" % (
        url, len(services), services)
    service_base = services[0].attrib['base']
    base_url = urlparse.urlunparse(
        (parsed_url.scheme, parsed_url.netloc, service_base, '', '', ''))
