...
class NetcdfOnOpendap(object):
    ...
    def __init__(self, url, index_variable_names, value_variable_names):
        ...
        self.index_variables = OrderedDict((name, NetcdfIndexVariable(
            self.dataset.variables[name])) for name in index_variable_names)
        self.value_variables = OrderedDict((name, NetcdfValueVariable(
            self.dataset.variables[name])) for name in value_variable_names)
...
class GeoTopOnOpendap(NetcdfOnOpendap):
    ...
    def __init__(self, url, path=None):
        NetcdfOnOpendap.__init__(self, url, index_variable_names=(
            'x', 'y', 'z'), value_variable_names=('lithok', 'strat'))
    ...
    def __layers(self, x_index, y_index, max_depth):
        ...
        layer_top_heights = [
            value+height_stride for value in list(self.index_variables['z'].values())]
        strat_type = self.value_variables['strat'].type
        lithok_type = self.value_variables['lithok'].type
        prev_strat = None
        prev_lithok = None
        surface_layer_height = None
        for strat, lithok, height in zip(reversed(strat_values), reversed(lithok_values), reversed(layer_top_heights)):
            ...
            if type(strat) == strat_type and type(lithok) == lithok_type and strat != -32767 and lithok != -127:
                if strat != prev_strat or lithok != prev_lithok:
                    ...
                    if depth < max_depth:
                        if len(results) > 0:
                            results[-1][-1] = depth
                        results.append([strat_translation_long[strat_translation[strat]],
                                        lithok_translation[lithok], depth, max_depth])
        if self.output_file is not None:
    ...
    def get_all_layers(self, x, y):
        ...
        max_depth = self.index_variables['z'].type(150)  # numpy.inf)
