...
l_gi_type = ["float()", "float()", "float()", "float()", "int()",
...
l_raster_format_str = ["INVALID", "AAIGrid", "IDF",
                       "PCRaster", "AIG", "BIL", "GTiff", "netCDF", "HDF4Image"]
l_raster_format_ext = ["invalid", "asc", "idf",
                       "map", "", "bil", "tif", "nc", "hdf"]
...
l_raster_format_str += ["ACE2", "ADRG", "ARG", "AirSAR", "BAG", "BIGGIF", "BLX", "BMP", "BSB", "BT", "CEOS", "COASP", "COSAR", "CPG", "CTG", "CTable2", "DIMAP", "DIPEx", "DOQ1", "DOQ2", "DTED", "E00GRID", "ECRGTOC", "EHdr", "EIR", "ELAS", "ENVI", "ERS", "ESAT", "FAST", "FIT", "FujiBAS", "GFF", "GIF", "GMT", "GRASSASCIIGrid", "GRIB", "GS7BG", "GSAG", "GSBG", "GSC", "GTX", "GXF", "GenBin", "HDF4", "HDF5", "HDF5Image", "HF2", "HFA", "HTTP", "IDA", "ILWIS", "INGR", "IRIS", "ISIS2", "ISIS3",
                        "JAXAPALSAR", "JDEM", "JP2OpenJPEG", "JPEG", "KMLSUPEROVERLAY", "KRO", "L1B", "LAN", "LCP", "LOSLAS", "Leveller", "MAP", "MBTiles", "MEM", "MFF", "MFF2", "MSGN", "NDF", "NGSGEOID", "NITF", "NTv2", "NWT_GRC", "NWT_GRD", "OGDI", "OZI", "PAux", "PCIDSK", "PDF", "PDS", "PNG", "PNM", "PostGISRaster", "R", "RIK", "RMF", "RPFTOC", "RS2", "RST", "Rasterlite", "SAGA", "SAR_CEOS", "SDTS", "SGI", "SNODAS", "SRP", "SRTMHGT", "TIL", "TSX", "Terragen", "USGSDEM", "VRT", "WCS", "WMS", "XPM", "XYZ", "ZMap"]
l_raster_format_ext += [s.lower()
                        for s in l_raster_format_str[len(l_raster_format_num):]]
l_raster_format_num += list(range(len(l_raster_format_num),
                                  len(l_raster_format_str)))
...
l_pcr_Struct.dtype.names = [
    "data_type", "data_bytesize", "structFormat", "cellRepr", "nodata"]
...
class rasterArr(object):
    ...
    def __init__(self, arr, gi=None, nodata=None, **kwargs):
        ...
        try:                                     # arr is rasterArr object: make deep copy of arr, gi and nodata
            arr, gi, nodata = deepcopy(arr.arr), deepcopy(
                arr.gi()), deepcopy(arr.nodata())
        except:
            # arr is masked array: nodata is set to fill_value
            if type(arr) == ma.core.MaskedArray:
                nodata = _value_dtype2type(arr.fill_value)
        # array: numpy MaskedArray object
        self.arr = ma.array(arr)
        # geographical information (geo_info, gi): dictionary with keys xll,yll,dx,dy,nrow,ncol,proj,ang
        self.__gi = gi2dict(gi, **kwargs)
        self.__nodata = nodata                     # nodata value

        # make object consistent
        # set number of dimensions of array to minimum of 2 (lower 2 dimensions are rows and columns)
        self.__set_ndim_arr()
        # mask nodata cells of array and re-set nodata of object
        ...
        # set nrow and ncol of geo_info to nrow and ncol of array
    ...
    def set_nodata(self, nodata=None):
        ...
        if type(nodata) in [str, bool, int, float]:
            ...
            if type(nodata) == str:
                ...
                try:
                    exec(
                        "self.arr.mask=ma.where(self.arr %s,True,self.arr.mask)" % (nodata))
                except:
                    exec("self.arr.mask=np.where(%s,True,self.arr.mask)" % (nodata))
                try:
                    if self.arr.mask.min():
                        self.__nodata = _value_dtype2type(
                            _get_prefered_nodata(self))
                    else:
                        raise
                except:
                    self.__nodata = _value_dtype2type(
    ...
    def set_mask(self, mask=None, add=False):
        ...
        if type(mask) in [str, bool, int, float]:
            ...
            if add:
                try:
                    mask.gi()
                    self.arr.mask = np.maximum(self.arr.mask, mask.arr)
                except:
                    self.arr.mask = np.maximum(self.arr.mask, mask)
            else:
                try:
                    mask.gi()
                    self.arr.mask = mask.arr
                except:
                    self.arr.mask = mask
            if self.arr.mask.ndim == 0:
                self.arr.mask = np.ones(self.arr.shape, bool)*self.arr.mask
    ...
    def cover(self, other):
        ...
        mask1 = a.arr.mask
        if mask1.ndim == 0:
            mask1 = np.ones(a.arr.shape, bool)*mask1
        try:
            other.gi()
    ...
    def __set_ndim_arr(self):
        ...
        if self.arr.ndim == 0:
            try:
                self.arr = ma.resize(
                    self.arr, (self.__gi["nrow"], self.__gi["ncol"]))
            except:
                pass
        for i in range(self.arr.ndim, 2):
            self.arr = ma.reshape(self.arr, (1,)+self.arr.shape)
        for i in range(3, self.arr.ndim):
            self.arr = self.arr[0]
        if self.arr.mask.ndim == 0:
            self.arr.mask = np.ones(self.arr.shape, bool)*self.arr.mask
    ...
    def __set_types_gi(self):
        ...
        # xll: float or None
        try:
            self.__gi["xll"] = float(self.__gi["xll"])
        except:
            self.__gi["xll"] = None
         # yll: float or None
        try:
            self.__gi["yll"] = float(self.__gi["yll"])
        except:
            self.__gi["yll"] = None
        ...
        try:
            if np.array(self.__gi["dx"]).ndim > 0:
                try:
                    ...
                    self.__gi["dx"] = np.array(self.__gi["dx"], _get_min_dtype(
                        np.array(1, float32), self.__gi["dx"].min(), self.__gi["dx"].max()))
                except:
                    self.__gi["dx"] = None
            else:
                self.__gi["dx"] = float(self.__gi["dx"])
        except:
            self.__gi["dx"] = None
        ...
        try:
            if np.array(self.__gi["dy"]).ndim > 0:
                try:
                    ...
                    self.__gi["dy"] = np.array(self.__gi["dy"], _get_min_dtype(
                        np.array(1, float32), self.__gi["dy"].min(), self.__gi["dy"].max()))
                except:
                    self.__gi["dy"] = None
            else:
                self.__gi["dy"] = float(self.__gi["dy"])
        except:
            self.__gi["dy"] = None
          # dx and dy: convert to float or array, depending on ieq
        try:
            self.__gi["dx"] = self.__dx_ieq()
        except:
            pass
        try:
            self.__gi["dy"] = self.__dy_ieq()
        except:
            pass
         # ncol: int or None
        try:
            self.__gi["ncol"] = int(self.__gi["ncol"])
        except:
            self.__gi["ncol"] = None
         # nrow: int or None
        try:
            self.__gi["nrow"] = int(self.__gi["nrow"])
        except:
            self.__gi["nrow"] = None
         # proj: int (0 or 1)
        try:
         ...
         # ang: float
        try:
            self.__gi["ang"] = float(self.__gi["ang"])
        except:
            self.__gi["ang"] = 0.0
         # crs: string
        try:
            if self.__gi["crs"] == None:
                raise
            self.__gi["crs"] = crs2epsg(
                self.__gi["crs"], "%s" % (self.__gi["crs"]))
        except:
            self.__gi["crs"] = ""
    ...
    def __set_nrowcol_gi(self):
        ...
        if np.array(self.__gi["dx"]).ndim > 0:
            ...
            if len(dx) != self.__gi["ncol"]:
                self.__gi["dx"] = dx.mean()
        if np.array(self.__gi["dy"]).ndim > 0:
            dy = np.ravel(np.array(self.__gi["dy"]))
            if len(dy) != self.__gi["nrow"]:
                self.__gi["dy"] = dy.mean()
    ...
    def get_gi(self, *args):
        ...
        for key in args:
            if key in ['gi', 'gi_list', 'nodata', 'xll', 'xur', 'yll', 'yur', 'dx', 'Dx', 'dy', 'Dy', 'ieq', 'nrow', 'ncol', 'proj', 'ang', 'crs']:
                try:
                    exec("l_result.append(self.%s())" % (key))
                except:
                    pass
        return l_result
    ...
    def extent(self):
        ...
        """
        try:
            return [self.xll(), self.yll(), self.xur(), self.yur()]
        except:
            return None
    ...
    def Dx(self):
        ...
        if type(self.dx()) not in [int, float]:
            try:
                return float(sum(self.dx()))
            except:
                return None
        else:
            try:
                return float(self.dx())*self.ncol()
            except:
                return None
    ...
    def Dy(self):
        ...
        if type(self.dy()) not in [int, float]:
            try:
                return float(sum(self.dy()))
            except:
                return None
        else:
            try:
                return float(self.dy())*self.nrow()
            except:
                return None
    ...
    def ieq(self):
        ...
        if type(self.dx()) not in [int, float]:
            try:
                if min(self.dx()) != max(self.dx()):
                    return 1
                elif type(self.dy()) not in [int, float]:
                    try:
                        if min(self.dy()) != max(self.dy()):
                            return 1
                        else:
                            return 0
                    except:
    ...
    def __dx_ieq(self):
        ...
        if self.ieq() == 1:
            if type(self.dx()) not in [int, float]:
                try:
                    return np.array(self.dx(), float32)
                except:
                    return None
            else:
                try:
                    return np.ones((self.ncol(),), float32)*self.dx()
                except:
                    return None
        else:
            if type(self.dx()) not in [int, float]:
    ...
    def __dy_ieq(self):
        ...
        if self.ieq() == 1:
            if type(self.dy()) not in [int, float]:
                ...
                try:
                    return np.ones((self.nrow(),), float32)*self.dy()
                except:
                    return None
        else:
            if type(self.dy()) not in [int, float]:
                try:
                    return float(self.dy()[0])
                except:
                    return None
            else:
    ...
    def write(self, f_raster, raster_format=0, frm_asc="%s", xy_center_asc=False, nodata_asc=None, itb_idf=None, nodata_idf=None, frm_map=None, f_hdr=None):
        ...
        :ref:`raster_formats`
        """
        if type(f_raster) not in [list, tuple]:
            f_raster = [f_raster]
        ...
        if type(raster_format) == str:
            try:
                raster_format = l_raster_format_num[1:][l_raster_format_str[1:].index(
                    raster_format)]
            except:
                raster_format = 0
        if raster_format not in l_raster_format_num[1:]:
            raise Exception("Cannot determine raster format (%s)" %
                            (f_raster[-1]))
        rnk = 3
        if self.arr.ndim == 2:
            self.arr, rnk = ma.reshape(self.arr, (1,)+self.arr.shape), 2
        self.__set_types_gi()
        ...
        if raster_format not in [5, 6, 7] or (raster_format in [5, 6, 7] and len(f_raster) != 1):
            for i in range(len(f_raster), self.arr.shape[0]):
                f_raster += ["%s_#%d%s" % (nam, i+1, ext)]
        for f in f_raster:
            d = os.path.dirname(os.path.abspath(f))
            if not os.path.isdir(d):
                os.makedirs(d)
        if raster_format == 1:
            for l in range(0, self.arr.shape[0]):
                self.__arr2asc(l, f_raster[l], frm_asc,
                               xy_center_asc, nodata_asc)
        elif raster_format == 2:
            for l in range(0, self.arr.shape[0]):
                self.__arr2idf(l, f_raster[l], itb_idf, nodata_idf)
        elif raster_format == 3:
            for l in range(0, self.arr.shape[0]):
                self.__arr2map(l, f_raster[l], frm_map)
        elif raster_format == 5:
            if len(f_raster) == 1:
                self.__arr2bil(-1, f_raster[0], f_hdr)
            else:
                for l in range(0, self.arr.shape[0]):
                    self.__arr2bil(l, f_raster[l], f_hdr)
        elif raster_format == 7:
            if len(f_raster) == 1:
                self.__arr2netcdf(-1, f_raster[0])
            else:
                for l in range(0, self.arr.shape[0]):
                    self.__arr2netcdf(l, f_raster[l])
        else:
            if len(f_raster) == 1:
                self.__arr2gdal(-1, f_raster[0], raster_format)
            else:
                for l in range(0, self.arr.shape[0]):
                    self.__arr2gdal(l, f_raster[l], raster_format)
        if rnk == 2:
    ...
    def __arr2asc(self, l, f_raster, frm_asc=None, xy_center_asc=False, nodata_asc=None):
        ...
        try:
            ...
            if dx == None:
                raise Exception('no cellsize information present, dx == None')
            if dy == None:
                raise Exception('no cellsize information present, dy == None')
            if type(dx) != float:
                dx = float(dx.mean())
            if type(dy) != float:
                dy = float(dy.mean())
    ...
    def __arr2idf(self, l, f_raster, itb_idf=None, nodata_idf=None):
        ...
        try:
            ...
            if self.mask().all():
                outf.write(struct.pack("=3L 7f 4B", 1271, ncol, nrow, xll, xur, yll,
                                       yur, nodata, nodata, nodata, self.ieq(), int(itb_flag), 0, 0))
            else:
                outf.write(struct.pack("=3L 7f 4B", 1271, ncol, nrow, xll, xur, yll, yur, self.arr.min(
                ), self.arr.max(), nodata, self.ieq(), int(itb_flag), 0, 0))
            if self.ieq() == 0:
            ...
            if self.crs() != "":
                ...
                if crs != "":
                    ...
                    outf.write(struct.pack("=2L%ds" %
                                           (len(crs)), 1, len(crs)/4, crs))
            outf.close()
        except Exception as err:
            try:
                outf.close()
    ...
    def __arr2map(self, l, f_raster, frm_map=None):
        ...
        try:
            ...
            if frm_map not in l_pcr_valueScale["frm_map"].tolist():
                if self.arr.dtype in [bool, bool8]:
                    frm_map = "boolean"
                elif self.arr.dtype in [float16, float32, float64]:
                    frm_map = "scalar"
                else:
                    frm_map = "nominal"

            xxx, valueScale, l_data_type = l_pcr_valueScale[l_pcr_valueScale["frm_map"] == frm_map][0]
            nodata = l_pcr_Struct["nodata"][l_pcr_Struct["data_type"]
                                            == l_data_type[0]][0]
            ...
            xxx, data_bytesize, structFormat, cellRepr, nodata = l_pcr_Struct[
                l_pcr_Struct["data_type"] == arr.dtype.name][0]

            if nodata == "np.nan":
                nodata = struct.unpack("=%s" % (structFormat), struct.pack(
                    "=%ds" % (data_bytesize), "\xff"*data_bytesize))

            arr = np.ravel(ma.filled(arr, np.array(nodata, arr.dtype)))

            outf = open(f_raster, "wb")
            outf.write(struct.pack("=27s", "RUU CROSS SYSTEM MAP FORMAT"))
            outf.write(struct.pack("=5s HLHLHL 14s 2H", "\x00"*5, 2, 0,
                                   self.proj(), 0, 1, 1, "\x00"*14, valueScale, cellRepr))
            outf.write(struct.pack("=%s" % ("%s%ds" % (structFormat, 8-data_bytesize)*2),
                                   minval, "\xff"*(8-data_bytesize), maxval, "\xff"*(8-data_bytesize)))
            outf.write(struct.pack("=ddLLddd", self.xll(),
                                   self.yur(), nrow, ncol, dx, dy, self.ang()))
    ...
    def __arr2bil(self, l, f_raster, f_hdr=None):
        ...
        try:
            ...
            if type(dy) != float:
                dy = float(dy.mean())
            if f_hdr == None:
                f_hdr = "%s.hdr" % (os.path.splitext(f_raster)[0])

            if l == -1:
                arr, nlay = np.array(self.arr), self.arr.shape[0]
            else:
                arr, nlay = np.array(self.arr[l]), 1

            if arr.dtype in [float16, float32, float64]:
                arr, nbit = np.array(arr, float32), 0
            else:
                nbit = arr.itemsize
            arr = np.ravel(arr)
    ...
    def __arr2netcdf(self, l, f_raster):
        ...
        try:
            ...
            if go:
                ...
                if l == -1:
                    llay = list(range(0, self.arr.shape[0]))
                else:
                    llay = [l]
                for lay in llay:
                    outf.createVariable("Band%d" % (
                        lay+1), self.arr.dtype, ("y_center", "x_center"), fill_value=self.nodata())
    ...
    def __arr2gdal(self, l, f_raster, raster_format):
        ...
        try:
            ...
            try:
                try:
                    raster_format = l_raster_format_str[raster_format]
                except:
                    raster_format = l_raster_format_str[0]
                driver = osgeo.gdal.GetDriverByName(raster_format)
                if driver.GetMetadata()[osgeo.gdal.DCAP_CREATE] != "YES":
                    raise
            except:
                raise Exception(
                    "Cannot write to raster_format %s (not supported or invalid)" % (raster_format))
            ...
            l_data_type = [["Byte", "UInt16", "Int16", "UInt32", "Int32", "Float32", "Float64", "CInt16", "CInt32", "CFloat32", "CFloat64"],
                           [osgeo.gdal.GDT_Byte, osgeo.gdal.GDT_UInt16, osgeo.gdal.GDT_Int16, osgeo.gdal.GDT_UInt32, osgeo.gdal.GDT_Int32, osgeo.gdal.GDT_Float32,
                               osgeo.gdal.GDT_Float64, osgeo.gdal.GDT_CInt16, osgeo.gdal.GDT_CInt32, osgeo.gdal.GDT_CFloat32, osgeo.gdal.GDT_CFloat64],
                           [uint8, uint16, int16, uint32, int32, float32,
                               float64, int16, int32, float32, float64],
                           ["B", "H", "h", "L", "l", "f", "d", "h", "l", "f", "d"]]
            if l == -1:
                arr, nlay = np.array(self.arr), self.arr.shape[0]
            else:
                arr, nlay = np.array(self.arr[l:l+1]), 1
            ...
            outf = driver.Create(f_raster, ncol, nrow, nlay,
                                 l_data_type[1][l_data_type[2].index(arr.dtype)])
            if proj == 0:
                outf.SetGeoTransform([xll, dx, rot1, yul, rot2, dy])
            else:
                outf.SetGeoTransform([xll, dx, rot1, yul, rot2, -dy])
            try:
                outf.SetProjection(crs2wkt(self.crs(), self.crs()))
            except:
                if self.crs() != "":
                    crs2prj("%s.prj" %
                            (os.path.splitext(f_raster)[0]), self.crs())
    ...
    def arr2figure(self, colorscale="jet", n_class=8, scale_type="linear", perc_bnd=[0, 100], use_underover=True, list_of_shapes=[]):
        ...
        if str(type(colorscale)) == "<class 'plot_func.legendScale'>":
            ...
            leg = legendScale(self, n=n_class, scale_type=scale_type,
                              perc_bnd=perc_bnd, colorscale=colorscale)
        ...
        ax = axes([pad/fig_w, pad/fig_h, ax_w/fig_w, ax_h/fig_h],
                  frameon=True, xticks=[], yticks=[])
        for child in ax.get_children():
            if isinstance(child, matplotlib.spines.Spine):
                child.set_edgecolor((0.5, 0.5, 0.5, 1))

        ax_cb = axes([3*pad/fig_w+ax_w/fig_w, 3*pad/fig_h, 0.25*cb_w /
                      fig_w, cb_h/fig_h], frameon=False, xticks=[], yticks=[])
        ax.set_xlim([self.xll(), self.xur()])
        ax.set_ylim([self.yll(), self.yur()])
        ax.imshow(self.arr, extent=[self.xll(), self.xur(), self.yll(
        ), self.yur()], cmap=leg.getCmap(), norm=norm, interpolation="nearest")

        if len(list_of_shapes) > 0:

            for i in range(0, len(list_of_shapes)):
                rec = list(list_of_shapes[i])
                for i in range(len(rec), 7):
                    rec += [None]
                ...
                if marker_size == None:
                    marker_size = 4
                if shp_type == "POINT":
                    ax.scatter(shp_xy[:, 0], shp_xy[:, 1], s=marker_size, c=fill_color,
                               marker=marker, linewidths=line_width, edgecolors=line_color)
                elif shp_type == "POLYLINE":
                    ax.add_collection(matplotlib.collections.LineCollection(
                        shp_xy, colors=line_color, linestyle="solid", linewidth=line_width))
                elif shp_type == "POLYGON":
                    ax.add_collection(matplotlib.collections.PatchCollection([matplotlib.patches.Polygon(shp_xy[i], closed=True) for i in range(
                        0, len(shp_xy))], facecolor=fill_color, linestyle="solid", linewidth=line_width, edgecolor=line_color))
        ...
        if self.count() == 0 and cmap.N == 1 and ticks[0] in [">", "<"] and ticks[-1] in [">", "<"]:
            ...
            ticks = ["nodata", "nodata"]
        cb = matplotlib.colorbar.ColorbarBase(ax_cb, cmap=cmap, norm=norm, boundaries=leg.getBoundaries(
        ), extend="neither", ticks=leg.getBoundaries())
        cb.set_ticklabels(ticks)
        setp(cb.ax.get_ymajorticklabels(), fontname="Arial",
             fontsize=10, fontweight="normal", color=(0, 0, 0, 1))
    ...
    def int(self):
        ...
        dt = _get_min_dtype([int(a.arr.min()), int(
            a.arr.max()), int(a.nodata()), int(a.arr.fill_value)])
    ...
    def float(self):
        ...
        a = self.copy()
        mask, nodata = a.arr.mask, a.nodata()
        dt = _get_min_dtype([float(a.arr.min()), float(
            a.arr.max()), float(a.nodata()), float(a.arr.fill_value)])
        if dt == float16:
            dt = float32
        a.arr = ma.array(a.arr, dtype=dt)
    ...
    def slice(self, l_rowcol):
        ...
        if type(self.dy()) == float:
            ...
            if self.proj() == 1:
                ...
                yll = self.yll()-(self.dy()[iRow_max:]).sum()
        gi = [xll, yll, dx, dy, arr.shape[0],
              arr.shape[1], self.proj(), self.ang()]
    ...
    def toGDALobj(self):
        ...
        rot1, rot2 = 0, 0
        gd = osgeo.gdal.GetDriverByName("MEM").Create("", self.ncol(
        ), self.nrow(), 1, l_data_type[1][l_data_type[2].index(self.arr.dtype)])
        if self.proj() == 0:
            gd.SetGeoTransform(
                [self.xll(), self.dx(), rot1, self.yur(), rot2, self.dy()])
        else:
            gd.SetGeoTransform(
                [self.xll(), self.dx(), rot1, self.yur(), rot2, -self.dy()])
...
def get_gi(rA, *args):
    ...
    if type(rA) in [tuple, list, dict]:
        ...
        if type(rA) in [tuple, list]:
            for key in args:
                try:
                    l_result.append(rA[l_gi_key.index(key)])
                except:
                    pass
        else:
            for key in args:
                try:
                    l_result.append(rA[key])
                except:
                    pass
        return l_result
    else:
        return rA.get_gi(*args)
...
def gi_extended(rA, as_list=False):
    ...
    if type(rA) in [tuple, list, dict]:
        ...
        result = rA.get_gi("xur", "yur", "Dx", "Dy")
    if as_list:
        return result
    else:
        return {"xur": result[0], "yur": result[1], "Dx": result[2], "Dy": result[3]}
...
def gi2dict(gi=None, **kwargs):
    ...
    if type(gi) in [tuple, list, dict]:
        if type(gi) in [tuple, list]:
            for i in range(0, min(len(l_gi_key), len(gi))):
                gi_new[l_gi_key[i]] = gi[i]
        else:
            for key in l_gi_key:
                try:
                    gi_new[key] = gi[key]
    ...
    for key in kwargs:
        try:
            gi_new[key] = kwargs[key]
        except:
            pass
    for i in range(0, len(l_gi_key)):
        if gi_new[l_gi_key[i]] != None:
            try:
                exec("gi_new[l_gi_key[i]]=%s" % (string.replace(
                    l_gi_type[i], "()", "(gi_new[l_gi_key[i]])")))
            except:
                pass
    return gi_new
...
def gi2list(gi=None, **kwargs):
    ...
    if type(gi) in [tuple, list, dict]:
        if type(gi) in [tuple, list]:
            ...
            for i in range(0, len(l_gi_key)):
                try:
                    gi_new[i] = gi[l_gi_key[i]]
                except:
                    pass
    for key in kwargs:
        try:
            gi_new[l_gi_key.index(key)] = kwargs[key]
        except:
            pass
    for i in range(0, len(l_gi_key)):
        try:
            exec("gi_new[i]=%s" %
                 (string.replace(l_gi_type[i], "()", "(gi_new[i])")))
        except:
            pass
    return gi_new
...
def gi_get_Dx(gi):
    ...
    if type(gi1["dx"]) not in [int, float]:
        try:
            return float(sum(gi1["dx"]))
        except:
            return None
    else:
        try:
            return float(gi1["dx"])*gi1["ncol"]
        except:
            return None
...
def gi_get_Dy(gi):
    ...
    if type(gi1["dy"]) not in [int, float]:
        try:
            return float(sum(gi1["dy"]))
        except:
            return None
    else:
        try:
            return float(gi1["dy"])*gi1["nrow"]
        except:
            return None
...
def set_gi(rA, **kwargs):
    ...
    if type(rA) in [tuple, list, dict]:
        if type(rA) in [tuple, list]:
            return gi2list(rA, **kwargs)
        else:
            return gi2dict(rA, **kwargs)
    else:
        rA.set_gi(**kwargs)
        return rA.gi()
...
def gi_set_extent(gi, l_extent, snap=False):
    ...
    try:
        ...
        proj = 1
    xll, xur = float(min(l_extent[0], l_extent[2])), float(
        max(l_extent[0], l_extent[2]))
    if proj == 0:
        yll, yur = float(max(l_extent[1], l_extent[3])), float(
            min(l_extent[1], l_extent[3]))
    else:
        yll, yur = float(min(l_extent[1], l_extent[3])), float(
            max(l_extent[1], l_extent[3]))
    if snap:
        xll = gi_new["xll"] + \
            _roundoff((xll-gi_new["xll"])/gi_new["dx"])*gi_new["dx"]
        xur = gi_get_xur(
            gi_new)+_roundoff((xur-gi_get_xur(gi_new))/gi_new["dx"])*gi_new["dx"]
        if proj == 0:
            yll = gi_new["yll"] - \
                _roundoff((yll-gi_new["yll"])/gi_new["dx"])*gi_new["dx"]
            yur = gi_get_yur(
                gi_new)-_roundoff((yur-gi_get_yur(gi_new))/gi_new["dx"])*gi_new["dx"]
        else:
            yll = gi_new["yll"] + \
                _roundoff((yll-gi_new["yll"])/gi_new["dx"])*gi_new["dx"]
            yur = gi_get_yur(
...
def _raster2info(f_raster, f_hdr=None):
    ...
    """
    # 0=INVALID
    raster_info, gi = [l_raster_format_str[0],
    ...
    if raster_info[0] == l_raster_format_str[0]:
        try:
            ...
            #if magic != 1271: raise
            ncol, nrow, xll, xur, yll, yur, minval, maxval, nodata, ieq, itb, x1, x2 = struct.unpack(
                "=2L 7f 4B", inf.read(40))
            if (os.path.getsize(f_raster)-44+bool(ieq-1)*8+bool(itb)*8+bool(ieq)*(nrow+ncol)*4-ncol*nrow*4) < 0:
                raise
            if ieq == 0:
                ...
                if itb == 1:
                    inf.read(8)
                dx, dy = np.reshape(np.fromstring(inf.read(
                    ncol*4), float32), (ncol,)), np.reshape(np.fromstring(inf.read(nrow*4), float32), (nrow,))
                Dx, Dy = dx.sum(), dy.sum()
                if dx.min() == dx.max():
                    dx = float(dx[0])
                if dy.min() == dy.max():
                    dy = float(dy[0])
            inf.close()
            if not ((abs(abs(xll-xur)-Dx) < 1e-4*ncol) and (abs(abs(yll-yur)-Dy) < 1e-4*nrow)):
                raise
            raster_info = [l_raster_format_str[2], 44+bool(ieq-1)*8+bool(itb)*8+bool(
                ieq)*(nrow+ncol)*4, 4, float32, nodata, 1, ["scalar", minval, maxval]]
    ...
    if raster_info[0] == l_raster_format_str[0]:
        try:
            ...
            if magic != "RUU CROSS SYSTEM MAP FORMAT":
                raise
            xxx, version, gisFileId, projection, attrTable, dataType, byteOrder, xxx, valueScale, cellRepr = struct.unpack(
                "=5s HLHLHL 14s 2H", inf.read(41))
            data_type, data_bytesize, structFormat, xxx, nodata = l_pcr_Struct[
                l_pcr_Struct["cellRepr"] == cellRepr][0]
            data_type = np.dtype(data_type)
            exec("nodata=%s" % (nodata))
            minVal, xxx, maxVal, xxx = struct.unpack(
                "=%s" % ("%s%ds" % (structFormat, 8-data_bytesize)*2), inf.read(16))
            xUL, yUL, nrRows, nrCols, cellSizeX, cellSizeY, angle = struct.unpack(
                "=ddLLddd", inf.read(48))
            inf.close()
            frm_map = l_pcr_valueScale["frm_map"][l_pcr_valueScale["valueScale"]
                                                  == valueScale][0]
            ...
            if proj == 0:
                ...
                yll = yUL-nrow*dy
            raster_info = [l_raster_format_str[3], 256, data_bytesize,
                           data_type, nodata, 1, [frm_map, minVal, maxVal]]
            crs = prj2crs("%s.prj" % (os.path.splitext(f_raster)[0]))
    ...
    if raster_info[0] == l_raster_format_str[0]:
        try:
            ...
            xll = float(x_rec[-1])
            if string.find(x_rec[0], "cent") != -1:
                xll -= dxy/2
            yll = float(y_rec[-1])
            if string.find(y_rec[0], "cent") != -1:
                yll -= dxy/2
            header_linesize = 5
            nodata = None
            ...
            if len(rec) >= 2:
                if string.find(rec[0], "nodata") != -1:
                    nodata = float(rec[-1])
                    if abs(nodata-int(nodata)) < 1e-7:
                        nodata = int(nodata)
                    header_linesize += 1
            inf.close()
            raster_info = [l_raster_format_str[1],
                           header_linesize, None, None, nodata, 1, None]
            crs = prj2crs("%s.prj" % (os.path.splitext(f_raster)[0]))
    ...
    if raster_info[0] == l_raster_format_str[0]:
        try:
            if f_hdr == None:
                f_hdr = "%s.hdr" % (os.path.splitext(f_raster)[0])
            inf = open(f_hdr, "r")
            recs = [string.split(string.strip(string.lower(line)))
            ...
            for rec in recs:
                if rec[0] == "nblocks":
                    ...
                    nbit = int(rec[1])/8
                    if nbit == 0:
                        nbit, data_type = 4, float32
                    else:
                        exec("data_type=int%d" % (nbit*8))
            if os.path.getsize(f_raster) != nlay*nrow*ncol*nbit:
                raise
            ...
            for rec in recs:
                if rec[0] == "ulxmap":
                    ...
                    nodata = float(rec[1])
            if xll == None or yur == None or dx == None or dy == None:
                raise
            xll = xll-0.5*dx
            yll = yur-nrow*dy+0.5*dy
            raster_info = [l_raster_format_str[5], 0,
                           nbit, data_type, nodata, nlay, None]
            crs = prj2crs("%s.prj" % (os.path.splitext(f_raster)[0]))
            gi = gi2dict([xll, yll, dx, dy, nrow, ncol, 1, 0.0, crs])
            return raster_info, gi
        except:
    ...
    if raster_info[0] == l_raster_format_str[0]:
        try:
            ...
            if crs == "" and raster_format == "netCDF":
                try:
                    crs = Dataset(f_raster).spatial_ref
                except:
                    pass
            crs = crs2epsg(crs, crs)
            if crs == "":
            ...
            for l in range(0, nlay):
                ...
                i_data_type = l_data_type[0].index(
                    osgeo.gdal.GetDataTypeName(layer.DataType))
                data_type += [l_data_type[2][i_data_type]]
                nodata += [layer.GetNoDataValue()]
            del gd, layer
            if nlay == 1:
                data_type, nodata = data_type[0], nodata[0]
            else:
                dt = data_type[0]
                for l in range(1, nlay):
                    dt = np.promote_types(dt, data_type[l])
                data_type = dt
                if min(nodata) == max(nodata):
                    nodata = nodata[0]
            data_bytesize = np.ones((1,), data_type).itemsize
            raster_info = [raster_format, None,
                           data_bytesize, data_type, nodata, nlay, None]
            gi = gi2dict([xll, yll, dx, dy, nrow, ncol, 1, 0.0, crs])
            return raster_info, gi
        except:
            try:
    ...
    # >> return: [raster_format, header_size, data_bytesize, data_type, nodata, nlay, [frm_map,minval,maxval]], geo_info
    return raster_info, gi
...
def _raster_format2ext(raster_format):
    ...
    if type(raster_format) == str:
        try:
            raster_format = l_raster_format_num[l_raster_format_str.index(
                raster_format)]
        except:
            raster_format = 0
    if raster_format not in l_raster_format_num:
        raster_format = 0
...
def _ext2raster_format(ext):
    ...
    if ext not in l_raster_format_ext:
        ...
        return l_raster_format_str[l_raster_format_ext.index(ext)]
...
def raster2arr(f_raster, nodata=None, gi=None, method="sample", step_xy=1, dtype_asc=None, f_hdr=None):
    ...
    :ref:`rescaling_resampling`
    """
    if not os.path.isfile(f_raster) and not os.path.isdir(f_raster):
        raise Exception("Raster does not exist (%s)" % (f_raster))
    ...
    elif raster_info[0] in [1, l_raster_format_str[1]]:
        rA = _asc2arr(f_raster, nodata, raster_info,
                      gi0, dtype_asc, gi, step_xy)
    elif raster_info[0] in [2, l_raster_format_str[2]]:
        rA = _idf2arr(f_raster, nodata, raster_info, gi0, gi, step_xy)
    elif raster_info[0] in [3, l_raster_format_str[3]]:
        rA = _map2arr(f_raster, nodata, raster_info, gi0, gi, step_xy)
    elif raster_info[0] in [4, 6, l_raster_format_str[4], l_raster_format_str[6]]+l_raster_format_num[7:]+l_raster_format_str[7:]:
        rA = _gdal2arr(f_raster, nodata, raster_info, gi0, gi, step_xy)
    elif raster_info[0] in [5, l_raster_format_str[5]]:
        rA = _bil2arr(f_raster, nodata, raster_info, gi0, f_hdr, gi, step_xy)
    if gi != None and method != None:
        try:
            rA = rescale_rA(rA, gi2dict(gi), method)
        except:
            pass
...
def rasters2arr(fl_raster, nodata=None, gi=None, method="sample", step_xy=1, dtype_asc=None, fl_hdr=None):
    ...
    if type(fl_raster) not in [list, tuple]:
        ...
        if type(fl_hdr) not in [list, tuple]:
            fl_hdr = [fl_hdr]*len(fl_raster)
...
def _asc2arr(f_raster, nodata=None, raster_info=None, gi0=None, dtype=None, crop_gi=None, step_xy=1):
    ...
    """
    try:
        if raster_info == None or gi0 == None:
            raster_info, gi0 = _raster2info(f_raster)
        if raster_info[0] not in [1, l_raster_format_str[1]]:
            raise Exception("Raster is not a valid asc file (%s)" % (f_raster))
        gi0 = gi2dict(gi0)
        nrow, ncol, nodata0 = gi0["nrow"], gi0["ncol"], raster_info[4]
        if dtype == None:
            dtype = float64
        inf = open(f_raster, "r")
        for i in range(0, raster_info[1]):
            inf.readline()
        # check cropping
        dr1, dr2, dc1, dc2 = 0, 0, 0, 0
        if crop_gi != None:
            [dr1, dr2], [dc1, dc2] = _calc_drc(gi0, crop_gi)
            dr1, dr2, dc1, dc2 = max(0, dr1), min(
                0, dr2), max(0, dc1), min(0, dc2)
            gi0["nrow"], gi0["ncol"] = nrow-dr1+dr2, ncol-dc1+dc2
            gi0["xll"] = gi0["xll"]+gi0["dx"]*dc1
            if gi0["proj"] == 0:
                gi0["yll"] = gi0["yll"]-gi0["dy"]*-dr2
            else:
                gi0["yll"] = gi0["yll"]+gi0["dy"]*-dr2
        ...
        # check resampling
        try:
            if type(step_xy) not in [list, tuple]:
                step_xy = [step_xy, step_xy]
            fct_x, fct_y = int(step_xy[0]), int(step_xy[1])
            if fct_x <= 1 and fct_y <= 1:
                raise
            ix, iy = np.arange(0, gi0["ncol"], fct_x)+((gi0["ncol"]-1) % fct_x)/2, gi0["nrow"] - \
                1-np.arange(0, gi0["nrow"], fct_y)[::-1] - \
                ((gi0["nrow"]-1) % fct_y)/2
        ...
        if nodata != None:
            ...
            # arr=np.where(arr == nodata0,nodata,arr)
...
def _idf2arr(f_raster, nodata=None, raster_info=None, gi0=None, crop_gi=None, step_xy=1):
    ...
    try:
        if raster_info == None or gi0 == None:
            raster_info, gi0 = _raster2info(f_raster)
        if raster_info[0] not in [2, l_raster_format_str[2]]:
            raise Exception("Raster is not a valid idf file (%s)" % (f_raster))
        gi0 = gi2dict(gi0)
        nrow, ncol, nodata0 = gi0["nrow"], gi0["ncol"], raster_info[4]
        # check cropping
        dr1, dr2, dc1, dc2 = 0, 0, 0, 0
        ...
        if resam:
            ...
            for R in range(0, nrow-dr1+dr2):
                if R in iy:
                    inf.read(dc1*raster_info[2])
                    arr[r] = np.fromstring(
                        inf.read(ncol2*raster_info[2]), raster_info[3])[ix]
                    inf.read(-dc2*raster_info[2])
                    r += 1
                else:
                    inf.read(ncol*raster_info[2])
          # no resampling
        else:
            for r in range(0, gi0["nrow"]):
                inf.read(dc1*raster_info[2])
                arr[r] = np.fromstring(
                    inf.read(gi0["ncol"]*raster_info[2]), raster_info[3])
                inf.read(-dc2*raster_info[2])
        inf.close()
        if nodata != None:
            # arr=np.where(arr == nodata0,nodata,arr)
...
def _map2arr(f_raster, nodata=None, raster_info=None, gi0=None, crop_gi=None, step_xy=1):
    ...
    try:
        if raster_info == None or gi0 == None:
            raster_info, gi0 = _raster2info(f_raster)
        if raster_info[0] not in [3, l_raster_format_str[3]]:
            raise Exception(
                "Raster is not a valid pcraster map file (%s)" % (f_raster))
        gi0 = gi2dict(gi0)
        nrow, ncol, nodata0 = gi0["nrow"], gi0["ncol"], raster_info[4]
        # check cropping
        dr1, dr2, dc1, dc2 = 0, 0, 0, 0
...
def _gdal2arr(f_raster, nodata=None, raster_info=None, gi0=None, crop_gi=None, step_xy=1):
    ...
    try:
        ...
        gi0 = gi2dict(gi0)
        nlay, nrow, ncol, nodata0, dt = raster_info[5], gi0[
            "nrow"], gi0["ncol"], raster_info[4], raster_info[3]
        # check cropping
        dr1, dr2, dc1, dc2 = 0, 0, 0, 0
        if crop_gi != None:
            [[dr1, dr2], [dc1, dc2]] = _calc_drc(gi0, crop_gi)
        ...
        for l in range(0, nlay):
            ...
            if crop:
                ...
                for r in range(dr1, nrow+dr2):
                    # x-offset,y-offset,x-size,y-size,x-buf(=x-size),y-buf(=y-size)
                    data = layer.ReadRaster(
                        dc1, r, ncol-dc1+dc2, 1, ncol-dc1+dc2, 1, l_data_type[1][i_data_type])
                    arr0[r-dr1] = np.array(struct.unpack(
                        "%d%s" % (ncol-dc1+dc2, l_data_type[3][i_data_type]), data), dtype=dt)
            else:
                data = layer.ReadRaster(dc1, dr1, ncol-dc1+dc2, nrow-dr1+dr2,
                                        ncol-dc1+dc2, nrow-dr1+dr2, l_data_type[1][i_data_type])
                arr0 = np.reshape(np.array(struct.unpack("%d%s" % (
                    (ncol-dc1+dc2)*(nrow-dr1+dr2), l_data_type[3][i_data_type]), data), dtype=dt), (nrow-dr1+dr2, ncol-dc1+dc2))
            if resam:
        ...
        for l in range(0, nlay):
            if nodata2[l] != None:
                # arr[l]=np.where(arr[l] == nodata1[l],nodata0,arr[l])
                arr[l][arr[l] == nodata1[l]] = nodata0
        if nodata != None:
            arr = np.array(arr, _get_min_dtype(arr, nodata))
            # arr=np.where(arr == nodata0,nodata,arr)
            arr[arr == nodata0] = nodata
...
def _bil2arr(f_raster, nodata=None, raster_info=None, gi0=None, f_hdr=None, crop_gi=None, step_xy=1):
    ...
    try:
        if raster_info == None or gi0 == None:
            raster_info, gi0 = _raster2info(f_raster, f_hdr)
        if raster_info[0] not in [5, l_raster_format_str[5]]:
            raise Exception("Raster is not a valid bil file (%s)" % (f_raster))
        gi0 = gi2dict(gi0)
...
def resample_rA(rA, step_xy=[1, 1]):
    ...
    try:
        step_is_gi = False
        if type(step_xy) == dict:
            step_is_gi = True
        elif type(step_xy) in [list, tuple]:
            if len(step_xy) >= 6:
                step_xy, step_is_gi = gi2dict(deepcopy(step_xy)), True
        else:
            step_xy = [step_xy, step_xy]
        if step_is_gi:
            gi = deepcopy(step_xy)
            x = rc2xy(np.append(np.zeros((1, gi["ncol"])), np.reshape(
                np.arange(0, gi["ncol"]), (1, gi["ncol"])), 0), gi)
            y = rc2xy(np.append(np.reshape(
                np.arange(0, gi["nrow"]), (1, gi["nrow"])), np.zeros((1, gi["nrow"])), 0), gi)
            r = xy2rc(y, rA.gi())[0]
            c = xy2rc(x, rA.gi())[1]
        else:
        ...
        if arr.mask.ndim == 0:
            arr.mask = np.ones(arr.shape, bool)*arr.mask
    ...
    if method != None:
        try:
            ...
            try:
                ...
                if rA.ieq() != 0:
                    raise Exception("Raster is non-equidistant: not supported")
                if type(to_gi["dx"]) not in [int, float] or type(to_gi["dy"]) not in [int, float]:
                    raise Exception(
                        "Target geo_info is non-equidistant: not supported")
                if None in rA.gi_list()[:6]:
                    raise Exception(
                        "Raster geo_info is not complete: %s" % (rA.gi()))
                if None in gi2list(to_gi)[:6]:
                    raise Exception(
                        "Target geo_info is not complete: %s" % (to_gi))
                if rA.proj() != to_gi["proj"] or rA.crs() != to_gi["crs"]:
                    mess = ""
                    if not same_crs(rA.crs(), to_gi["crs"]):
                        crs1, crs2 = crs2epsg(
                            rA.crs(), ""), crs2epsg(to_gi["crs"], "")
                        if not (crs1 == "" or crs2 == ""):
                            mess += "Crs is different:\n  A: %s\n  B: %s\n" % (
                                crs2epsg(rA.crs(), rA.crs()), crs2epsg(to_gi["crs"], to_gi["crs"]))
                    if rA.proj() != to_gi["proj"]:
                        mess += "Y projection is different: A: %s, B: %s" % (
                            rA.proj(), to_gi["proj"])
                    if mess != "":
                ...
                if method not in ["sum", "min", "max", "harm", "log10", "log", "mean"]:
                    ...
                    if method == "sum":
                        if arr.dtype == float64:
                            ...
                            dt = float32
                        if dxy3[2] > 1:
                            arr = ma.array(arr/float(dxy3[2]), dtype=dt)
                        if dxy3[3] > 1:
                            arr = ma.array(arr/float(dxy3[3]), dtype=dt)
...
def _calc_drc(gi1, gi2):
    ...
    if proj1 == 1:
        dr = [int(_rounddown((yur1-yur2)/dy1)), (yll1-yll2)/dy1]
    else:
        dr = [int(_rounddown((yur2-yur1)/dy1)), (yll2-yll1)/dy1]
    if dr[1]-int(dr[1]) < 1e-7:
        dr[1] = int(_roundoff(dr[1]))
    else:
        dr[1] = int(_roundup(dr[1]))
    dc = [int(_rounddown((xll2-xll1)/dx1)), (xur2-xur1)/dx1]
    if dc[1]-int(dc[1]) < 1e-7:
        dc[1] = int(_roundoff(dc[1]))
    else:
        dc[1] = int(_roundup(dc[1]))
    return dr, dc
...
def _calc_dxy(gi1, gi2):
    ...
    if abs(xll1-xll2) > 1e-7:
        dx = _calc_gcd(dx, xll1-xll2)
    dy = _calc_gcd(dy1, dy2)
    if abs(yll1-yll2) > 1e-7:
        dy = _calc_gcd(dy, yll1-yll2)
    return dx, dy
...
def _calc_drc_dxy(gi1, gi2):
    ...
    if abs((xll1-xll2)/(max(dx1-dx2, min(dx1, dx2)))) < 1e-2:
        xll1, gi1[0] = xll2, xll2
    if abs((yll1-yll2)/(max(dy1-dy2, min(dy1, dy2)))) < 1e-2:
        yll1, gi1[1] = yll2, yll2

    fdx, fdy = int(_roundup(dx2/dx1)), int(_roundup(dy2/dy1))

    dr1, dc1 = _calc_drc(gi1, gi2)
    dr1[0], dr1[1], dc1[0], dc1[1] = max(
        dr1[0], -fdy), min(dr1[1], fdy), max(dc1[0], -fdx), min(dc1[1], fdx)
    dx3, dy3 = _calc_dxy(gi1, gi2)

    xll3, xur3 = xll1+dc1[0]*dx1, xur1+dc1[1]*dx1
    if proj1 == 1:
        yll3, yur3 = yll1-dr1[1]*dy1, yur1-dr1[0]*dy1
    else:
        yll3, yur3 = yll1+dr1[1]*dy1, yur1+dr1[0]*dy1
    gi3 = [xll3, yll3, dx3, dy3, int(
        _roundoff(abs(yur3-yll3)/dy3)), int(_roundoff((xur3-xll3)/dx3)), proj1, ang1]
...
def _reform_arr(arr, drc, nodata=None):
    ...
    try:
        arr, gi, nodata, is_rA = deepcopy(arr.arr), deepcopy(
            arr.gi()), deepcopy(arr.nodata()), True
    except:
        arr = deepcopy(arr)
    try:
        arr, nodata, is_ma = ma.filled(arr.copy()), arr.fill_value, True
    except:
        if type(nodata) not in [bool, int, float]:
            nodata = _get_prefered_nodata(arr)
            arr = np.array(arr, _get_min_dtype(arr, nodata))
    nodata = _value_dtype2type(nodata)
    if drc[0][0] > 0:
        arr = arr[drc[0][0]:]
        ...
        arr = np.concatenate(
            [np.ones((-drc[0][0], arr.shape[-1]), arr.dtype)*nodata, arr], -2)
    if drc[0][1] < 0:
        arr = arr[:drc[0][1]]
    elif drc[0][1] > 0:
        arr = np.concatenate(
            [arr, np.ones((drc[0][1], arr.shape[-1]), arr.dtype)*nodata], -2)
    if drc[1][0] > 0:
        arr = arr[:, drc[1][0]:]
    elif drc[1][0] < 0:
        arr = np.concatenate(
            [np.ones((arr.shape[-2], -drc[1][0]), arr.dtype)*nodata, arr], -1)
    if drc[1][1] < 0:
        arr = arr[:, :drc[1][1]]
    elif drc[1][1] > 0:
        arr = np.concatenate(
            [arr, np.ones((arr.shape[-2], drc[1][1]), arr.dtype)*nodata], -1)
    if is_rA:
...
def _split_cells(arr, n_xsplit=1, n_ysplit=1):
    ...
    if n_xsplit > 1 or n_ysplit > 1:
        ...
        if n_xsplit > 1:
            if is_ma:
                arr = ma.repeat(arr, n_xsplit, -1)
            else:
                arr = np.repeat(arr, n_xsplit, -1)
        if n_ysplit > 1:
            if is_ma:
                arr = ma.repeat(arr, n_ysplit, -2)
            else:
                arr = np.repeat(arr, n_ysplit, -2)
        if is_rA:
...
def _merge_cells(arr, n_xmerge=1, n_ymerge=1, method="mean", nodata=None):
    ...
    if n_xmerge > 1 or n_ymerge > 1:
        if arr.dtype == float64:
            ...
            dt = float32
        try:
            nodata, is_ma = arr.fill_value, True
        except:
            if type(nodata) not in [bool, int, float]:
                nodata = _get_prefered_nodata(arr)
                arr = np.array(arr, _get_min_dtype(arr, nodata))
        ...
        nodata = _value_dtype2type(nodata)
        arr = ma.reshape(ma.swapaxes(ma.reshape(arr, arr.shape[:-2]+(arr.shape[-2]/n_ymerge, n_ymerge, arr.shape[-1])), -2, -1), arr.shape[:-3]+(
            arr.shape[-2]/n_ymerge, arr.shape[-1]/n_xmerge, n_ymerge*n_xmerge))
        if method == "sum":
            dt = arr.dtype
            arr = ma.array(arr.sum(-1), fill_value=nodata)
            arr = ma.array(arr, dtype=_get_min_dtype(
                np.ones((1,), dt), arr.min(), arr.max(), arr.fill_value))
        elif method == "min":
            arr = ma.array(arr.min(-1), arr.dtype, fill_value=nodata)
        elif method == "max":
            arr = ma.array(arr.max(-1), arr.dtype, fill_value=nodata)
        elif method == "harm":
            ...
            mask = np.logical_or(arr.mask, arr.data <= 0)
            arr.data[mask] = 1
            arr = 10**ma.array(ma.array(np.log10(arr.data),
                                        mask=mask, dtype=dt).mean(-1), dtype=dt)
            arr.data[arr.mask], arr.fill_value = nodata, nodata
        elif method == "log":
            mask = np.logical_or(arr.mask, arr.data <= 0)
            arr.data[mask] = 1
            arr = ma.exp(ma.array(ma.array(np.log(arr.data),
                                           mask=mask, dtype=dt).mean(-1), dtype=dt))
            arr.data[arr.mask], arr.fill_value = nodata, nodata
        else:
            arr = ma.array(arr.mean(-1), dt, fill_value=nodata)
        if not is_ma:
            arr = ma.filled(arr)
        if is_rA:
...
def rA_nonequi2equi(rA, dx=None, dy=None):
    ...
    if rA.ieq() == 0:
        ...
        gi2 = [rA.xll(), rA.yll(), dx, dy, int(rA.Dy()/dy+0.01*dy),
               int(rA.Dx()/dx+0.01*dx), rA.proj(), rA.ang(), rA.crs()]
        gi2 = gi2dict(gi2)
        ...
        if rA.gi()["proj"] == 1:
            yi = np.array(
                np.interp(y2, y1[::-1], np.arange(0, len(y1))[::-1], -1, -1), int32)
        else:
            yi = np.array(
                np.interp(y2, y1, np.arange(0, len(y1)), -1, -1), int32)
...
def same_crs(crs1, crs2):
    ...
    :ref:`projection`
    """
    c1 = osgeo.osr.SpatialReference()
    c1.ImportFromWkt(crs2wkt(crs1))
    c2 = osgeo.osr.SpatialReference()
    c2.ImportFromWkt(crs2wkt(crs2))
    if c1.IsSame(c2) == 1:
        return True
    else:
        return False
...
def reproject_rA(rA, to_crs, from_crs=None, method="sample", xyur=None):
    ...
    lut_method = [['sample', 'near', 'bilinear', 'cubic', 'cubicspline', 'lanczos', 'mean',   'average', 'mode', None],
                  ['near',  'near', 'bilinear', 'cubic', 'cubicspline',
                      'lanczos', 'average', 'average', 'mode', None],
                  [0,       0,     1,         2,      3,             4,       5,        5,        6,     0]]
    if method in lut_method[0]:
        method = lut_method[1][lut_method[0].index(method)]
    else:
        raise Exception(
            "ERROR: method '%s' not possible with reproject_rA." % (method))
        method = 'near'
    if method == None:
        try:
            to_gi["dx"] = None
    ...
    try:
        ...
        gd1 = osgeo.gdal.GetDriverByName("MEM").Create(
            "", rA.ncol(), rA.nrow(), 1, l_data_type[1][l_data_type[2].index(arr.dtype)])
        rot1, rot2 = 0, 0
        if rA.proj() == 0:
            gd1.SetGeoTransform(
                [rA.xll(), rA.dx(), rot1, rA.yur(), rot2, rA.dy()])
        else:
            gd1.SetGeoTransform(
                [rA.xll(), rA.dx(), rot1, rA.yur(), rot2, -rA.dy()])
        ...
        if to_gi == None:
            ...
            gd2 = osgeo.gdal.GetDriverByName("MEM").Create(
                "", to_gi["ncol"], to_gi["nrow"], 1, l_data_type[1][l_data_type[2].index(arr.dtype)])
            if to_gi["proj"] == 0:
                gd2.SetGeoTransform(
                    [to_gi["xll"], to_gi["dx"], rot1, gi_get_yur(to_gi), rot2, to_gi["dy"]])
            else:
                gd2.SetGeoTransform(
                    [to_gi["xll"], to_gi["dx"], rot1, gi_get_yur(to_gi), rot2, -to_gi["dy"]])
        ...
        i_data_type = l_data_type[0].index(
            osgeo.gdal.GetDataTypeName(layer.DataType))
        data = layer.ReadRaster(0, 0, ncol, nrow, ncol,
                                nrow, l_data_type[1][i_data_type])
        arr = np.reshape(np.array(struct.unpack("%d%s" % (
            ncol*nrow, l_data_type[3][i_data_type]), data), dtype=l_data_type[2][i_data_type]), (nrow, ncol))
        nodata = layer.GetNoDataValue()
...
def warp_rA(rA, to_crs, from_crs=None, exe_warp=None, method="sample", screen_output=False, xyur=None):
    ...
    try:
        ...
        if method in lut_method[0]:
            ...
            raise Exception(
                "ERROR: method '%s' not possible with gdalwarp." % (method))
        if method == None:
            try:
                to_gi["dx"] = None
                to_gi["dy"] = None
        ...
        if from_crs != "" and to_crs != "":
            ...
            if epsg != "" and str(epsg)[:3].upper() != "UTM":
                ...
                outf = open(r"%s\out.prj" % (tmp), "w")
                outf.write(to_crs)
                outf.close()
                args += ["-t_srs", "out.prj"]

        args += ["-srcnodata", '"%s"' %
                 (rA.nodata()), "-dstnodata", '"%s"' % (rA.nodata()), "-r", method]

        try:
            if to_gi["xll"] != None and to_gi["yll"] != None and xur != None and yur != None:
                args += ["-te", to_gi["xll"],
                         min(to_gi["yll"], yur), xur, max(to_gi["yll"], yur)]
        except:
            pass
        try:
...
def if_then_else(if_statement, then_statement, else_statement=None):
    ...
    try:
        ...
        IF, THEN, ELSE = deepcopy(if_statement), deepcopy(
            then_statement), deepcopy(else_statement)
        try:
            to_gi, nodata = THEN.gi(), THEN.nodata()
        except:
            try:
                to_gi, nodata = ELSE.gi(), ELSE.nodata()
            except:
                try:
                    to_gi, nodata = IF.gi(), IF.nodata()
                except:
                    raise Exception(
                        "ERROR: IF, THEN and ELSE are all not a rasterArr object")
        dt = _get_min_dtype(THEN, ELSE, nodata)
        dt = int32

        try:
            IF.gi()
            IF = (rescale_rA(IF, to_gi, method=method).bool()).arr
        except:
            try:
                IF.dtype
                if IF.ndim == 0:
        ...
        try:
            THEN.gi()
            THEN = rescale_rA(THEN, to_gi, method=method).arr
        except:
            try:
                THEN.dtype
                if THEN.ndim == 0:
                ...
                THEN = ma.array(
                    THEN*ma.ones((to_gi["nrow"], to_gi["ncol"]), dtype=dt))
        if THEN.mask.ndim == 0:
        ...
        if type(ELSE) != type(None):
            try:
                ELSE.gi()
                ELSE = rescale_rA(ELSE, to_gi, method=method).arr
            except:
                try:
                    ELSE.dtype
                    if ELSE.ndim == 0:
                    ...
                    ELSE = ma.array(
                        ELSE*ma.ones((to_gi["nrow"], to_gi["ncol"]), dtype=dt))
            if ELSE.mask.ndim == 0:
...
def rc_arr2val(rc, rA):
    ...
    result : numpy array (MaskedArray)
    """
    arr_nodata = np.maximum(np.maximum(
        rc[0] >= rA.nrow(), rc[0] < 0), np.maximum(rc[1] >= rA.ncol(), rc[1] < 0))
...
def _idf2mm(f, access="r"):
    ...
    if raster_info[4] == None:
        try:
            raster_info[4] = raster_info[6][1]-1
        except:
            raster_info[4] = -999999
...
# xy: 2xN matrix; l_mm: {raster_info,geo_info,[inf,mm]}
def _xy_mm2val(xy, l_mm, nodata=None, interpolate=False):
...
# rc: 2xN matrix; l_mm: {raster_info,geo_info,[inf,mm]}
def _rc_mm2val(rc, l_mm, nodata=None, consecutive=False):
    """Function to get values from raster info and mmap instance using row,col indices.
    """
    frm = "=%s" % (l_pcr_Struct["structFormat"][[np.dtype(d)
                                                 for d in l_pcr_Struct["data_type"]].index(l_mm[0][3])])
    ...
    rc = np.array(rc)
    if consecutive:
        l_mm[2][1].seek(l_mm[0][1]+l_mm[0][2] *
                        (rc[0][0]*l_mm[1]["ncol"]+rc[1][0]))
        arr = np.fromstring(l_mm[2][1].read(
            len(rc[0])*l_mm[0][2]), dtype=l_mm[0][3])
        arr[arr == nodata0] = nodata
        try:
            arr[np.isnan(arr)] = nodata
        except:
            arr[arr == l_mm[0][4]] = nodata
    else:
        # cp=np.where(rc[0] >= 0,np.where(rc[0] < l_mm[1]["nrow"],np.where(rc[1] >= 0,np.where(rc[1] < l_mm[1]["ncol"],True,False),False),False),False)
        cp = (rc[0] >= 0)*(rc[0] < l_mm[1]["nrow"]) * \
            (rc[1] >= 0)*(rc[1] < l_mm[1]["ncol"])
        # ind=l_mm[0][1]+l_mm[0][2]*(np.where(cp,rc[0].copy(),0)*l_mm[1]["ncol"]+np.where(cp,rc[1].copy(),0))
        ind = l_mm[0][1]+l_mm[0][2]*(cp*rc[0]*l_mm[1]["ncol"]+cp*rc[1])
        ...
        for i in range(0, len(arr)):
            ...
            if v != nodata0:
                arr[i] = v
        try:
            arr = np.where(cp, np.where(np.isnan(arr), nodata, arr), nodata)
        except:
            arr = np.where(cp, np.where(
                arr == l_mm[0][4], nodata, arr), nodata)
    return arr
...
def _rc_val2mm(rc, val, l_mm, consecutive=False):
    ...
    if consecutive:
        l_mm[2][1].seek(l_mm[0][1]+(rc[0][0]*l_mm[1]
                                    ["ncol"]+rc[1][0])*l_mm[0][2])
...
def idfs2stat(l_idf, stat="mean", ncel=100000, assume_same_mask=True, weights=None):
    ...
    if arr_tot.mask.ndim == 0:
        arr_tot.mask = np.ones(arr_tot.shape, bool)*arr_tot.mask
    ...
    if stat[0] == "p":
        ...
        if weights != None:
            ...
            if assume_same_mask:
                ...
                for i in range(0, len(ii)):
                    ...
                    for j in range(0, len(l_idf)):
                        ...
                        arr[j] = ma.masked_values(np.fromstring(
                            mm.read(dd[i]*raster_info[2]), raster_info[3]), l_nodata[j])
                        mm.close()
                        inf.close()
                    ...
                    for j in cpi1:
                        result += [_weighted_percentile(
                            arr[:, j], weights, [perc/100.])]
                    arr_tot.data[ii[i]:ii[i]+dd[i]
                                 ][cpi1] = np.array(result, raster_info[3])
                ...
                for i in range(0, len(ii)):
                    ...
                    for c in range(0, dd[i]):
                        try:
                            ...
                            arr_tot[ii[i]+c] = _weighted_percentile(np.compress(cp, np.ravel(
                                arr[:, c].data), 0), np.compress(cp, weights, 0), [perc/100.])
                        except:
            ...
            if assume_same_mask:
                ...
                for i in range(0, len(ii)):
                    ...
                    arr_tot.data[ii[i]:ii[i]+dd[i]][cpi1] = np.array(
                        np.percentile(np.compress(cp, arr, 1), perc, 0), raster_info[3])
                    arr_tot.data[ii[i]:ii[i]+dd[i]][cpi2] = arr_tot.fill_value
                    arr_tot.mask[ii[i]:ii[i]+dd[i]][cpi2] = True
                ...
                for i in range(0, len(ii)):
                    ...
                    for c in range(0, dd[i]):
                        try:
                            arr_tot[ii[i]+c] = np.percentile(np.compress(
                                np.ravel(arr[:, c].mask == False), np.ravel(arr[:, c].data), 0), perc, 0)
                        except:
                            arr_tot.data[ii[i]+c] = arr_tot.fill_value
                            arr_tot.mask[ii[i]+c] = True
        ...
        for i in range(0, len(ii)):
            ...
            if stat == "sum":
                if weights != None:
                    arr_tot[ii[i]:ii[i]+dd[i]] = (
                        arr*((arr.mask == False)*np.reshape(weights, (len(weights), 1)))).sum(0)
                else:
                    arr_tot[ii[i]:ii[i]+dd[i]] = arr.sum(0)
            elif stat == "count":
                if weights != None:
                    arr_tot[ii[i]:ii[i]+dd[i]] = ((arr.mask == False)
                                                  * np.reshape(weights, (len(weights), 1))).sum(0)
                else:
                    arr_tot[ii[i]:ii[i]+dd[i]] = (arr.mask == False).sum(0)
            elif stat == "mean":
                arr_tot[ii[i]:ii[i]+dd[i]
                        ] = ma.average(arr, axis=0, weights=weights)
            elif stat == "std":
                arr_tot[ii[i]:ii[i]+dd[i]] = arr.std(0)
            elif stat == "var":
                arr_tot[ii[i]:ii[i]+dd[i]] = arr.var(0)
            elif stat == "min":
                arr_tot[ii[i]:ii[i]+dd[i]] = arr.min(0)
            elif stat == "max":
                arr_tot[ii[i]:ii[i]+dd[i]] = arr.max(0)
...
# ll=lower left: coordinates for lower left corner of cell instead of center
def rc2xy(rc, gi, ll=False, outsideVal=None):
...
def rc2rc(gi1, gi2, raveled=False, rc_splitted=False):
    ...
    cp = (rc2[0] >= 0)*(rc2[0] < gi2["nrow"]) * \
        (rc2[1] >= 0)*(rc2[1] < gi2["ncol"])
    rc2[0][cp == False] = -1
    rc2[1][cp == False] = -1
...
def masks2rc_overlap(rA1, rA2):
    ...
    """
    rA1_rc = xy2rc([[rA2.xll()+rA2.dx()*0.5, rA2.xur()-rA2.dx()*0.5],
                    [rA2.yur()-rA2.dy()*0.5, rA2.yll()+rA2.dy()*0.5]], gi=rA1.gi())
    rA2_rc = xy2rc([[rA1.xll()+rA1.dx()*0.5, rA1.xur()-rA1.dx()*0.5],
                    [rA1.yur()-rA1.dy()*0.5, rA1.yll()+rA1.dy()*0.5]], gi=rA2.gi())

    rA1_r1, rA1_r2 = max(0, rA1_rc[0].min()), min(
        rA1.nrow(), rA1_rc[0].max()+1)
    rA1_c1, rA1_c2 = max(0, rA1_rc[1].min()), min(
        rA1.ncol(), rA1_rc[1].max()+1)
    rA2_r1, rA2_r2 = max(0, rA2_rc[0].min()), min(
        rA2.nrow(), rA2_rc[0].max()+1)
    rA2_c1, rA2_c2 = max(0, rA2_rc[1].min()), min(
        rA2.ncol(), rA2_rc[1].max()+1)

    rA1_rc = np.indices((rA1_r2-rA1_r1, rA1_c2-rA1_c1)) + \
        np.array([[[rA1_r1]], [[rA1_c1]]])
    rA2_rc = np.indices((rA2_r2-rA2_r1, rA2_c2-rA2_c1)) + \
        np.array([[[rA2_r1]], [[rA2_c1]]])

    cp = np.ravel((rA1.mask()[rA1_r1:rA1_r2, rA1_c1:rA1_c2] == False)
                  * (rA2.mask()[rA2_r1:rA2_r2, rA2_c1:rA2_c2] == False))
    rc = np.array([[np.ravel(rA1_rc[0])[cp], np.ravel(rA1_rc[1])[cp]], [
                  np.ravel(rA2_rc[0])[cp], np.ravel(rA2_rc[1])[cp]]])
...
def window_arr(arr, multArr=None, dWin=None, method="mean", round=False, unitCell=True):
    ...
    l_method = [
        ["mean", "harm", "log", "log10", "min",
            "max", "sum", "std", "var", "count"],
    ...
    if method in ["mean", "harm", "log", "log10", "sum", "count"]:
        ...
        arr = ((window_arr(arr**2, multArr=multArr, method="sum") + n * mn **
                2 - 2 * mn * window_arr(arr, multArr=multArr, method="sum"))/n)**0.5
        if method == "var":
            arr = arr**2
        msk = arr.mask
        arr = arr.data
...
def _window_min(arr, multArr, fill_value=1e31):
    multArr, r1in, r2in, r1out, r2out, c1in, c2in, c1out, c2out = _multArr2indices(
        arr, multArr)
    arrWin = np.ones(arr.shape, arr.dtype)*fill_value
    for i in range(0, len(r1in)):
        arrWin[r1out[i]:r2out[i], c1out[i]:c2out[i]] =\
            np.minimum(arrWin[r1out[i]:r2out[i], c1out[i]:c2out[i]],
                       arr[r1in[i]:r2in[i], c1in[i]:c2in[i]]*multArr[i])
    return arrWin
...
def _window_max(arr, multArr, fill_value=-1e31):
    ...
    for i in range(0, len(r1in)):
        arrWin[r1out[i]:r2out[i], c1out[i]:c2out[i]] =\
            np.maximum(arrWin[r1out[i]:r2out[i], c1out[i]:c2out[i]],
                       arr[r1in[i]:r2in[i], c1in[i]:c2in[i]]*multArr[i])
    return arrWin
...
def _getMultArr(dWin, round=False, unitMult=1):
    if type(dWin) == str:
        ...
        if p == "circle_radius":
            ...
            multArr[multArr <= 0] = 1
            multArr[multArr > 1] = 0
            multArr[(multArr > 0)*(multArr < 1)] = 1 - \
                multArr[(multArr > 0)*(multArr < 1)]
...
def idfs2mdf(l_idf, f_mdf, rgb=None):
    ...
    for i in range(0, len(l_idf)):
        ...
        outf.write('"%s","%s",%d,9,0\n' % (l_idf[i], os.path.splitext(
            os.path.basename(l_idf[i]))[0], rgb[i][0]+rgb[i][1]*2**8+rgb[i][2]*2**16))
...
def rasterStack(l_rA, nodata=None, to_gi=None, method="sample"):
    ...
    try:
        ...
        if to_gi != None:
            ...
            to_gi = l_rA[0].gi()
    if type(nodata) not in [str, bool, int, float]:
        nodata = l_rA[0].nodata()
    l_rA = [rescale_rA(rA, to_gi, method) for rA in l_rA]
    for rA in l_rA:
        rA.set_nodata(nodata)
...
def put_rA(rA, rA_in, method="sample"):
    ...
    :ref:`indexing_slicing`
    """
    rc = xy2rc([[rA_in.xll(), rA_in.xur()], [
               rA_in.yur(), rA_in.yll()]], rA.gi())
    rc = [[max(0, rc[0][0]), min(rA.nrow()-1, rc[0][1])],
          [max(0, rc[1][0]), min(rA.ncol()-1, rc[1][1])]]
    r, c = rc[0][0], rc[1][0]
    xy = rc2xy(rc, rA.gi(), True)
    xll, xur, yur, yll = xy[0][0], xy[0][1], xy[1][0], xy[1][1]
    if xll > rA_in.xll()+rA.dx()*1e-4:
        xll -= rA.dx()
    elif xll < rA_in.xll()-rA.dx()*(1-1e-4):
        xll += rA.dx()
    if xur < rA_in.xur()-rA.dx()*1e-4:
        xur += rA.dx()
    elif xur > rA_in.xur()+rA.dx()*(1-1e-4) >= rA_in.xur():
        xur -= rA.dx()
    if yll > rA_in.yll()+rA.dy()*1e-4:
        yll -= rA.dy()
    elif yll < rA_in.yll()-rA.dy()*(1-1e-4):
        yll += rA.dy()
    if yur < rA_in.yur()-rA.dy()*1e-4:
        yur += rA.dy()
    elif yur > rA_in.yur()+rA.dy()*(1-1e-4):
        yur -= rA.dy()
    rc = xy2rc([[xll, xll+rA.dx()*1e-4], [yur, yur-rA.dy()*1e-4]], rA.gi())
    r, c = rc[0].max(), rc[1].max()
    nrow, ncol = int(_roundoff((yur-yll)/rA.dy())
                     ), int(_roundoff((xur-xll)/rA.dx()))
    gi = [xll, yll, rA.dx(), rA.dy(), nrow, ncol, rA.proj(), rA.ang()]
    arr_in = rA_in.rescale(gi, method)
    arr_out = rA.arr.copy()
    if arr_out.mask.ndim == 0:
        arr_out.mask = np.ones(arr_out.shape, bool)*arr_out.mask
...
def _get_min_dtype(l_val, *args):
    ...
    for v in l_val:
        try:
            ...
            if type(v) != type(None):
                dt = np.promote_types(
                    dt, np.min_scalar_type(_value_dtype2type(v)))
        except:
            pass
    return dt
...
def _value_dtype2type(v):
    ...
    """
    try:
        if v.dtype in [bool, bool8]:
            return bool(v)
        elif v.dtype in [float16, float32, float64]:
            return float(v)
        else:
            return int(v)
    except:
...
def _get_prefered_nodata(arr):
    ...
    try:
        arr.gi()
        arr1 = np.compress(np.ravel(arr.arr.mask) == False,
                           np.ravel(arr.arr.data), 0)
        nodata = arr.nodata()
    except:
        try:
            arr1 = np.compress(np.ravel(arr.mask) == False,
                               np.ravel(arr.data), 0)
            nodata = arr.fill_value
        except:
            arr1 = np.array(arr)
    ...
    if nodata == None or np.in1d(nodata, arr1)[0]:
        ...
        for i in range(0, len(l_prefered_nodata)):
            if l_prefered_nodata[i][0] == arr1.dtype:
                ...
                break
        if not fnd:
            if arr1.dtype in [bool, bool8]:
                nodata = False
            elif arr1.dtype in [uint8, uint16, uint32, uint64]:
                nodata = arr1.max()+1
            else:
                nodata = arr1.min()-1
    return nodata
...
def _weighted_percentile(data_1D, weights_1D, l_perc=[0.5]):
    ...
    S = np.arange(0, len(data_1D))*weights_1D+(len(data_1D)-1) * \
        np.insert(np.cumsum(weights_1D)[:-1], 0, [0])
    S = S/S[-1]
