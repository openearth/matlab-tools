...
# $Id: nhimodel_outputs.py 15858 2019-10-22 15:18:09Z pronk_mn $
# $Date: 2019-10-22 08:18:09 -0700 (Tue, 22 Oct 2019) $
# $Author: pronk_mn $
# $Revision: 15858 $
# $HeadURL: https://svn.oss.deltares.nl/repos/openearthtools/trunk/python/applications/wps/nhidataportaal/processes/nhimodel_outputs.py $
# $Keywords: $
...
class nhimodel_IO():
    ...
    def geoserverGroupLayers(self, name, layer, layeriso, sld_layer='nhiraster', sld_layeriso='nhisolines'):
        ...
        logging.info(
            '''GEOSERVER [groupLayersREST]: code={}'''.format(r.status_code))
    ...
    def geoserverUploadShp(self, zippath, tmpdir, sld_style='nhisolines'):
        ...
        auth = HTTPBasicAuth(
            self.config['GEOSERVER_USER'], self.config['GEOSERVER_PASS'])
        ...
        with open(zippath, 'rb') as f:
            ...
            logging.info(
                '''GEOSERVER [uploadREST]: code={}'''.format(r.status_code))
    ...
    def addSLD(self, layername, sld_style):
        cat = Catalog(self.config['GEOSERVER_HOST']+'/rest',
                      username=self.config['GEOSERVER_USER'], password=self.config['GEOSERVER_PASS'])
        ...
        try:
            ...
            logging.info(
                'ERROR while connecting to geoserver to change SLD styling')
            pass
    ...
    def insideLayer(self, xin, yin, layernbin, epsgin=28992):
        try:
            conn = psycopg2.connect("""dbname='{d}' user='{u}' host='{h}' password='{p}'""".format(
                d=self.config['POSTGIS_DB'], u=self.config['POSTGIS_USER'], h=self.config['POSTGIS_HOST'], p=self.config['POSTGIS_PASS']))
            cur = conn.cursor()
            sqlquery = """SELECT st_value(rast,st_setsrid(st_point({x},{y}),{epsg})) FROM ibound{layernb} WHERE ST_Intersects(rast,st_setsrid(st_point({x},{y}),{epsg}));""".format(
                x=xin, y=yin, epsg=epsgin, layernb=layernbin)
