...
class WCS:
    ...
    def __init__(self, host, layer, outdir, random, outfname, ext, fformat):
        self.id = layer
        ...
        self.resx, self.resy = (self.hx-self.lx) / \
            self.cx, (self.hy-self.ly)/self.cy
        self.width = self.cx
        self.height = self.cy
        self.outdir = outdir
        self.random = random
    ...
    def getw(self):
        ...
        gc = self.wcs.getCoverage(identifier=self.id,
                                  bbox=self.bbox,
                                  format='image/geotiff',  # self.fformat
                                  crs=self.crs,
                                  width=self.width,
                                  height=self.height)
        # random unique filename
        filename = self.outfname + '_' + self.random + '.tif'  # self.ext
...
class LS:
    ...
    def __init__(self, outfname, ext, fformat, outdir, random, awkt, crs, host, layer, sampling=1):
        self.wkt = awkt
        self.crs = crs
        # Initiates WCS service to get some parameters about the grid.
        self.gs = WCS(host, layer, outdir, random, outfname, ext, fformat)
    ...
    def line(self):
        ...
        self.x1, self.y1 = int(
            self.ax // self.gs.resx), int(self.ay // self.gs.resy)
        self.x2, self.y2 = int(self.bx // self.gs.resx) + \
            1, int(self.by // self.gs.resy)+1
        ...
        if flipx and flipy:  # who draws these lines?
           ...
           #logging.info("Both flipped")
            self.x2, self.y2 = int(
                self.bx // self.gs.resx), int(self.by // self.gs.resy)
            self.x1, self.y1 = int(self.ax // self.gs.resx) + \
                1, int(self.ay // self.gs.resy)+1
        elif flipx:
            # bottom right to top left
           #logging.info("X flipped")
            self.x2, self.y1 = int(
                self.bx // self.gs.resx), int(self.ay // self.gs.resy)
            self.x1, self.y2 = int(self.ax // self.gs.resx) + \
                1, int(self.by // self.gs.resy)+1
        elif flipy:
            # top left to bottom right
           #logging.info("Y flipped")
            self.x1, self.y2 = int(
                self.ax // self.gs.resx), int(self.by // self.gs.resy)
            self.x2, self.y1 = int(self.bx // self.gs.resx) + \
                1, int(self.ay // self.gs.resy)+1
        else:
            # normal
        ...
        self.xlist = np.linspace((self.ax/self.gs.resx)-min(self.x1, self.x2),
                                 (self.bx/self.gs.resx)-min(self.x1, self.x2), num=self.subdiv)
        self.ylist = np.linspace((self.ay/self.gs.resy)-min(self.y1, self.y2),
                                 (self.by/self.gs.resy)-min(self.y1, self.y2), num=self.subdiv)
    ...
    def json(self):
        ...
        self.xco = np.linspace(
            self.ax+self.gs.lx, self.bx+self.gs.hx, len(self.values))
        self.yco = np.linspace(
            self.ay+self.gs.ly, self.by+self.gs.hy, len(self.values))
        # note that this is not a real representation at ALL, just for testing.
        self.distances = list(map(float, np.linspace(
            0, len(self.values), len(self.values)) * (self.length / len(self.values))))
