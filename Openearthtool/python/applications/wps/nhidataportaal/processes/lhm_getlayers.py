...
class Process(WPSProcess):
    def __init__(self):
        ...
        WPSProcess.__init__(self,
                            identifier="lhm_getlayers",
                            title="Get available models and layers",
                            abstract="""Returns a json of available layers for all available models from a THREDDS location""",
                            version="1.0",
                            storeSupported=True,
        ...
        for url in urls:
            ...
            for d in lst:
                ...
                for i in ncs:
                    ...
                    s = i.split('/')
                    dictnc[s[len(s)-1]] = [s[len(s)-4], s[len(s)-3],
                                           s[len(s)-2], i, ds.variables['Band1'].long_name]
                    ds.close()
        logging.info('data collected from '+catalogurl)
        io = io.StringIO()
        json.dump(dictnc, io)
        io.seek(0)
