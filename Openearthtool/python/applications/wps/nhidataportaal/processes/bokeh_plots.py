...
# $Id: bokeh_plots.py 15858 2019-10-22 15:18:09Z pronk_mn $
...
class bokeh_Plot:
    ...
    def plot_Tseries(self, data, posx, posy, locid, output_html):
        # Data preparation
        nbs = []
        result = []
        ...
        for elem in data:
            ...
            if not value == None:
                nbs.append(n)
                result[n][0].append(datetime.strptime(
                    date.split(' ')[0], '%Y-%m-%d'))
                result[n][1].append(float(value))

        logging.info(' '.join([str(locid), str(posx), str(posy)]))
        tit = 'Tijdreeks voor locatie met ID = {} en positie [x,y] = [{},{}] meter'.format(
            locid, posx, posy)

        # Plot per column
        TOOLS = "pan,wheel_zoom,box_zoom,reset,save"
        p = figure(width=800, height=540, x_axis_type="datetime",
                   title=tit, tools=TOOLS)
        p.xaxis.axis_label = 'datum'
        p.yaxis.axis_label = 'grondwateronntrekking (?)'

        for n in np.unique(nbs):
            p.circle(x=result[n][0], y=result[n][1],
                     size=5, color=self.colorscheme[n])
            p.line(x=result[n][0], y=result[n][1], color=self.colorscheme[n], legend=' '.join(
                ['filter', str(n)]))
