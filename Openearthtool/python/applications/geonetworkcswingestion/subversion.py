...
"""
$Id: subversion.py 15429 2019-05-18 15:08:40Z jong_jn $
$Date: 2019-05-18 08:08:40 -0700 (Sat, 18 May 2019) $
$Author: jong_jn $
$Revision: 15429 $
$HeadURL: https://svn.oss.deltares.nl/repos/openearthtools/trunk/python/applications/geonetworkcswingestion/subversion.py $
"""
        ...
        if regex_url.search(target): # working on a remote
            if target.startswith(self.wc_root_url):
                ...
                if match:
                    ...
                    # define final target
                    target = Path(self.wc_root, relative_url)
                    # strip last part from relative url
                    relative_url = Path(relative_url).parent
                    dir_splits = relative_url.parts
                    # loop over intermediate paths
                    for i, d in enumerate(dir_splits):
                        p = Path(self.wc_root, *dir_splits[:(i + 1)])
                        ...
                        if not p.exists():
                            ...
                            self.svn_update(target=p.as_posix(), depth='empty')
...
if __name__ == '__main__':
    ...
    svn = Subversion(Path(__file__).parent.absolute())
    # print(svn)
