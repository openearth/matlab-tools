...
def main():   
    ...
    if not os.path.exists(clone_map):
        ...
        ds = ds = gdal.Open(clone_map,GA_ReadOnly)
    ...
    ''' read settings '''
    snapgaugestoriver=bool(int(wt.configget(config,"settings","snapgaugestoriver","1")))
    burnalltouching= bool(int(wt.configget(config,"settings","burncatchalltouching","1")))
    ...
    if not spatialref == None:
        ...
        clone2dem_transform = osr.CoordinateTransformation(srs_DEM,srs_clone)
    else:
        dem_EPSG = clone_EPSG
    ...
    stats = wt.windowstats(dem_in,clone_rows,clone_columns,clone_trans,srs_clone,resultdir,percentile)
...
#    else:
    ...
    rivshp = rivclipshp
    shapes = wt.Reach2Nodes(rivclipshp,clone_EPSG_int,cellsize*verticetollerance,workdir)
    ...
    else: call(('gdal_rasterize','-burn','1','-l',rivshp_att,rivshp,rivers_tif))
...
#    else:
    ...
    catchments = pcr.cover(pcr.ifthen(catchments > 0, pcr.ordinal(catchments)),pcr.ifthen(riverburn>0,pcr.ordinal(pcr.spreadzone(pcr.nominal(catchments),pcr.ifthen(riverburn>0, pcr.scalar(1)),1))))    
