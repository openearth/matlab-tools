...
def block_max(timeseries,freq,threshold=None,peak_dist=1,realpeak=False):
    ...
    for i,peak in enumerate(BM.values):
        if i+1 < npeak:
            idx1 = np.where(timeseries.index == BM.index[i+1])[0]
        else:
            idx1 = -1    
        idx0 = np.where(timeseries.index == BM.index[i])[0]
        if len(idx0) == 0:
            idx0 = 0
        idx[i] = int(idx0 + np.where(timeseries.iloc[idx0:idx1].values == peak)[0][-1])
        
    BM.index = timeseries.index[idx.astype(int)]
