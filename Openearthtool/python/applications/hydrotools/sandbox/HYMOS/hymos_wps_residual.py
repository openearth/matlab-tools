...
http://localhost/cgi-bin/pywps.cgi?&service=wps&version=1.0.0&request=DescribeProcess&identifier=hymos_wps_residual
http://localhost/cgi-bin/pywps.cgi?&service=wps&version=1.0.0&request=Execute&identifier=hymos_wps_residual&datainputs=[parameterid=P.obs;locations=M_AZ_001;startdate=20090101;enddate=20100101]
...
URL of source:           $HeadURL: https://svn.oss.deltares.nl/repos/openearthtools/trunk/python/applications/hydrotools/sandbox/HYMOS/hymos_wps_residual.py $
CodeID:                  $ID$
...
class Process(WPSProcess):
    def __init__(self):
        ...
        WPSProcess.__init__(self,
                            identifier="hymos_wps_residual",
                            title="""Residual curve""",
                            abstract="""This returns a residual curve
                            time series for a selected parameter and location.
                            The residual is calculated from the mean of the
                            series.
                            """,
                            version="1.1",
                            storeSupported=True,
    ...
    def execute(self):
        ...
        logging.info('locations    ' + self.locations.getValue())
        ...
        if len(self.sdate.getValue()) < 8:
            ...
            if len(self.edate.getValue()) < 8:
                ...
                io = bokeh_plots.bokeh_residual(
                    self.parameterID.getValue(),
                    self.locations.getValue(),
                    '19810101', '19820101', **arg_dict
                )
        else:
            io = bokeh_plots.bokeh_residual(self.parameterID.getValue(),
                                            self.locations.getValue(),
                                            self.sdate.getValue(),
                                            self.edate.getValue(),
                                            **arg_dict)
