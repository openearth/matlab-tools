...
http://localhost/cgi-bin/pywps.cgi?&service=wps&version=1.0.0&request=DescribeProcess&identifier=hymos_wps_timeseries
http://localhost/cgi-bin/pywps.cgi?&service=wps&version=1.0.0&request=Execute&identifier=hymos_wps_timeseries&datainputs=[parameterid=P.obs.month;locations=M_AZ_001;startdate=20090101;enddate=20100101]
...
URL of source:           $HeadURL: https://svn.oss.deltares.nl/repos/openearthtools/trunk/python/applications/hydrotools/sandbox/HYMOS/hymos_wps_timeseries.py $
CodeID:                  $ID$
...
class Process(WPSProcess):
    def __init__(self):
        ...
        WPSProcess.__init__(self,
                            identifier="hymos_wps_timeseries",
                            title="""Time series""",
                            abstract="""This returns the
    ...
    def execute(self):
        ...
        if len(self.sdate.getValue()) < 8:
            ...
            if len(self.edate.getValue()) < 8:
                ...
                io = bokeh_plots.bokeh_timeseries(
                    self.parameterID.getValue(),
                    self.locations.getValue(),
                    '19810101', '19820101', **arg_dict
                )
        else:
            io = bokeh_plots.bokeh_timeseries(self.parameterID.getValue(),
                                              self.locations.getValue(),
                                              self.sdate.getValue(),
                                              self.edate.getValue(),
                                              **arg_dict)
