...
def gdal_sample_points(lat,lon,raster_file,win_size=1, func=None):
    ...
    if src_ds == None:
        ...
        values = np.array([])
        if (np.size(lat)==1)&(type(lat) != list):
            lon = [lon]; lat = [lat]
        for my,mx in zip(lat,lon):
    
            #Convert from map to pixel coordinates.
            px,py = MapToPixel(mx,my,gt)
            if (px>=0)&(px<=cols)&(py>=0)&(py<=rows): # check if wihtin map extent
                if win_size > 1:
                    intval=rb.ReadAsArray(int(px-win_size/2.),int(py-win_size/2.),win_size, win_size)
                else:
