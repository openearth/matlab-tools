"""
This is the __init__ file for hydrotools.statistics
Add imports to your new functions as follows:

# import functions in new_file.py in a new namespace as follows 
(preferred for a whole bunch of functions forming a separate subpackage):
import new_file
# now you can call a function new_fun in new_file.py as follows:
hydrotools.statistics.new_file.new_fun

# import functions under the current namespace as follows (preferred for separate functions):
from new_file import *
# now you can call a function new_fun in new_file.py as follows:
hydrotools.statistics.new_fun

$Id: __init__.py 11407 2014-11-20 13:04:30Z winsemi $
$Date: 2014-11-20 14:04:30 +0100 (Thu, 20 Nov 2014) $
$Author: winsemi $
$Revision: 11407 $
$HeadURL: https://svn.oss.deltares.nl/repos/openearthtools/trunk/python/applications/hydrotools/hydrotools/statistics/__init__.py $
$Keywords: $

"""
