...
def class_legend(ax, classes, cmap, extend=None, ncol=1, bbox_to_anchor=None, \
    ...
    Inputs:
        ...
        classTitle:     title on top of legend
    ...
    if np.logical_or(extend == 'max', extend == 'both'):
        ...
        ClassPatch = patches.Rectangle((0,0), 1, 1, fc=classColor, ec='#999999')
        p.append(ClassPatch)
    leg = ax.legend(p, classLabels, loc='right', ncol=1, bbox_to_anchor=bbox_to_anchor, handlelength=.7, prop=prop, title=classTitle) # ,columnspacing=.75,handletextpad=.25, , 
