...
libnefis.Inqfel.argtypes = libnefis.Inqnel.argtypes = [pint, pchar, pchar,
                                                       pchar, pchar, pchar,
                                                       pint, pint, pint, pint]
libnefis.Inqfcl.argtypes = libnefis.Inqncl.argtypes = [pint, pchar, pint,
                                                       pint, achartype]
libnefis.Inqncl.argtypes = libnefis.Inqncl.argtypes = [pint, pchar, pint,
                                                       pint, achartype]
libnefis.Inqfgr.argtypes = libnefis.Inqngr.argtypes = [pint, pchar, pchar,
                                                       pint, pint, pint]
...
class Nefis:
    ...
    def _getallelement(self):
        ...
        while not error:
            ...
            if error:
                break
            self._allelements[elname.value.strip()] = \
                Element(elname.value, elmtyp.value, nbytsg.value, elmdes.value,
                        elmndm.value, list(elmdms)[:elmndm.value],
                        nbytes.value)
    ...
    def getdata(self, groupname, elementname, grprange=''):
        ...
        for it in grprange:
            ...
            usrord = aint5(0, 0, 0, 0, 0)  # dummy parameter
