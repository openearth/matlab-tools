# python version of Matlab counterpart ncwritetutorial_grid_lat_lon_orthogonal

__version__ = "$HeadURL: https://svn.oss.deltares.nl/repos/openearthtools/trunk/python/OpenEarthTools/openearthtools/io/netcdf/netCDF4_tutorial_grid_lat_lon_curvilinear.py $" + "$Revision: 8903 $"
...
f = netCDF4.Dataset('netCDF4_tutorial_grid_lat_lon_orthogonal.nc', 'w', 
...
f.title           = 'ncwritetutorial_grid_lat_lon_orthogonal'
f.institution     = M['institution']
f.source          = ''
f.history         = "$HeadURL: https://svn.oss.deltares.nl/repos/openearthtools/trunk/matlab/io/netcdf/nctools/ncwritetutorial_grid_lat_lon_curvilinear.m $ $Id: ncwritetutorial_grid_lat_lon_curvilinear.m 8901 2013-07-09 06:51:31Z boer_g $"
...
# create dimensions
f.createDimension('lat'   ,     D['lat'].shape[0])
f.createDimension('lon'   ,     D['lon'].shape[0])
...
nc_lat = f.createVariable('lat'       , 'f8',         ('lat'      ),zlib=True,fill_value=NaN) # always double, CF convention: y first
nc_lon = f.createVariable('lon'       , 'f8',               ('lon'),zlib=True,fill_value=NaN) # always double
nc_w   = f.createVariable('projection', 'i' ,(                    )                         )
if OPT['bounds']:
    nc_lonc= f.createVariable('lon_bnds'  , 'f8',('bounds',      'lon'),zlib=True,fill_value=NaN) # always double
    nc_latc= f.createVariable('lat_bnds'  , 'f8',('bounds','lat'      ),zlib=True,fill_value=NaN) # always double
nc_z   = f.createVariable('depth'         , 'f4',('time'  ,'lat','lon'),zlib=True,fill_value=NaN) # may be single

# Note! If very large arrays are written, use chunksizes, example given below. 
# More on chunksizes: see http://netcdf4-python.googlecode.com/svn/trunk/docs/netCDF4.Variable-class.html
# nc_z = f.createVariable('depth', 'f4', ('time', 'lat', 'lon'), zlib=True,
#                        fill_value=NaN,
#                        chunksizes=(1000, 1000))  # may be single
...
if OPT['bounds']:
    nc_lonc[:,:] = CF.cor2bounds(D['lonc'])
    nc_latc[:,:] = CF.cor2bounds(D['latc'])
