...
# $Id: odv2timeseries.py 11742 2015-02-26 09:43:26Z santinel $
...
def odv2timeseries(pngFilepath,ODV,cname,zname=[],clims=[],zlims=[],times=[],cmapstr="jet",log10=0,colorvalue="blue",markertype="o",markersizevalue=6,alphavalue=1): # matplotlib
    ...
    if log10:
       ...
       for i in range(0,len(t)):
          plt.plot_date(t[i],np.log10(c[i]), color=sco.to_rgba(Z[i],alpha=alphavalue), markersize=markersizevalue, marker=markertype)
       ...
       # set axes limits
       odv2timeseriesaxes(ax,times,np.log10(clims),t,np.log10(c))
       ...
       plt.hold('true')
       for i in range(0,len(t)):
          #logging.info(i); #logging.info(Z[i]); #logging.info(sco.to_rgba(Z[i],alpha=alphavalue))
