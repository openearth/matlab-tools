...
# $Date: 2019-04-19 17:06:09 +0200 (vr, 19 apr 2019) $
# $Author: hendrik_gt $
# $Revision: 15359 $
...
class Odvfile(Base): # each file contains only one CDI
   ...
   #cdi           = Column(String , ForeignKey('cdi.cdi', ondelete='CASCADE'),nullable=False)
    ...
    entrydate     = Column(DateTime(timezone=True))
...
class Edmo(Base):
    ...
    code          = Column(Integer, index=True,unique=True,nullable=False)                  # unique EDMO_code number, could be PK?
    name          = Column(String)
...
class Parameter(Base):
    ...
    p02_id        = Column(Integer , ForeignKey('p02.id', ondelete='CASCADE'),nullable=True)
...
class P02(Base): # SeaDataNet Parameter Discovery Vocabulary
    __tablename__ = 'p02'
    ...
    altlabel      = Column(String)
    definition    = Column(String)
    l04_id        = Column(Integer , ForeignKey('l04.id', ondelete='CASCADE'),nullable=True)
    def __repr__(self):
        return "<P02: id=%s, identifier='%s', altlabel='%s')>" % (self.id, self.identifier, self.altlabel)

class L04(Base): # Matrix Categories
    __tablename__ = 'l04'
...
class S27(Base): # BODC substances
    __tablename__ = 's27'
    id            = Column(Integer,Sequence('s27_id_seq'),primary_key=True)  # technical key as PK
    identifier    = Column(String, index=True,unique=True)         # candidate key, variable length confirmed by Roy Lowry
    preflabel     = Column(String)
    altlabel      = Column(String)
    definition    = Column(String)
    def __repr__(self):
        return "<S27: id=%s, identifier='%s', altlabel='%s')>" % (self.id, self.identifier, self.altlabel)    
...
class Cdi(Base): # each CDI can be spread over multiple files, whose name is not always identical to local_cdi_id (prefix and/or suffix)
    ...
    cdi           = Column(String   , index=True,unique=True,nullable=False)                  # currently we define: cdi=edmo_code:local_cdi_id
    local_cdi_id  = Column(String   , index=True,nullable=False) 			       # only unique per edmo_code, how to handle that?
...
class Observation(Base):
    ...
    datetime      = Column(DateTime(timezone=False),nullable=False)     # SDN requires UTC
    ...
    z_id          = Column(Integer, ForeignKey('parameter.id', ondelete='CASCADE'))
    dbversion     = Column(Integer)
