...
def odv2timeprofile_allinrange(pngFilepath,ODV,cname,zname=[],clims=[],zlims=[],times=[],cmapstr="jet",log10=0,colorvalue="blue",markertype="o",markersizevalue=6,alphavalue=1): # matplotlib
...
#       c    = np.ma.masked_invalid(np.array(c.as_matrix(),'float')) # np.log10 can't handle pandas arrays
    ...
    # _allinrange option
    c_allinrange_ind = np.where(np.logical_and(C>=Clims[0], C<=Clims[1]))
    t_allinrange = np.asarray(t)[c_allinrange_ind]
    c_allinrange = C[c_allinrange_ind]
    z_allinrange = z[c_allinrange_ind]
    
    if log10:
       # TODO!!! log10 has to be applied to the colorbar!!
       for i in range(0,len(t_allinrange)):
          plt.plot_date(t_allinrange[i],z_allinrange[i], color=sco.to_rgba(c_allinrange[i],alpha=alphavalue), markersize=markersizevalue, marker=markertype)
       plt.colorbar(scp).set_label(clabel)
       
       odv2timeseriesaxes(ax,times,zlims,t_allinrange,z_allinrange)
       ...
       #ax.set_yticklabels(yticklabels);
        ...
        #logging.info(t_allinrange)
        #logging.info(t)
        #logging.info(z_allinrange)
        #logging.info(z)
