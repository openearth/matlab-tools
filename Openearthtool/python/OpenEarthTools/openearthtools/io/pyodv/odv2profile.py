...
# $Id: odv2profile.py 11742 2015-02-26 09:43:26Z santinel $
# $Date: 2015-02-26 01:43:26 -0800 (Thu, 26 Feb 2015) $
# $Author: santinel $
# $Revision: 11742 $
...
def odv2profile(pngFilepath,ODV,cname,zname=[],clims=[],zlims=[],times=[],cmapstr="jet",log10=0,colorvalue='blue',markertype="o",markersizevalue=6,linestylevalue='solid',linewidthvalue=1,alphavalue=1): # matplotlib
...
#       #sco.ax1.set_yticklabels([datetime.datetime.strptime(times[0], "%Y-%m-%dT%H:%M:%SZ").strftime("%Y-%m-%d"), datetime.datetime.strptime(times[1], "%Y-%m-%dT%H:%M:%SZ").strftime("%Y-%m-%d")])
    ...
    if log10:
        for i in np.arange(len(minima)):
            plt.plot(np.log10(c[minima[i]:maxima[i]+1]),z[minima[i]:maxima[i]+1],color=sco.to_rgba(T[i],alpha=alphavalue),marker=markertype,markersize=markersizevalue,linestyle=linestylevalue,linewidth=linewidthvalue,alpha=alphavalue) # too many lines cover the points
        ...
        for i in np.arange(len(minima)):
            ...
            plt.plot(c[minima[i]:maxima[i]+1],z[minima[i]:maxima[i]+1],color=sco.to_rgba(T[minima[i]],alpha=alphavalue),marker=markertype,markersize=markersizevalue,linestyle=linestylevalue,linewidth=linewidthvalue,alpha=alphavalue) # too many lines cover the points
            #logging.info(T[i])
