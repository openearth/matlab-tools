...
def odv2edmocdi_allinrange(ODV,cname,zname=[],clims=[],zlims=[],times=[],plot_type=[]):
    ...
    # get indices for values within the range, with _allinrange option
    if plot_type=='profile':
        o_c = np.where(np.logical_and(c>=Clims[0], c<=Clims[1]))
        o_z = np.where(np.logical_and(z>=Zlims[0], z<=Zlims[1]))
        indrng0 = np.intersect1d(o_z[0],o_c[0])
        o_t = np.where(np.logical_and(np.asarray(T)>=Tlims[0], np.asarray(T)<=Tlims[1]))
        indrng = np.intersect1d(indrng0,o_t[0])
        
    elif plot_type=='timeseries':
        o_c = np.where(np.logical_and(c>=Clims[0], c<=Clims[1]))
        o_t = np.where(np.logical_and(np.asarray(T)>=Tlims[0], np.asarray(T)<=Tlims[1]))
        indrng0 = np.intersect1d(o_t[0],o_c[0])
        o_z = np.where(np.logical_and(z>=Zlims[0], z<=Zlims[1]))
        indrng = np.intersect1d(indrng0,o_z[0])
        
    elif plot_type=='timeprofile':
        o_z = np.where(np.logical_and(z>=Zlims[0], z<=Zlims[1]))
        o_t = np.where(np.logical_and(np.asarray(T)>=Tlims[0], np.asarray(T)<=Tlims[1]))
        indrng0 = np.intersect1d(o_z[0],o_t[0])
        o_c = np.where(np.logical_and(c>=Clims[0], c<=Clims[1]))
        indrng = np.intersect1d(indrng0,o_c[0])
