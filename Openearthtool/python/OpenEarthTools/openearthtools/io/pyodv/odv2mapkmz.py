...
# $Id: odv2mapkmz.py 11737 2015-02-24 16:20:51Z santinel $
# $Date: 2015-02-24 08:20:51 -0800 (Tue, 24 Feb 2015) $
# $Author: santinel $
# $Revision: 11737 $
# $HeadURL: https://svn.oss.deltares.nl/repos/openearthtools/trunk/python/OpenEarthTools/openearthtools/io/pyodv/odv2mapkmz.py $
# $Keywords: $

# TODO: include cname within the list of odv2mapkmz function arguments
...
def odv2mapkmz(kmzFilepath,ODV,cname,clims=[],log10=0,cmapstr="jet",alphavalue=1): # simplekml
    ...
    # color limits autoscale
    if len(clims)==0:
       clims    = [0,0]
       clims[0] = min(ODV.data[cname])
       clims[1] = max(ODV.data[cname])       
    ...
    for irow in range(0, N):
        ...
        time.append(times[irow].isoformat())
        #logging.info(time[irow])
        #logging.info(times[irow])
        ...
        # balloonstyle
        if Balloon:
            pnt.stylemap.highlightstyle.balloonstyle.text = "<![CDATA[<h3>" + stnam + " $[name]</h3> $[description]]]>"
