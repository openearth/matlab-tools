...
$Id: sqlfunctions.py 14139 2018-02-01 11:57:46Z schoonve $
$Date: 2018-02-01 03:57:46 -0800 (Thu, 01 Feb 2018) $
$Author: schoonve $
$Revision: 14139 $
$HeadURL: https://svn.oss.deltares.nl/repos/openearthtools/trunk/python/OpenEarthTools/openearthtools/io/pyodv/sqlfunctions.py $
$Keywords: $
...
def flushCopyBuffer(bufferFile,atable,credentials,cols,asep):
    conn = psycopg2.connect("dbname={dbname} host={host} user={user} password={password} port={port}".format(**credentials))
    cur = conn.cursor()
...
def get_credentials(credentialfile,dbase=None):
    ...
    for i in fdbp:
        ...
        if str.strip(item[0]) == 'port':
            credentials['port'] = str.strip(item[1])
...
def get_colnames(name,credentials):
    """
    get column headers from 'name' file
    """
    conn = psycopg2.connect("dbname={dbname} host={host} user={user} password={password}".format(**credentials))
    cur = conn.cursor()
    strSql = 'SELECT * FROM {} LIMIT 0'.format(name)
    try:
        cur.execute(strSql)
        column_names = [p[0] for p in cur.description]
        return column_names
    except Exception,e:
        print e.message
    finally:
