...
!                                                                      *
     ...
     &                   KGRPNT          ,XCGRID  ,                       30.72
     &                   YCGRID          ,KGRBND  ,                       40.30
     &                   OURQT           ,COMPSPC)                        40.51 40.30 40.61mud
!                                                                      *
!***********************************************************************
!
      USE TIMECOMM                                                        40.41
...
!     40.G1G, Dec. 07: calculate spectrum-averaged real and imag wavelength k(x,y) from wave length k(x,y,sigma)
!
!  2. Purpose
!
...
!xxxxxfor each location). array VOQR shows with quantity is written in xxx40.61mud
!     for each location). array VOQR shows with quantity is written in    40.61mud
!     each column.
...
!
      ...
      COMPDA(:,JLMR ) = 0.                                               !40.61mud
      COMPDA(:,JLMI ) = 0.                                               !40.61mud
      CALL SWKMEAN(COMPSPC(1,1,JKWVR),SPCSIG,AC2,COMPDA (1,JLMR),0)      !40.61mud CHANGE NODATAVALUE !
      CALL SWKMEAN(COMPSPC(1,1,JKWVI),SPCSIG,AC2,COMPDA (1,JLMI),0)      !40.61mud CHANGE NODATAVALUE !
      DO INDX = 1,MCGRD
         IF (COMPDA (INDX,JLMI)==0) THEN
         COMPDA (INDX,JLMI) = 0
       ELSE
         COMPDA (INDX,JLMI) = 2*PI/COMPDA (INDX,JLMI) ! make units from WLEN [m] to K [rad/m], avoid 1/0 is Infinity
       ENDIF
       ENDDO
       ENDIF

      !write(*,*) '-SWOUTP:------------------'
      !write(*,*) 'COMPDA (:,JLMR) = WLENMR '
      !write(*,*) 'integrated over frequencies from COMPSPC(1,1,JKWVR)'
      !write(*,*)  COMPDA (:,JLMR)
      !write(*,*) '--------------------------'
      !write(*,*) 'COMPDA (:,JLMI) =  KI'
      !write(*,*) 'integrated over frequencies from COMPSPC(1,1,JKWVI)'
      !write(*,*)  COMPDA (:,JLMI)
      !write(*,*) '--------------------------'
!
...
!
     ...
     &                 COMPDA(1,JDP2)                           ,        !40.61mud
     &                 COMPDA                             ,COMPSPC)      !40.61mud
!    &                 COMPDA(1,JLMR)      ,COMPDA(1,JLMI),COMPSPC)      !40.61mud
...
!       ***** spectral output *****
        IF (RTYPE(1:2) .EQ. 'SP') THEN                                    20.28
          ...
          ELSE
             IF (IMUD==1) THEN             
!            Also accomodate kreal and kimag
             ALLOCATE(AUX1(5*MSC))                                        40.61mud
             ELSE
...
!
     ...
     &       IVTYPE.EQ.44 .OR. IVTYPE.EQ.48 .OR. IVTYPE.EQ.53      )      40.61 40.51 40.41 40.00
     &   BKC = MAX (1, BKC)
!
...
!
 ...
 121    FORMAT (' SWOEXD, type:', 4I3)
     ...
     &               VOQ(1,VOQR(4)) ,KGRPNT, COMPDA(1,JDP2))
      ENDIF
!
!     Mudlayer thickness                                                 40.61mud
!
 122  IF (OQPROC(60)) THEN
        IF (ITEST.GE.50 .OR. IOUTES .GE. 10) WRITE (PRTEST, 121) 4,
     &  VOQR(60), JMUDL2
 123    FORMAT (' SWOEXD, type:', 4I3)
        CALL SWIPOL (COMPDA(1,JMUDL2), OVEXCV(60), XC, YC, MIP,
...
!
     ...
     &  VOQR(57)
        CALL SWIPOL (COMPDA(1,JDSXM), OVEXCV(57), XC, YC, MIP,
...
!
     ...
     &  VOQR(52)
        ALLOCATE(TARR(MIP,2))
        CALL SWIPOL (COMPDA(1,JDP2), OVEXCV(4), XC, YC, MIP,
     &               TARR(1,1), KGRPNT, COMPDA(1,JDP2))
        IF (VARWLV) THEN
           CALL SWIPOL (COMPDA(1,JWLV2), OVEXCV(51), XC, YC, MIP,
     &                  TARR(1,2), KGRPNT, COMPDA(1,JDP2))
        ELSE
           TARR(:,2)=0.
        END IF
        DO IP = 1, MIP
           F1 = TARR(IP,1)
           F2 = TARR(IP,2)
           IF (.NOT.EQREAL(F1,OVEXCV( 4)) .AND.
     &         .NOT.EQREAL(F2,OVEXCV(51)) ) VOQ(IP,VOQR(52))= F1-F2-WLEV
        END DO
        DEALLOCATE(TARR)
      ENDIF
!
...
!                     swcomm4.inc reactivated
!     40.41, Oct. 04: common blocks replaced by modules, include files removed
!     40.51, Jul. 05: routines SWIPOL and SWOINA are made consistent
!
!  2. PURPOSE
!
...
!
      DO 100 IP=1,MIP
        ...
        IF (KREPTX .EQ. 0) THEN                                           40.13
          ...
          IF (SX1.LT.0.01 .OR. JX1.EQ.0)   THEN                           40.51
             SX1 = 0.                                                     40.51
             SX2 = 1.                                                     40.51
          ENDIF                                                           40.51
          IF (SX2.LT.0.01 .OR. JX1.EQ.MXC) THEN                           40.51
             SX2 = 0.                                                     40.51
             SX1 = 1.                                                     40.51
          ENDIF                                                           40.51
        ...
        IF (SY1.LT.0.01 .OR. JY1.EQ.0)   THEN                             40.51
           SY1 = 0.                                                       40.51
           SY2 = 1.                                                       40.51
        ENDIF                                                             40.51
        IF (SY2.LT.0.01 .OR. JY1.EQ.MYC) THEN                             40.51
           SY2 = 0.                                                       40.51
           SY1 = 1.                                                       40.51
        ENDIF                                                             40.51
        ...
        ELSE
          ...
          WW1 = SX1*SY1
          IF (DEP2(INDX1).LE.DEPMIN) OUTSID = .TRUE.                      40.51 40.00
          INDX2 = KGRPNT(JX2,JY1)                                         30.21
          WW2 = SX2*SY1
          IF (DEP2(INDX2).LE.DEPMIN) OUTSID = .TRUE.                      40.51 40.00
          INDX3 = KGRPNT(JX1,JY2)                                         30.21
          WW3 = SX1*SY2
          IF (DEP2(INDX3).LE.DEPMIN) OUTSID = .TRUE.                      40.51 40.00
          INDX4 = KGRPNT(JX2,JY2)                                         30.21
          WW4 = SX2*SY2
          IF (DEP2(INDX4).LE.DEPMIN) OUTSID = .TRUE.                      40.51 40.00
          IF (OUTSID) THEN
            FOUTP(IP) = EXCVAL
          ELSE
...
!                                                                      *
     ...
     &                   DEPXY      ,                                     30.50
     &                   COMPDA                 ,COMPSPC)                !40.61mud
!    &                   WLENMR     ,WLENMI     ,COMPSPC)                !40.61mud
!                                                                      *
!***********************************************************************
!
      USE OCPCOMM4                                                        40.41
...
!     MIP     Int    input    number of output points
!xxxxxXC, YC  real   input    comp. grid coordinatesxxxxxxxxxxxxxxxxxxxxxx40.61mud
!     XC, YC  real   input    OUTPUT grid coordinates                    !40.61mud
...
! 13. Source text
!
      PARAMETER  (NVOTP=21) !19)                                          40.51 40.41 40.00 40.61mud
      REAL       XC(MIP)        ,YC(MIP)       ,AC2(MDC,MSC,MCGRD),
     &           VOQ(MIP,*)     ,
     &           WK(*)          ,
     &           CG(*)          ,ACLOC(MDC,MSC),
     &           NE(*)          ,NED(*)        ,DEPXY(MCGRD), 
!    &           WLENMR(MCGRD)  ,WLENMI(MCGRD)                           !40.61mud
     &           COMPDA(MCGRD,MCMVAR)                                    !40.61mud 
      REAL       COMPSPC(MCGRD,MSC,MCSVAR)                               !40.61mud
!-!   REAL       EKTOTR         ,EKTOTI                                  !40.61mud 
      REAL       WLENMR         ,KI                                      !40.61mud 
!
...
!
     ...
     &            42, 43, 44, 47, 48, 53, 58, 59/                         40.51 40.41 40.00 40.61mud
...
!
!       compute k and Cg
!       INTERPLATE FROM COMPSC(1,:,JKVR)                                 !40.61mud
!
...
!          new integration method involving FRINTF                        20.59
           DO IS=1, MSC
              ...
              DO ID=1,MDC
                ETOT  = ETOT  + FRINTF *  SIG2 * ACLOC(ID,IS)            !40.61mud
                EKTOT = EKTOT + FRINTF *  SKK  * ACLOC(ID,IS)            !40.61mud
...
!482       FORMAT (' computation average k ', 6(1X,F8.3))
     ...
     &           VOQ(IP,VOQR(IVTYP2)) = 4.* SQRT(ETOT*DDIR) / WLMEAN
        ...
        ENDIF
!                                                                         40.61mud
!       real and imaginary wave length mud                                40.61mud
!                                                                         40.61mud
        IVTYP1 = 58
        IVTYP2 = 59
        IF (OQPROC(IVTYP1) .OR. OQPROC(IVTYP2)) THEN
...
!         Interpolate EKTOTR from WLENMI to one output point              40.61mud
!         (to get average wave length with mud real part)                 40.61mud

          IF (OQPROC(IVTYP1)) THEN                                       !40.61mud
 !        CALL SWIPOL(COMPDA(1,JLMR),OVEXCV(IVTYP1),XC(IP),YC(IP),1,     !40.61mud
          CALL SWIPOL(COMPDA(1,JLMR),OVEXCV(IVTYP1),XC(IP),YC(IP),1,     !40.61mud
     &                WLENR,KGRPNT,DEPXY)
          ENDIF
!         Interpolate EKTOTR from WLENMI to one output point              40.61mud
!         (to get average wave length with mud imag part)                 40.61mud

          IF (OQPROC(IVTYP2)) THEN                                       !40.61mud
!         CALL SWIPOL(COMPDA(1,JLMI),OVEXCV(IVTYP1),XC(IP),YC(IP),1,     !40.61mud
          CALL SWIPOL(COMPDA(1,JLMI),OVEXCV(IVTYP2),XC(IP),YC(IP),1,     !40.61mud
     &                KI   ,KGRPNT,DEPXY)
          ENDIF
	    !EKTOTR and EKTOTI,FRINTF shoudl be equal to MSC*MDC when debugging by
          !adding 1 for every direction and frequency in subroutine ADDLMEAN.
...
!-!
!-!! ==================================
!-!! ALSO TAIL FOR IMAGINARY PART ????? DOES NOT SEEM TO MAKE A DIFFERENCE
...
!-! 481          CONTINUE
!-!           ENDIF
!-!           IF (ETOT.LE.0.) THEN                                          !40.61mud
!-!             IF (OQPROC(IVTYP1)) VOQ(IP,VOQR(IVTYP1)) = OVEXCV(IVTYP1)
!-!             IF (OQPROC(IVTYP2)) VOQ(IP,VOQR(IVTYP2)) = OVEXCV(IVTYP2)
!-!           ELSE
!-!             IF (OQPROC(IVTYP1))
!-!     &           VOQ(IP,VOQR(IVTYP1)) = PI2 * (ETOT / EKTOTR)
!-!             IF (VOQ(IP,VOQR(IVTYP1)) > OVULIM(IVTYP1)) ! no need for that due to construction above
!-!     &           VOQ(IP,VOQR(IVTYP1)) = WLMEAN ! OVEXCV(IVTYP1) !0
!-!
!-!
!-!             IF (OQPROC(IVTYP2))
!-!     &           VOQ(IP,VOQR(IVTYP2)) = min(PI2 * (ETOT / EKTOTI),
!-!     &                                      OVULIM(IVTYP2))! to prevent *** in ASCII output
!-!
!-!!             IF (EKTOTI < TINY(EKTOTI)) ! no need for that due to construction above
!-!!     &           VOQ(IP,VOQR(IVTYP2)) = OVEXCV(IVTYP2) !0
!-!           ENDIF

           IF (WLENR<=0) THEN ! current NODATAVALUE
             IF (OQPROC(IVTYP1)) VOQ(IP,VOQR(IVTYP1)) = OVEXCV(IVTYP1)
             IF (OQPROC(IVTYP2)) VOQ(IP,VOQR(IVTYP2)) = OVEXCV(IVTYP2)
           ELSE
             IF (OQPROC(IVTYP1)) VOQ(IP,VOQR(IVTYP1)) = WLENR
             IF (OQPROC(IVTYP2)) VOQ(IP,VOQR(IVTYP2)) = KI
           ENDIF
...
!
      ...
      SX1 = 1. - SX2
      IF (KREPTX .EQ. 0) THEN                                             40.13
        IF (JX1.LT.0)   GOTO 890                                          40.51
        IF (JX1.GT.MXC) GOTO 890                                          40.51
...
!
      ...
      SY2 = YC + 1. - FLOAT(JY1)
      SY1 = 1. - SY2
      IF (JY1.LT.0)   GOTO 890                                            40.51
      IF (JY1.GT.MYC) GOTO 890                                            40.51
