...
!     KDIF      Fraction of energy that is reflected in a diffuse way     40.28
...
!               performed                                                 40.17
!     POWD      Power of cosine in diffusive reflecion                    40.28
!     POWS      Power of cosine in scattered reflecion                    40.18
...
!
        ...
        OBSTMP%RFCOEF(7) = 0.                                             40.31
        OBSTMP%RFCOEF(8) = 0.                                             40.31
!
...
!
        ...
        IF (KEYWIS ('REFL')) THEN                                         40.09
          ...
          IF (ITRAS.EQ.0 .AND. ((DUM < 0.) .OR. (DUM > 1.))) THEN         40.51 40.14
            CALL MSGERR(3,'Kt^2 + Kr^2 > 1 or Kt^2 + Kr^2 < 0! ')         40.51 40.14
          ENDIF                                                           40.14
...
!                                                                         40.18
          ...
          IF (KEYWIS('RDIFF')) THEN                                       40.18
            ...
            CALL INREAL ('POWS', POWS, 'REQ', 1.)                         40.18
            DUM = MOD(POWS,1.)                                            40.18
            IF (POWS < 0.) THEN                                           40.38
              CALL MSGERR(3,'Power POWS is not a positive number! ')      40.38
            ENDIF                                                         40.38
            IF (DUM .NE. 0.) THEN                                         40.18
              CALL MSGERR(3,'Power POWS is not an integer number! ')      40.18
            ENDIF                                                         40.18
            OBSTMP%RFCOEF(2) = POWS                                       40.31
!                                                                         40.18
            CALL INREAL ('POWD', POWD, 'STA', 1.)                         40.28
            DUM = MOD(POWD,1.)                                            40.28
            IF (POWD < 0.) THEN                                           40.38
              CALL MSGERR(3,'Power POWD is not a positive number! ')      40.38
            ENDIF                                                         40.38
            IF (DUM .NE. 0.) THEN                                         40.28
              CALL MSGERR(3,'Power POWD is not an integer number! ')      40.28
            ENDIF                                                         40.28
            OBSTMP%RFCOEF(3) = POWD                                       40.31
            CALL INREAL ('KDIF', KDIF, 'STA', 0.)                         40.28
            IF ((KDIF < 0.) .OR. (KDIF > 1.)) THEN                        40.28
              CALL MSGERR(3,'KDIF > 1 or KDIF < 0! ')                     40.28
            ENDIF                                                         40.28
            OBSTMP%RFCOEF(4) = KDIF                                       40.31
          ELSE                                                            40.18
...
!
          ...
          IF (KEYWIS('RFD')) THEN                                         40.28
            ...
            ENDIF                                                         40.28
            OBSTMP%RFCOEF(5) = FD1                                        40.31
            OBSTMP%RFCOEF(6) = FD2                                        40.31
            OBSTMP%RFCOEF(7) = FD3                                        40.31
            OBSTMP%RFCOEF(8) = FD4                                        40.31
          ELSE                                                            40.28
            LRFRD = 0                                                     40.31 40.28
          ENDIF                                                           40.28
...
!         air-sea temperature difference                                  40.03
          IGRD = 10
          PSNAME = 'ASTDGRID'
        ELSE IF (KEYWIS ('MUDL')) THEN                                    40.55
!         fluid mud layer                                                 40.55
          IGRD = 11                                                       40.55
          PSNAME = 'MUDLGRID'                                             40.55
        ELSE
          IGRD = 1
          PSNAME = 'BOTTGRID'
        ENDIF
...
!
        IF (OPTG.EQ.3) THEN                                               30.60
          ...
          IF (KEYWIS ('EXC')) THEN                                        30.60
            ...
            OVEXCV(1) = EXCFLD(8)                                         40.41
            OVEXCV(2) = EXCFLD(9)                                         40.41
          ENDIF                                                           30.60
        ENDIF                                                             30.60
!
...
!
      IF (KEYWIS ('MUD')) THEN
        IMUD = 1
        CALL INREAL('ALPHA'  ,PMUD( 1),'UNC',0.)
        CALL INREAL('RHOM'   ,PMUD( 2),'UNC',0.)
        CALL INREAL('RHO0'   ,PMUD( 3),'UNC',0.)
        CALL INREAL('NU'     ,PMUD( 4),'UNC',0.)
        CALL INREAL('LAYER'  ,PMUD( 5),'UNC',0.)
        CALL INREAL('DISPERR',PMUD( 6),'UNC',0.)                         !40.61mud
        CALL INREAL('DISPERI',PMUD( 7),'UNC',0.)                         !40.61mud
        CALL INREAL('SOURCE' ,PMUD( 8),'UNC',0.)                         !40.61mud
        CALL INREAL('CG'     ,PMUD( 9),'UNC',0.)                         !40.61mud
        CALL INREAL('POWER'  ,PMUD(10),'UNC',0.)                         !40.61mud
        JLMR   = MCMVAR + 1                                               40.61mud
        JLMI   = MCMVAR + 2                                               40.61mud
        MCMVAR = MCMVAR + 2                                               40.61mud
        JKWVI  = MCSVAR + 1                                               40.61mud
        JKWVR  = MCSVAR + 2                                               40.61mud
        MCSVAR = MCSVAR + 2                                               40.61mud
        ! NMOVAR = 60 ! instead of 57 to accomodate WLENMR, WLENMI, MUDL  40.61mud
	write(*,*) 'PMUD  ', PMUD
        GOTO 100
      ENDIF
!
...
!
      IF (KEYWIS ('TRI')) THEN
        ...
        CALL INREAL ('CUTFR', PTRIAD(2), 'STA', 5.0)                      40.55 30.82
        CALL INREAL ('URCRIT', PTRIAD(4), 'UNC', 0.0)                     40.13
        CALL INREAL ('URSLIM', PTRIAD(5), 'STA', 0.01)                    40.41 40.23 40.13
!       reserve space in array Compda for Ursell number                   40.03
        IF (JURSEL.LE.1) THEN
          MCMVAR = MCMVAR + 1
          JURSEL = MCMVAR                                                 40.03
        ENDIF
...
!       fluid mud layer
        IGR1   = 11
        VARMUD = .TRUE.                                                   40.55
        IF (JMUDL2.LE.1) THEN                                             40.55
          MCMVAR = MCMVAR + 2                                             40.55
          JMUDL2 = MCMVAR - 1                                             40.55
          JMUDL3 = MCMVAR                                                 40.55
        ENDIF                                                             40.55
      ELSE
        CALL  WRNKEY
...
!
     ...
     &             IFLNHD(IGR1), IFLNHF(IGR1))
      ...
      ELSE IF (IGR1.EQ.11) THEN                                           40.55
         IF (.NOT.ALLOCATED(MUDLF)) ALLOCATE(MUDLF(MXG(IGR1),MYG(IGR1)))  40.55
         CALL SWCOPR( TARR, MUDLF, MXG(IGR1)*MYG(IGR1) )                  40.55
      END IF                                                              40.31
...
!     WRITE (NREF, 103) MXC*MYC, 'number of locations'
	WRITE (NREF, '(3(I6,X), T41, A)')
     &                  MXC*MYC,MXC,MYC, 'number of locations,mxc,myc'   !40.51G
      DO IX = 1, MXC
        DO IY = 1, MYC
