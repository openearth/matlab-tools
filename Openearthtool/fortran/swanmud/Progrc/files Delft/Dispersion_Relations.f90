...
!     $Id: Dispersion_Relations.f90 4624 2011-05-31 19:01:26Z boer_g $ 
!     $Date: 2011-05-31 12:01:26 -0700 (Tue, 31 May 2011) $ 
!     $Author: boer_g $ 
!     $Revision: 4624 $ 
!     $HeadURL: https://svn.oss.deltares.nl/repos/openearthtools/trunk/fortran/swanmud/Progrc/files%20Delft/Dispersion_Relations.f90 $ 
!     $Keywords: $. 
...
real :: Dispersion_Relations_version = 1.0
...
!****************************************************************
      ...
      CALL DZANLY(DFDEWIT,0.01,0.01,0,1,1,kk,400,xxi,Info)
      kmudD = xxi
...
!     (dWIT is DR according to De Wit(1995), is FKRAN minus term3 and term1)
      ...
      complex*16             :: coshmd, sinhmd, coshmh, sinhmh, m
      ...
      coshkd=(cdexp(K*D)+cdexp(-K*D))/2.0
      sinhkd=(cdexp(K*D)-cdexp(-K*D))/2.0
      coshkh=(cdexp(K*H)+cdexp(-K*H))/2.0
      sinhkh=(cdexp(K*H)-cdexp(-K*H))/2.0

      coshmd=(cdexp(m*D)+cdexp(-m*D))/2.0
      sinhmd=(cdexp(m*D)-cdexp(-m*D))/2.0
      coshmh=(cdexp(m*H)+cdexp(-m*H))/2.0
      sinhmh=(cdexp(m*H)-cdexp(-m*H))/2.0

      !NB: H=waterdiepte
      !D=bodemlaagdikte
      !precies anders als in DeWit(1995)

      term4 = + coshmd*Rhom*coshkh/K               &
              - Rhow*sinhkh*sinhmd/m               &
              + Rhow*sinhkh*coshmd*D
      term2 = + Rhom*G*K*coshkh*sinhmd/m           &
              - Rhom*G*K*coshkh*coshmd*D           &
              - Rhom*G*coshmd*sinhkh
      term0 = - K**2.0*G**2.0*sinhkh*coshmd*Rhow*D &
              - K**2.0*G**2.0*sinhkh*sinhmd*Rhom/m &
              + K**2.0*G**2.0*sinhkh*coshmd*Rhom*D &
              + K**2.0*G**2.0*sinhkh*sinhmd*Rhow/m

      ! De daadwerkelijke determinant
      R       = + term4*Omega**4          &
                + term2*Omega**2  &
                + term0*Omega**0
      hyp2F   = real(R)**2.0 + imag(R)**2.0
...
!****************************************************************
      ...
      LOGICAL, SAVE           :: condition1          = .false.
      LOGICAL, SAVE           :: condition2          = .false.
      logical                 :: improve_with_argand = .true.
      ...
      ELSE
        ...
        if (improve_with_argand) then

          checkKRAN1 = DFKRAN(kmudD)

          IF (abs(real(checkKRAN1)) > 1E-2 .OR. abs(imag(checkKRAN1)) > 1E-2) THEN
!            write(*,*) ' '
!            write(*,'(A15,2E14.4)') 'kmudD = ',      real(kmudD),     imag(kmudD)
!            write(*,'(A15,2E14.4)') 'checkKRAN1 = ', real(checkKRAN2), imag(checkKRAN1)

                CALL DARGAND(kmudcor)

                checkKRAN2 = DFKRAN(kmudcor)
!                write(*,'(A15,2E14.4)') 'kmudcor = ',      real(kmudcor),     imag(kmudcor)
!                write(*,'(A15,2E14.4)') 'checkKRAN2 = ', real(checkKRAN2), imag(checkKRAN2)


                IF (abs(real(checkKRAN2)) <= abs(real(checkKRAN1)) .AND. abs(imag(checkKRAN2)) <= abs(imag(checkKRAN1))) THEN
                     condition1 = .true.
                ENDIF
                IF (abs(real(checkKRAN2)) <= 1E-2 .AND. abs(imag(checkKRAN2)) <= 1E-2) THEN
                     condition2 = .true.
                ENDIF
                ...
                ELSE
                     CountNoImpr = CountNoImpr + 1 
                ENDIF
                write(*,*) 'Improvements:   ', Countcorr, ' of which no improvement: ', CountNoImpr

          ! sign change cg
          ! IF (real(checkKRAN2) <= 0) THEN
          !          kmudD = 0.
          ! write(*,*) 'kreal < 0'
          ! ENDIF
        ...
        endif ! if (improve_with_argand) then
...
!     (KRAN is DR according to KRAN, corrected version of DR DeWit(1995))
      ...
      term4 = + coshmd*Rhom*coshkh/K &
              - Rhow*sinhkh*sinhmd/m &
              + Rhow*sinhkh*coshmd*D
      term3 = - 2.0*ic*K*Rhom*num*coshmd**2.0*coshkh &
              + 2.0*ic*K*Rhom*num*coshmd*coshkh &
              + 2.0*ic*K*num*Rhom*sinhmd**2.0*coshkh
      term2 = + Rhom*G*K*coshkh*sinhmd/m &
              - Rhom*G*K*coshkh*coshmd*D &
              - Rhom*G*coshmd*sinhkh
      term1 = + 2.0*ic*K**2.0*G*Rhom*num*sinhkh*coshmd**2.0 &
              - 2.0*ic*K**2.0*G*Rhom*num*sinhkh*coshmd &
              - 2.0*ic*K**2.0*G*Rhom*num*sinhkh*sinhmd**2.0 
      term0 = - K**2.0*G**2.0*sinhkh*Rhow*coshmd*D &
              - K**2.0*G**2.0*sinhkh*sinhmd*Rhom/m &
              + K**2.0*G**2.0*sinhkh*coshmd*D*Rhom &
              + K**2.0*G**2.0*sinhkh*sinhmd*Rhow/m
...
!****************************************************************
      ...
      ! Iteration Procedure with DZANLY, give back kmudD
      CALL DZANLY(DFDALR,0.01,0.01,0,1,1,kk,400,xxi,Info)
      kmudD     = xxi
