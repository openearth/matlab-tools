...
!
     ...
     &                   IT         ,KGRPNT     ,
     &                   XCGRID     ,YCGRID     ,                         30.72
     &                   CROSS      ,COMPSPC    )                         40.31 40.30 40.61mud
!
!******************************************************************
...
!     SWRECVAC                                                            40.30
!     SWSENDAC                                                            40.30
!     MSGERR : Handles error messeges according to severity               40.41
...
!      |
!      |-------+ SWOMPU                      carries out computation for one grid point 40.61mud
!      |       |
!      |       |--------+ SKWAVK             determ. of waveparameters: imageinary k
!      |       |
!      |       |--------+ SWAPAR/SWAPARM     determ. of waveparameters: group velocity
!      |       |
...
!      |       |        |
!      |       |        +-------+ SWMUD      fluid mud-induced wave dissipation
!      |       |        |
!      |       |        +-------+ SWCAP      white capping
!      |       |        |
!      |       |        +-------+ SSURF      wave breaking
...
!
     ...
     &         COMPDA(MCGRD,MCMVAR)                                       40.31
      REAL     COMPSPC(MCGRD,MSC,MCSVAR)                                  40.61mud
...
!
 ...
 7015   FORMAT('                      : ITRIAD ',I12  ,' IQUAD ',I12)
        WRITE(PRINTF,7016) IMUD                                           40.55
 7016   FORMAT('                      : IMUD   ',I12)                     40.55
...
!
 ...
 7325     FORMAT(' JONSWAP (`73)        : GAMMA  ',E12.4)
        ...
        ENDIF
!
        IF (IMUD.EQ.1) THEN                                               40.55
          WRITE(*,*) 'Fluid mud is enabled, refer to:'                  40.55
          ...
          WRITE(*,*) 'power          ',PMUD(10)
        ELSE                                                              40.55
          WRITE (PRINTF, *) 'Fluid mud is off'                            40.55
        ENDIF                                                             40.55
!
        IF (IWCAP.EQ.1) THEN
...
!
        ...
        ENDDO
!       initialise wave lenghts (cumulative EKtot) with mud to 0 for each iteration
!       this is done in parallel within OpenMP environment                40.61mud
!
!-!        DO IP = I1GRD,I2GRD                                              !40.61mud
!-!          COMPDA(IP,JLMR ) = 0.                                          !40.61mud
!-!          COMPDA(IP,JLMI ) = 0.                                          !40.61mud
!-!        ENDDO                                                            !40.61mud
!
...
!
 ...
 2003      FORMAT('  ITRIAD  ',I4,' IBOT    ',I4,' ISURF   ',I4)
           WRITE(PRINTF,2004) IMUD                                        40.55
 2004      FORMAT('  IMUD    ',I4)                                        40.55
           WRITE(PRINTF,*)' -----------------------------------------',
     &                    '----------------------'
        ENDIF
...
!$              END IF                                                    40.31
     ...
     &            CAX1             ,CAY1             ,COMPSPC             40.61mud 40.22
     &                                                                 )
...
!
     ...
     &                   ISLMIN   ,NFLIM    ,NRSCAL   ,                   40.23
     &                   CAX1,CAY1          ,COMPSPC                      40.61mud 33.08
     &                                                 )
!
!************************************************************************
...
!                   6. Fluid mud-induced wave dissipation
!       -----------------------------------------------------------------
...
!FIRST indicates whether SWOMPU is called for 1st time                    40.61mud
...
!
!m!         *** Compute wave number due to presence of mud, will be used for
!m!             1. Adjustment of wave group celerity (SWAPAR => KSCIP1)
!m!             2. Mud source term (SWMUD)
!m! 
...
! BUT ONLY ONCE
	...
	FIRST = .false.
        write(*,*) 'mud version calls always SPROSDM instead of SPROSD'
!
...
!
      ELSE
      write(*,*) 'Calculating mud dispersion relation ...'    
...
!
          ...
          IF ( IMUD.GE.1 ) THEN                                          !40.61mud
          COMPSPC(:,:,JKWVR) = 0.
          COMPSPC(:,:,JKWVI) = 0.
          CALL SWAPARM( COMPDA(1,JDP2), KWAVE, CGO, SPCSIG ,             !40.61mud
     &              COMPSPC(1,1,JKWVR),COMPSPC(1,1,JKWVI))               !40.61mud
          ELSE                                                           !40.61mud
...
!     *** in the gridpoints of the stencil                  ***
      ...
      CALL SWTSTA(110)                                                    40.23
      IF ( IMUD.GE.1 ) THEN                                              !40.61mud
        ! note that only KWAVE(MSC,1) is valid, the dependence of the other points 
        ! KWAVE(MSC,2:end) on the sweep directions is not taken into account, and 
        ! is calculated in SPROSDM. GJdB
        CALL SWAPARM( COMPDA(1,JDP2), KWAVE, CGO, SPCSIG ,               !40.61mud
     &              COMPSPC(1,1,JKWVR),COMPSPC(1,1,JKWVI))               !40.61mud
        ELSE                                                             !40.61mud
...
!     *** directional domain : IDCMIN-1 until IDCMAX+1     ***
      ...
      CALL SWTSTA(113)
!     IF ( IMUD.GE.1               ! whether there is mud in first place !40.61mud
!     &	.AND. PMUD( 9)==1        ! whether mud should affect Cg     = PMUD(9)
!     &	.AND. PMUD(11)==1) THEN  ! whether mud should affect refrac = PMUD(11) ! extend PMUD !
...
!	   DO IS = 1, MSC
!	    DO ID = 1, MDC
!          write(*,'(3I3,5f7.4)') ID,IS,IC,CAX(ID,IS,IC),CAY(ID,IS,IC),
!     &                                    CAS(ID,IS,IC),CAD(ID,IS,IC),
!     &                                    KWAVE(IS,IC)
!	    ENDDO
!	   ENDDO
!	  ENDDO
...
!
     ...
     &  SWMATR(1,1,JDISB)   ,SWMATR(1,1,JDISU)   ,SWMATR(1,1,JDISW)   ,   40.61
     &  SWMATR(1,1,JDISM)   ,                                             40.61
     &  COMPSPC,                                                          !COMPSPC(1,1,JKWVR)  ,COMPSPC(1,1,JKWVI)  ,                        40.61mud
     &  XIS                 ,COMPDA(1,JFRC2)     ,IT                  ,   40.00
     &  COMPDA(1,JMUDL2)    ,                                             40.55
     &  COMPDA(1,JURSEL)    ,LSWMAT(1,1,JABIN)   ,REFLSO                  40.41 40.03
     &                                                                )   30.21
        CALL SWTSTO(117)                                                  40.23

!          write(*,'(4(f11.8,a),2(E11.3,a))') KWAVE (50,1),' ',
!     &                                       CGO   (50,1),'|',
!     &                                       CAX   (50,50,1),' ',
!     &                                       CAY   (50,50,1),'|',
!     &                                       CAD   (50,50,1),' ',
...
!
        ...
        IF (PNUMS(20).LT.100.) CALL PHILIM (AC2, SWMATR(1,1,JAOLD),       40.31 40.16
     &                  CGO, KWAVE,                                       40.00
...
!
 700    CONTINUE                                                          40.07
     ...
     &               SWMATR(1,1,JDIS0)  ,SWMATR(1,1,JDIS1)  ,
     &               SWMATR(1,1,JDISB)  ,COMPDA(1,JDSXB)    ,             40.61
     &               SWMATR(1,1,JDISU)  ,COMPDA(1,JDSXS)    ,             40.61
     &               SWMATR(1,1,JDISW)  ,COMPDA(1,JDSXW)    ,             40.61
     &               SWMATR(1,1,JDISM)  ,COMPDA(1,JDSXM)    ,             40.61
     &               SWMATR(1,1,JLEK1)  ,SPCSIG                           30.72
     &                                                      )
      ! This is not the right place to calculate lmean directly because we are now in
      ! swompu that knows info for only one grid points, and the variables are zero in that
      ! part of the matrix that is not in the current sweep. So we have to use a
      ! cumulative sequential adding method for EKTOT, and add the tail afterwards.

!-!        IF (IMUD.EQ.1) THEN
!-!        CALL ADDLMEAN  (AC2                ,LSWMAT(1,1,JABIN)  ,       !40.61mud
!-!     &                  COMPSPC(KCGRD(1),1:MSC,JKWVR) ,
!-!     &                  COMPDA (1,JLMR   )  , SPCSIG)                  !40.61mud
!-!        CALL ADDLMEAN  (AC2                ,LSWMAT(1,1,JABIN)  ,       !40.61mud
!-!     &                  COMPSPC(KCGRD(1),1:MSC,JKWVI) ,
!-!     &                  COMPDA (1,JLMI   )  , SPCSIG)                  !40.61mud
!-!
!-!        ENDIF

      !  if (ix==105 and (iy==42)) THEN
      !  write(*,*) '----> EKTOTr,EKTOTi in point (ix,iy)',ix,iy
      !  write(*,*) COMPDA(KCGRD(1),JLMR),COMPDA(KCGRD(1),JLMI)
      !  write(*,*) '-AC2-------------------'
      !  write(*,*) AC2(1:MDC, 1:MSC, KCGRD(1))
      !  endif
...
!$OMP MASTER                                                              40.31
      ARR(1) = HSMN2                                                      40.30
      ARR(2) = SMN2                                                       40.30
      ARR(3) = REAL(NINDX)                                                40.30
      CALL SWREDUCE( ARR, 3, SWREAL, SWSUM )                              40.30
      CALL SWREDUCE( NINDX, 1, SWINT, SWSUM )                             40.41
      ARR(3) = REAL(NINDX)                                                40.41
!OMP#ifndef _OPENMP                                                           40.31
      IF (STPNOW()) RETURN                                                40.30
!OMP#endif                                                                    40.31
      HSMN2 = ARR(1) / ARR(3)                                             40.30 30.82
       SMN2 = ARR(2) / ARR(3)                                             40.30 30.82
!$OMP END MASTER                                                          40.31
!$OMP BARRIER                                                             40.31
!
...
!
     ...
     &                     ZELEN      ,SPCDIR     ,ANYWND     ,
     &                     DISSC0     ,DISSC1     ,
     &                     DISBOT     ,DISSRF     ,DISWCP     ,           40.61
     &                     DISMUD     ,                                   40.61
     &                     COMPSPC,                                       !KMUDR      ,KMUDI      ,                      !40.61mud
     &                     XIS        ,FRCOEF     ,IT         ,           30.00
     &                     MUDL2      ,                                   40.55
...
!     40.41, Oct. 04: common blocks replaced by modules, include files removed
!     40.55, Dec. 05: introducing fluid mud-induced dissipation model
...
!   ------------------------------------------------------------
!   If SWMUD is on (IMUD > 0 ) then,
!     Call SWMUD  to compute the source term due to fluid mud
!      dissipation according to Winterwerp et al (2006)
!   ------------------------------------------------------------
...
!     Changed ICMAX to MICMAX, since MICMAX doesn't vary over gridpoint   40.22
      ...
      REAL  :: PLMUD (MDC,MSC,NPTST)                                      40.55
      ...
      REAL  :: DISSC0(MDC,MSC)
      REAL  :: DISSC1(MDC,MSC)
      REAL  :: DISBOT(MDC,MSC)                                            40.61
      REAL  :: DISSRF(MDC,MSC)                                            40.61
      REAL  :: DISWCP(MDC,MSC)                                            40.61
      REAL  :: DISMUD(MDC,MSC)                                            40.61
!      REAL  :: KMUDR (MDC,MSC) ! KMUDR (MSC) sufficient                 !40.61mud
!      REAL  :: KMUDI (MDC,MSC) ! KMUDI (MSC) sufficient                 !40.61mud
!-!
      REAL     COMPSPC(MCGRD,MSC,MCSVAR)                                 !40.61mud
!-!
...
!
      DO ISC = 1, MSC
        DO IDC = 1, MDC
          DISSC0(IDC,ISC) = 0.
          DISSC1(IDC,ISC) = 0.
          DISBOT(IDC,ISC) = 0.                                            40.61
          DISSRF(IDC,ISC) = 0.                                            40.61
          DISWCP(IDC,ISC) = 0.                                            40.61
          DISMUD(IDC,ISC) = 0.                                            40.61
!          KMUDR (IDC,ISC) = 0.                                          !40.61mud
!          KMUDI (IDC,ISC) = 0.                                          !40.61mud
        ENDDO
        ! KMUDR (    ISC) = 0.      ! sufficient                         !40.61mud
        ! KMUDI (    ISC) = 0.      ! sufficient                         !40.61mud
      ENDDO

!     KMUDR(KCGRD(1),:) = 0.                                              40.61mud calculated one level up !!!
!     KMUDI(KCGRD(1),:) = 0.                                              40.61mud calculated one level up !!!
!
...
!
      CALL SWTSTA(138)                                                    40.55
      IF ( IMUD.GE.1 ) THEN                                               40.55
!
!     *** wave-mud interactions ***                                       40.55
!
!        *** energy dissipation according to Winterwerp et al (2006)      40.55
!
         CALL SWMUDD(DEP2   ,MUDL2    ,IMATDA ,SPCSIG    ,                40.55
     &               IDCMIN ,IDCMAX   ,PLMUD  ,ISSTOP    ,                40.55
     &               DISSC1 ,DISMUD   ,IX     ,IY        ,                40.61 40.55
     &               COMPSPC(1,1,JKWVR),                                 !40.61mud
     &               COMPSPC(1,1,JKWVI),PLBTFR,DISBOT,CGO)               !40.61mud
!-!  &               KWAVE  ,IX     ,IY       ,KMUDR  ,KMUDI)            !40.61mud
!
        ! all sweeps use same array, so always a part of the array is zero !!!
        !write(*,*) 'kmudr at ', IX,IY,' for all frequencies'
        !write(*,*) 1,KMUDR(KCGRD(1),:)
        !write(*,*) 'kmudi at ', IX,IY,' for all frequencies'
        !write(*,*) 1,KMUDI(KCGRD(1),:)

      END IF                                                              40.55
      CALL SWTSTO(138)                                                    40.55
!
...
!
     ...
     &                ISSTOP  ,DISSC0  ,DISSC1  ,DISSRF                   40.61 30.21
     &                                                              )
!
      END IF
...
!
     ...
     &                            CGO     ,UFRIC   ,                      40.53
     &                            DEP2    ,DISSC1  ,DISSC0  ,DISWCP  )    40.61 40.12
