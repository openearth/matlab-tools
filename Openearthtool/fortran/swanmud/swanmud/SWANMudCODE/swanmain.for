...
!     COMPSPC:    array containing various data depending on grid         40.61G
!                 and frequency space                                     40.61G
...
!
      ...
      REAL   , ALLOCATABLE :: COMPSPC(:,:,:)                              40.61G      
!
...
!       --- initialisation of necessary geographic arrays containing      40.61G
!           spectral info                                                 40.61G

        IF ( MCSVAR.NE.0 ) THEN                                           40.61G
           IF(.NOT.ALLOCATED(COMPSPC))                                    40.61G
     &                    ALLOCATE(COMPSPC(MCGRD,MSC,MCSVAR),STAT=ISTAT)  40.61G
           IF ( ISTAT.NE.0 ) THEN                                         40.61G
              CHARS(1) = NUMSTR(ISTAT,RNAN,'(I6)')                        40.61G
              CALL TXPBLA(CHARS(1),IF1,IL1)                               40.61G
              MSGSTR =                                                    40.61G
     &         'Allocation problem: array COMPSPC and return code is '//  40.61G
     &         CHARS(1)(IF1:IL1)                                          40.61G
              CALL MSGERR ( 4, MSGSTR )                                   40.61G
              RETURN                                                      40.61G
           END IF                                                         40.61G
           COMPSPC = 0.                                                   40.61G
        ELSE                                                              40.61G
           IF(.NOT.ALLOCATED(COMPSPC)) ALLOCATE(COMPSPC(0,0,0))           40.61G
        ENDIF                                                             40.61G
...
!               step before performing output request                     !40.61SM
     ...
     &                    COMPDA (1, JMUDL2),SPCSIG,MSC,1,1,              !40.61SM
...
!     --- deallocate all allocated arrays                                 40.31
      ...
      IF (ALLOCATED(COMPSPC)) DEALLOCATE(COMPSPC)                         40.61G
...
!     40.61G,Oct. 06: output variable WLENMR and WLENMI added
!
!  2. Purpose
!
...
! 13. Source text
!
      VERTXT = 'mudspecial'                                               40.61G
      VERNUM = 40.61                                                      40.61G
      WRITE (VERTXT, '(F5.2)') VERNUM                                     40.03
      CALL BUGFIX ('A')                                                  !40.61G
      CALL BUGFIX ('.')                                                  !40.61G
      CALL BUGFIX ('m')                                                  !40.61G
      CALL BUGFIX ('u')                                                  !40.61G
      CALL BUGFIX ('d')                                                  !40.61G
      CALL BUGFIX ('.')                                                  !40.61G
      CALL BUGFIX ('I')                                                  !40.61G
      CALL BUGFIX ('M')                                                  !40.61G
      CALL BUGFIX ('S')                                                  !40.61G
      CALL BUGFIX ('L')                                                  !40.61G
!
...
!   MUD  [alpha]  [rhom]  [rho0]  [nu]  [layer] [disperr] [disperi] [source] [cg] [power]
      PMUD( 1)   = 1.      ![alpha]                                       40.55
      PMUD( 2)   = 1300.   ![rhom]                                        40.55
      PMUD( 3)   = RHO     ![rho0]                                        40.55
      PMUD( 4)   = 0.0027  ![nu]                                          40.55
      PMUD( 5)   = 0.0     ![layer]                                       40.55
      PMUD( 6)   = 3       ![disperr]-1=MUDFile 0=Guo      1=Gade1958, 2=DeWit, !40.61G
      PMUD( 7)   = 3       ![disperi] 3=Delft   4=Dalr1978, 5=Ng                !40.61G
      PMUD( 8)   = 3       ![source]  0=none,1=Gade1958, 3=Delft,               !40.61G
      PMUD( 9)   = 1       ![cg]      0=no infl mud on prop, 1=infl mud on prop !40.61G
...
!     added for air-sea temp. diff.:
      JASTD2= 1                                                           40.03
      JASTD3= 1                                                           40.03
!     added for fluid mud layer                                           40.55
      JMUDL2= 1                                                           40.55
      JMUDL3= 1                                                           40.55
!     added for wave lengths mud                                          40.61G
      JLMR  = 1                                                          !40.61G
      JLMI  = 1                                                          !40.61G
!
...
!     pointers in array COMPSPC                                           40.61G
!
      MCSVAR = 0                                                          40.61G
!     subarray sequence number 1 is used only for unused subarrays        40.61G
      JKWVR = 1                                                           40.61G
      JKWVI = 1                                                           40.61G
!
!     ***** test output control *****
...
!       next two lines only for nonstat water level
      ...
      ENDIF
      IF (VARMUD) THEN                                                    40.55
        COMPDA(1,JMUDL2) = 0.                                             40.55
        COMPDA(1,JMUDL3) = 0.                                             40.55
      ENDIF
...
!
          ...
          ENDIF
!
!     ***** compute fluid mud layer *****                                 40.55
!
          IF (VARMUD) THEN
             XMUD = SVALQI (XP, YP, 11, MUDLF, 1 ,IX ,IY)                 40.55
             COMPDA(INDX,JMUDL2) = XMUD                                   40.55
             COMPDA(INDX,JMUDL3) = XMUD                                   40.55
          ENDIF
!
   10   CONTINUE
   20 CONTINUE
...
!
!!!!    -------------------------------------------------------------------------
!      IF ( IMUD.GE.1 .AND. IBOT.GE.1 ) THEN                               40.55
!        CALL MSGERR(1,                                                  !40.61G
!    &           'Bottom friction is de-activated in case of fluid mud')  40.55 40.61G
!         IBOT = 0
!         CALL MSGERR(1,                                                  !40.61G
!     &           'DISBOT=0 where (DISMUD > DISBOT), and vv..')           !40.61G
!
!      END IF
!
!!!	THIS PART HAS TO BE REMOVED TO RUN SWAN PROPERLY   !!!
!!!    -------------------------------------------------------------------------
...
!     field 11: field containing fluid mud layer
      IF (IFLDYN(11) .EQ. 1) THEN
        CALL FLFILE (11, 0, MUDLF, 0.,
     &               0, JMUDL2, JMUDL3, 0, 0, 0,
     &               1., 0.,
     &               COMPDA, XCGRID, YCGRID,
     &               KGRPNT, IERR)
        IF (STPNOW()) RETURN
...
!          unformatted reading
           IF (DORDER.LT.0) THEN                                          40.55
              READ (NDSD,END=940,ERR=920)
     &               ((BAUX1(IANG,IFRE),IANG=NANG,1,-1),IFRE=1,1)
              READ (NDSD,END=930,ERR=920)
     &               ((BAUX1(IANG,IFRE),IANG=NANG,1,-1),IFRE=2,NFRE)
           ELSE
              READ (NDSD,END=940,ERR=920)
     &               ((BAUX1(IANG,IFRE),IANG=1,NANG),IFRE=1,1)
              READ (NDSD,END=930,ERR=920)
     &               ((BAUX1(IANG,IFRE),IANG=1,NANG),IFRE=2,NFRE)
           ENDIF
        ELSEIF ((.NOT.UNFORM).AND.(BTYPE.EQ.'WW3N')) THEN                 40.05
!
!         WW3 reading
...
!         format reading (except WW3)
          IF (DORDER.LT.0) THEN                                           40.31
            READ (NDSD,*,END=940,ERR=920)                                 40.31
     &             ((BAUX1(IANG,IFRE),IANG=NANG,1,-1),IFRE=1,1)           40.31
            READ (NDSD,*,END=930,ERR=920)                                 40.31
     &             ((BAUX1(IANG,IFRE),IANG=NANG,1,-1),IFRE=2,NFRE)        40.31
          ELSE                                                            40.31
            READ (NDSD,*,END=940,ERR=920)                                 40.31
     &             ((BAUX1(IANG,IFRE),IANG=1,NANG),IFRE=1,1)              40.31
            READ (NDSD,*,END=930,ERR=920)                                 40.31
     &             ((BAUX1(IANG,IFRE),IANG=1,NANG),IFRE=2,NFRE)           40.31
          ENDIF                                                           40.31
        ENDIF
!
        IF (ITEST.GE.120) THEN
