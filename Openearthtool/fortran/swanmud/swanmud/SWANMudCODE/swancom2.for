...
!     SWMUD   (Fluid mud-induced wave dissipation)                        40.55
...
!     DISBOT    2D    Total bottom friction dissipation for output purposes
...
!
              ...
              DISBOT(ID,IS) = DISBOT(ID,IS) + SBOTEO                      40.61
 690        CONTINUE
          END IF
 700    CONTINUE
!
...
!
     ...
     &                  ISSTOP  ,DISSC0  ,DISSC1  ,DISSRF                 40.61 30.81 30.21
     &                                                              )
!
!****************************************************************
!
...
!                       (meant for output)
!     DISSRF  output:   Total surf dissipation for output purposes
...
!
      DO 101 IS = 1, ISSTOP
        DO 100 IDDUM = IDCMIN(IS), IDCMAX(IS)
          ...
          DISSRF(ID,IS) = DISSRF(ID,IS) + REAL(SURFA1-SURFA0)             40.61
 100    CONTINUE
 101  CONTINUE
!
!     *** test output ***
...
!
     ...
     &                   CGO     ,UFRIC   ,                               40.53
     &                   DEP2    ,DISSIP  ,DISIMP  ,DISWCP  )             40.61 40.12
!
!****************************************************************
!
      USE SWCOMM3                                                         40.41
...
!     DISSIP: Array containing dissipation
!     DISIMP: Array containing implicit part of the dissipation
!     DISWCP: Total whitecapping dissipation for output purposes
...
!     Changed ICMAX to MICMAX, since MICMAX doesn't vary over gridpoint   40.22
      ...
      REAL, INTENT(IN OUT) :: DISWCP(MDC,MSC)                             40.61
...
!                                                                         40.53
          ...
          P = 0.5*PWCAP(10)*(1. + TANH( 10.*( (B/PWCAP(12))**0.5 - 1.)))  40.53
!                                                                         40.53
!  Calculate WCAP(IS) from B(k) and P                                     40.53
!                                                                         40.53
          STP_OV = KM_WAM * SQRT(ETOT)                                    40.53
          WCAP(IS) = PWCAP(1)*(B/PWCAP(12))**(P/2.) *                     40.53
...
!
          DO IDDUM = IDCMIN(IS), IDCMAX(IS)
            ...
            DISWCP(ID,IS) = DISWCP(ID,IS) + WCAP(IS)-WCIMPL(IS)           40.61
...
!                                                                         40.30
          DO IDDUM = IDCMIN(IS), IDCMAX(IS)                               40.30
            ...
            DISSIP(ID,IS) = DISSIP(ID,IS) + CUMSTP(IS,ID)                 40.30
            DISWCP(ID,IS) = DISWCP(ID,IS) + CUMSTP(IS,ID)                 40.61
...
!
              ...
              DMUD = DMUD + PLMUD (ID,IS,IPTST) * SIG2AC                  40.55
...
!
                ...
                DMUD = DMUD + PLMUD (ID,IS,IPTST) * SIGACT                40.55
...
!             factor DDIR is due to integration over directions
     ...
     &                          BTFR*FAC, DMUD*FAC, WBRK*FAC, TRIA*FAC,   40.55 40.00
     &                          NL4*FAC                                   40.00
...
!
     ...
     &                              + PLWNDB(ID,IS,IPTST) * SIGACT
                ...
                PLMUD (ID,IS,IPTST) = PLMUD (ID,IS,IPTST) * SIGACT        40.55
...
!               PLWNDB is used temporarily for energy density             40.00
            ...
            CALL WRSPEC (IFS2D, PLMUD (1,1,IPTST))                        40.55
