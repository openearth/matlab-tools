!****************************************************************
!
subroutine NewtonRoot(Func,FuncDer,ERRABS,ERRREL,kk,ITMAX,xxi,Info)
!
!****************************************************************
        IMPLICIT NONE
!  This subroutine is witten based on the algorithm presented in
!  Press et al 1992 Numerical Recipes, The Art of Scientific Computing
!  page: 387
!  Modified for complex functions
!
!  written by: Seyed Mostafa Siadatmousavi
!
      complex*16            :: kk
      integer               :: Info, ITMAX
      complex*16            :: xxi
      real                  :: ERRABS , ERRREL
      integer               :: i, NN,NK,NG
      complex(8)            :: xnext, xc, ynext, temp
      INTERFACE
         FUNCTION Func(r)
          complex(8):: Func
          complex(8), INTENT(IN) :: r
        END FUNCTION Func
        FUNCTION FuncDer(r)
          complex(8):: FuncDer
          complex(8), INTENT(IN) :: r
        END FUNCTION FuncDer
      END INTERFACE
!     Func  :    The function we need its zeros
!     ERRABS:    If (ABS(F(Z)).LE.ERRABS)then Z is accepted as a zero.
!     ERRREL:    A zero is accepted if the difference in two successive
!                   approximations to this zero is within ERRREL
!     kk    :    Initial value
!     ITMAX :    Maximum number of itterations
!     xxi   :    Potential solution
!     Info  :    >0 successful, 0: more itteration may help, <0: failed
!                    to converge
!     Newton Method
      xc = kk  
      Do i=1,ITMAX
        temp =funcDer(xc)
	if ( abs(temp) .lt. ERRABS ) then
          exit
        else
	  xnext=xc-Func(xc)/temp
          ynext=Func(xnext)
          if ( ( abs( xnext - xc ) .lt. ERRREL ) .or.                    &
                              ( abs( ynext ) .lt.  ERRABS ) ) then
             exit
           else
             xc = xnext
           endif
        endif
      EndDo
      if ( i .eq. ITMAX ) then
        Info =  0
      elseif (abs( ynext ) .lt.  ERRABS) then
        Info =  i
      else
        Info = -i
     endif
      xxi = xnext
end subroutine NewtonRoot



!****************************************************************
!
subroutine MullerNewton(Func,FuncDer,ERRABS,ERRREL,kk,ITMAX,xxi,Info)
!
!****************************************************************
!
        IMPLICIT NONE
!  This subroutine is witten based on the algorithm presented in
!  Press et al 1992 Numerical Recipes, The Art of Scientific Computing
!  page: 364 
!  This subroutine is written to replace the DZANLY from 
!  commercial library IMSL
!
!  written by: Seyed Mostafa Siadatmousavi
!
      complex*16            :: kk
      integer               :: i,Info, ITMAX
      complex*16            :: xxi
      real                  :: ERRABS , ERRREL
      complex(8)            :: x0, x1, x2, y0, y1, y2, x3, y3
      complex(8)            :: a, b, c, q , den1, den2, dis
      INTERFACE
         FUNCTION Func(r)
          complex(8):: Func
          complex(8), INTENT(IN) :: r
        END FUNCTION Func
        FUNCTION FuncDer(r)
          complex(8):: FuncDer
          complex(8), INTENT(IN) :: r
        END FUNCTION FuncDer
      END INTERFACE
!     Func  :    The function we need its zeros
!     ERRABS:    If (ABS(F(Z)).LE.ERRABS)then Z is accepted as a zero.
!     ERRREL:    A zero is accepted if the difference in two successive
!                   approximations to this zero is within ERRREL
!     kk    :    Initial value
!     ITMAX :    Maximum number of itterations
!     xxi   :    Potential solution
!     Info  :    >0 successful, 0: more itteration may help, <0: failed
!                    to converge
!     Mullers Method
      x0 = kk
!     initial values (used Newton method for second and third initial values)
      y0  = Func(x0);    
      x1 = x0- y0/FuncDer(x0)
      y1 = Func(x1);    
      x2 = x1- y1/FuncDer(x1)
      y2 = Func(x2);    
      Do i=1,ITMAX
        q=( x2 - x1 )/( x1 - x0 )
        a = q * y2 - q * ( 1 + q ) * y1 + q * q * y0
        b = ( 2 * q + 1 ) * y2 - ( 1+q ) * ( 1 + q ) * y1 + q * q * y0
        c = ( 1 + q ) * y2 ;
        dis= b * b - 4 * a * c
        den1 = ( b + sqrt ( dis ) );
        den2 = ( b - sqrt ( dis ) )
        if ( abs( den1 ) .lt. abs( den2 )  ) then
          den1 = den2
        endif
        if ( abs( den1 ) .ge. 1e-9 ) then
           x3 = x2 - (x2 - x1) * ( 2 * c / den1 )
           y3 = Func( x3 )
           x0 = x1
           y0 = y1
           x1 = x2
           y1 = y2
           if ( ( abs( x3 - x2 ) .lt. ERRREL ) .or.                    &
                                      ( abs( y3 ) .lt.  ERRABS ) ) then
             exit
           else
             x2 = x3
             y2 = y3
           endif
        else
           exit
        endif
      EndDo
      if ( i .eq. ITMAX ) then
        Info =  0
      elseif (abs( y3 ) .lt.  ERRABS) then
        Info =  i
      else
        Info = -i
     endif
      xxi = x3
end subroutine MullerNewton

!****************************************************************
!
subroutine Muller(Func,ERRABS,ERRREL,NK,NN,NG,kk,ITMAX,xxi,Info)
!
!****************************************************************
!  This subroutine is witten based on the algorithm presented in
!  Press et al 1992 Numerical Recipes, The Art of Scientific Computing
!  page: 364 
!  This subroutine is written to replace the DZANLY from 
!  commercial library IMSL
!
!  written by: Seyed Mostafa Siadatmousavi
!
      complex*16            :: kk
      integer               :: Info, ITMAX
      complex*16            :: xxi
      real                  :: ERRABS , ERRREL
      integer               :: i
      complex(8)            :: x0, x1, x2, y0, y1, y2, x3, y3, xc
      complex(8)            :: a, b, c, q , den1, den2, dis
      INTERFACE
        FUNCTION Func(r)
          complex(8):: Func
          complex(8), INTENT(IN) :: r
        END FUNCTION Func
      END INTERFACE
!     Func  :    The function we need its zeros
!     ERRABS:    If (ABS(F(Z)).LE.ERRABS)then Z is accepted as a zero.
!     ERRREL:    A zero is accepted if the difference in two successive
!                   approximations to this zero is within ERRREL
!     NK    :    The number of previously known zeros (Not used in this
!                   algorithm)
!     NN    :    The number of new zeros to be found (Equals to 1 in
!                   this algorithm))
!     NG    :    The number of initial guesses provided (Equals to 1 in
!                   this algorithm))
!     kk    :    Initial value
!     ITMAX :    Maximum number of itterations
!     xxi   :    Potential solution
!     Info  :    >0 successful, 0: more itteration may help, <0: failed
!                    to converge
      xxi=0;
!     Mullers Method with very bad initializations!
!     This implementation is only used for DKDALR1978; and may be replaced with MullerNewton soon
      xc = kk
      x0 = xc - 1
      x1 = xc
      x2 = xc + 1
      y0 = Func( x0 )
      y1 = Func( x1 )
      y2 = Func( x2 )
      Do i=1,ITMAX
        q=( x2 - x1 )/( x1 - x0 )
        a = q * y2 - q * ( 1 + q ) * y1 + q * q * y0
        b = ( 2 * q + 1 ) * y2 - ( 1+q ) * ( 1 + q ) * y1 + q * q * y0
        c = ( 1 + q ) * y2 ;
        dis= b * b - 4 * a * c
        den1 = ( b + sqrt ( dis ) );
        den2 = ( b - sqrt ( dis ) )
        if ( abs( den1 ) .lt. abs( den2 )  ) then
          den1 = den2
        endif
        if ( abs( den1 ) .ge. 1e-9 ) then
           x3 = x2 - (x2 - x1) * ( 2 * c / den1 )
           y3 = Func( x3 )
           x0 = x1
           y0 = y1
           x1 = x2
           y1 = y2
           if ( ( abs( x3 - x2 ) .lt. ERRREL ) .or.                    &
                                      ( abs( y3 ) .lt.  ERRABS ) ) then
             exit
           else
             x2 = x3
             y2 = y3
           endif
        else
           exit
        endif
      EndDo
      if ( i .eq. ITMAX ) then
        Info =  0
      elseif (abs( y3 ) .lt.  ERRABS) then
        Info =  i
      else
        Info = -i
     endif
      xxi = x3
end subroutine Muller
