...
!        40.41, Aug 04: transmission coeff (OBREDF) removed
!        40.41, Oct. 04: common blocks replaced by modules, include files removed
!
!     2. PURPOSE
!
...
!
     ...
     &              + (RDY(1) + RDY(2)) * CAY(ID,IS,1)
              ...
              WEIG2 = 1. - WEIG1
!
              FAC_A = WEIG1 * AC2(ID,IS,KCGRD(2))                         40.41 40.00
              FAC_B = WEIG2 * AC2(ID,IS,KCGRD(3))                         40.41 40.00
!
              IF (ACUPDA)                                                 40.13
...
!       *** call KSCIP1 to compute KWAVE and CGO ***
     ...
     &                 CGO(1,IC), N, ND)                                  40.41
!	    IS = 50
!         write(*,'(i3,1(f7.4,a),f7.5,a)') IS,KWAVE (IS,1),' ',
!     &                                        CGO   (IS,1)
      ENDIF
!
...
!     The same holds for DISBOT, DISSRF, DISWCP and DISMUD for bottom 
!     friction-, surf-, whitecapping-, and fluid mud dissipation respectively.
!
! 12. Structure
!
!     -------------------------------------------------------------
...
!
     ...
     &         DISBOT(MDC,MSC)  ,DSXBOT(MCGRD)      ,                     40.61
     &         DISSRF(MDC,MSC)  ,DSXSRF(MCGRD)      ,                     40.61
     &         DISWCP(MDC,MSC)  ,DSXWCP(MCGRD)      ,                     40.61
     &         DISMUD(MDC,MSC)  ,DSXMUD(MCGRD)      ,                     40.61
...
!
            DSXBOT(KCGRD(1)) = DSXBOT(KCGRD(1)) + DSDD * DISBOT(IDC,ISC)  40.61
     &                                           * AC2(IDC,ISC,KCGRD(1))  40.61
            DSXSRF(KCGRD(1)) = DSXSRF(KCGRD(1)) + DSDD * DISSRF(IDC,ISC)  40.61
     &                                           * AC2(IDC,ISC,KCGRD(1))  40.61
            DSXWCP(KCGRD(1)) = DSXWCP(KCGRD(1)) + DSDD * DISWCP(IDC,ISC)  40.61
     &                                           * AC2(IDC,ISC,KCGRD(1))  40.61
            DSXMUD(KCGRD(1)) = DSXMUD(KCGRD(1)) + DSDD * DISMUD(IDC,ISC)  40.61
     &                                           * AC2(IDC,ISC,KCGRD(1))  40.61
!
          ENDIF
  90    CONTINUE
...
!     --- loop over all X-connections
     ...
     &          .AND. DEP2(INDR).GT.DEPMIN) THEN
               IF ( OPTG.EQ.1 ) THEN
                  TMP = (DIFPARAM(INDR) - DIFPARAM(INDL))/(2.*DX)
               ELSE
                  DXLOC = 0.5*(XCGRID(IX+1,IY)-XCGRID(IX-1,IY))
                  DXLOC = ABS(DXLOC)
                  IF (EQREAL(DXLOC,0.)) DXLOC=0.01
                  TMP = (DIFPARAM(INDR) - DIFPARAM(INDL))/
     &                 (2.*DXLOC)
               END IF
...
!     --- loop over all Y-connections
     ...
     &          .AND. DEP2(INDT).GT.DEPMIN) THEN
               IF ( OPTG.EQ.1 ) THEN
                  TMP = (DIFPARAM(INDT) - DIFPARAM(INDB))/(2.*DY)
               ELSE
                  DYLOC = 0.5*(YCGRID(IX,IY+1)-YCGRID(IX,IY-1))
                  DYLOC = ABS(DYLOC)
                  IF (EQREAL(DYLOC,0.)) DYLOC=0.01
                  TMP = (DIFPARAM(INDT) - DIFPARAM(INDB))/
     &                 (2.*DYLOC)
               END IF
