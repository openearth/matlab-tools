...
!     PARAMETER (MXOUTL=360)
      PARAMETER (MXOUTL=480) !increased from 360 to 480                   40.61mud
!
!  6. Local variables
!
!     NUMDEC
...
!           write (short) names of output quantities
            ...
            DO  JVAR = 1, NVAR
              ...
              ELSE
                LFIELD = 11
              ENDIF
!           Check for width OUTLIN                                        40.61mud
            IF (LINKAR+LFIELD > MXOUTL) THEN                             !40.61mud
            write(*,*) 'SWAN OUTPUT ERROR:  output line width > ',       !40.61mud
     &      'max allowed characters per line (',MXOUTL,                  !40.61mud
     &      ') for parameter ',JVAR ,'of', NVAR,'. Reduce # parameters.' !40.61mud
            ENDIF                                                        !40.61mud
...
!                                                                      *
     ...
     &                   SPCSIG, SPCDIR, DEP2, KGRPNT, IONOD,COMPSPC)     40.31 30.72 40.61mud
!                                                                      *
!***********************************************************************
!
      USE OCPCOMM2                                                        40.41
...
!         full 2-D spectrum
 ...
 104        FORMAT (E14.4, T41, A)
     ...
     &                        'variance densities in m2/Hz/degr'          40.00
        ...
        ELSE
!         1-D spectrum
          IF (imud==1) THEN                                               40.61mud
          WRITE (NREF, 132) 5                                             40.61mud
          ELSE
          WRITE (NREF, 132) 3                                             40.61mud
          ENDIF                                                           40.61mud
!         QUANT # 1
          IF (INRHOG.EQ.1) THEN
...
!         QUANT # 4
          ...
          WRITE (NREF, 104)  0.    ,'exception value'                     40.61mud CHANGE TO OVEXCV(IVTYPE) IVTYPE=58 !!!!
!         QUANT # 5
          WRITE (NREF, 102) 'KIMAG','Imaginary wave number 2L fluid mud'  40.61mud
          WRITE (NREF, 102) 'rad/m','unit'                                40.61mud
          WRITE (NREF, 104)  0.    ,'exception value'                     40.61mud CHANGE TO OVEXCV(IVTYPE) IVTYPE=59 !!!!
          ENDIF
        ENDIF
      ENDIF
!
...
!
     ...
     &               OFAC        ,KGRPNT     ,IERR        ,COMPSPC)       40.00 40.61mud
!
        IF (IERR.GT.0) THEN
...
!             write frequency spectra to file
              if (IMUD==1) THEN                                           40.61mud
              WRITE (NREF, 117) (ACLOC(IFR+JJ*MSC), JJ=0,4)               40.00
 117          FORMAT (E12.4, 2F7.1,E12.4,E12.4)                           40.61mud
              ELSE                                                        40.61mud
              WRITE (NREF, 116) (ACLOC(IFR+JJ*MSC), JJ=0,2)               40.00
 116          FORMAT (E12.4, 4F7.1)                                       40.61mud
              ENDIF                                                       40.61mud
            ENDDO
          ENDIF
        ENDIF
...
!                                                                      *
     ...
     &                   DEP       ,DEP2      ,UX        ,                40.00
     &                   UY        ,ECOS      ,ESIN      ,
     &                   OFAC      ,KGRPNT    ,IERR      ,COMPSPC)        30.21 40.61mud
!                                                                      *
!***********************************************************************
!
      USE OCPCOMM4                                                        40.41
...
!
      DO 110 ID = 1, MDC
        IF (ICUR.GT.0 .AND. OTYPE.LT.0) THEN ! With currents, abs frequencies
          UDIR = UX * ECOS(ID) + UY * ESIN(ID)
        ENDIF
        DO 100 ISIGM = 1, MSC
!
!         interpolate local action density
!         mud:   also local wave numbers
!
          ACLL = 0.
          !IF (IMUD==1) THEN
          !KRLL = 0.
...
!
          IF (RF11.GT.1.E-10) THEN
            ...
            !IF (IMUD==1) THEN
            !KRLL =        RF11 * COMPSPC     (IND11,ISIGM,JKWVR)
            !KILL =        RF11 * COMPSPC     (IND11,ISIGM,JKWVI)
            !ENDIF
          ENDIF
          IF (RF12.GT.1.E-10) THEN
            ACLL = ACLL + RF12 * AC2(ID,ISIGM,IND12)
            !IF (IMUD==1) THEN
            !KRLL = KRLL + RF12 * COMPSPC     (IND12,ISIGM,JKWVR)
            !KILL = KILL + RF12 * COMPSPC     (IND12,ISIGM,JKWVI)
            !ENDIF
          ENDIF
          IF (RF21.GT.1.E-10) THEN
            ACLL = ACLL + RF21 * AC2(ID,ISIGM,IND21)
            !IF (IMUD==1) THEN
            !KRLL = KRLL + RF21 * COMPSPC     (IND21,ISIGM,JKWVR)
            !KILL = KILL + RF21 * COMPSPC     (IND21,ISIGM,JKWVI)
            !ENDIF
          ENDIF
          IF (RF22.GT.1.E-10) THEN
            ACLL = ACLL + RF22 * AC2(ID,ISIGM,IND22)
            !IF (IMUD==1) THEN
            !KRLL = KRLL + RF22 * COMPSPC     (IND22,ISIGM,JKWVR)
            !KILL = KILL + RF22 * COMPSPC     (IND22,ISIGM,JKWVI)
            !ENDIF
          ENDIF
...
!
     ...
     &                                    ) THEN  ! No currents, abs frequencies
!
...
!             1-D spectrum of rel. frequency
     ...
     &           ISIGM, ECLL, (ACLOC(ISIGM+JJ*MSC),JJ=0,2)
              ...
              !IF (IMUD==1) THEN
              !ACLOC(ISIGM+3*MSC) = KRLL ! actually there is no need to do this inside theta loop
              !ACLOC(ISIGM+4*MSC) = KILL ! actually there is no need to do this inside theta loop
              !IF (ITEST.GE.250 .OR. IOUTES .GE. 40) WRITE (PRTEST, 83)
     &        !   ISIGM, (ACLOC(ISIGM+JJ*MSC),JJ=0,4)
              !ENDIF
...
!
            ...
            !IF (IMUD==1) THEN                                            40.61mud            
            !IF (ISIGM==1 .and. ID==1) THEN
            !write(*,*) 'Neglected effect mud on k on presence currents'  40.61mud            
            !ENDIF
            !...                                                          40.61mud
...
!             swap the two values
  ...
  90        CONTINUE
          ENDIF
 100    CONTINUE ! DO 100 ISIGM = 1, MSC
 ...
 110  CONTINUE ! DO 110 ID = 1, MDC

!      MUD (outside directional loop, and outside ICUR flag)
!      Only for 1D spectra !!
! ------------------------------------

        IF (IMUD==1 . and. ABS(OTYPE).EQ.1) THEN
        if (FIRST) then
        write(*,*) 'Neglected effect mud on k on presence currents'      !40.61mud            
        FIRST = .false.
        endif
...
!
          ...
          ENDIF
            ...
            IF (ITEST.GE.250 .OR. IOUTES .GE. 40) WRITE (PRTEST, 83)
     &         ISIGM, (ACLOC(ISIGM+JJ*MSC),JJ=0,4)
        ENDDO ! DO 100 ISIGM = 1, MSC
        ENDIF ! IF (IMUD==1) THEN
! ------------------------------------
