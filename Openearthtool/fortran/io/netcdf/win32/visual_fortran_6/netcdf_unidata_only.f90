...
program netcdfTest
  ...
  integer, dimension(numFrTimes) :: frTimeVals
  real fillVal; !use typeSizes with real(kind = FourByteReal) needed when setting compiler option /real_size:64 to prevent " NetCDF: Not a valid data type or _FillValue type mismatch" error upon execution
  real, dimension(2) :: validRange;
  ...
  call check(nf90_put_att(ncFileID, nf90_global, "history", &
                     "created with $Id: netcdf_unidata_only.f90 5826 2012-03-02 11:19:00Z boer_g $ $HeadURL: https://svn.oss.deltares.nl/repos/openearthtools/trunk/fortran/io/netcdf/win32/visual_fortran_6/netcdf_unidata_only.f90 $"))
