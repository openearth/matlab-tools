...
DF_Sens_NCDF <- function(file_sens,workdir){
  ...
  value         = get.var.ncdf(nc=FS,varid="ph")
...
data_tot_FB <- data_total_FB[data_total_FB$value_sens >= 0 & data_total_FB$value_sens <= 14,] ## remove values outside range
...
data_tot_SF   <- data_total_SF[data_total_SF$value_sens >= 0 & data_total_SF$value_sens <= 14,] ## remove values outside range
...
data_tot_CTD   <- data_total_CTD[data_total_CTD$value_sens >= 0 & data_total_CTD$value_sens <= 14,] ## remove values outside range
...
#in time
ggplot(data2sub,aes(time,value))+geom_point()+facet_wrap(~Locatie)+ylim(7,9)


#scatterplots per year and location                  
p<-ggplot(data_tot_SF,aes(value_mwtl, value_sens,color=year))
p + geom_point() +ylim(7,9)+xlim(7,9)+ 
  labs(xlab="values MWTL", ylab="values SF", title="pH SF") + facet_wrap(~location)

p<-ggplot(data_tot_FB,aes(value_mwtl, value_sens,color=year))
p + geom_point()  + scale_colour_brewer(palette="Accent") +ylim(7,9)+xlim(7,9)+ 
  labs(xlab="values MWTL", ylab="values FB", title=" ph FB") + facet_wrap(~location)

p<-ggplot(data_tot_CTD,aes(value_mwtl, value_sens,color=year))
p + geom_point()  +ylim(7,9)+xlim(7,9)+ 
  labs(xlab="values MWTL", ylab="values CTD", title=" ph CTD") + facet_wrap(~location)

data_tot_SF2<-data_tot_SF[data_tot_SF$value_sens >= 7 & data_tot_SF$value_sens <= 9 & data_tot_SF$value_mwtl >= 7 & data_tot_SF$value_mwtl <= 9,]
data_tot_FB2<-data_tot_FB[data_tot_FB$value_sens >= 7 & data_tot_FB$value_sens <= 9 & data_tot_FB$value_mwtl >= 7,]
data_tot_CTD2<-data_tot_CTD[data_tot_CTD$value_sens >= 7 & data_tot_CTD$value_sens <= 9 & data_tot_CTD$value_mwtl >= 7,]

##plot all with regression
cor.test(data_tot_SF$value_sens,data_tot_SF2$value_mwtl)
p<-ggplot(data_tot_SF,aes(value_mwtl, value_sens))
p + geom_point() +  
  labs(xlab="values MWTL", ylab="values SF", title=" pH SF")+
  stat_smooth(aes(value_mwtl, value_sens),method = "lm") +
   annotate("text", x = 7, y = 9, label = "r=0.95")

cor.test(data_tot_FB2$value_mwtl,data_tot_FB2$value_sens)
p<-ggplot(data_tot_FB2,aes(value_mwtl, value_sens))
p + geom_point() + 
  labs(xlab="values MWTL", ylab="values FB", title=" pH FB")
  stat_smooth(aes(value_sens, value_mwtl),method = "lm")+
  annotate("text", x = 7, y = 9, label = "r=0.17")

cor.test(data_tot_CTD$value_mwtl,data_tot_CTD$value_sens)
p<-ggplot(data_tot_CTD,aes(value_mwtl, value_sens))
p + geom_point() + 
  labs(xlab="values MWTL", ylab="values CTD", title=" pH CTD")+
  stat_smooth(aes(value_sens, value_mwtl),method = "lm")+
  annotate("text", x = 7.75, y = 8.5, label = "r=0.79")
                  
# with median and percentiles
require(plyr)

FB_stat<-ddply(data_tot_FB2,"time_mwtl", summarise,
...
ggplot(FB_stat,aes(value_mwtl,median_sens,ymin=low_sens,ymax=high_sens))+
  ...
  geom_point()+
  labs(title=" pH median FB")+
  stat_smooth(aes(value_mwtl,median_sens),se=FALSE,method = "lm")+
  annotate("text", x =7.5, y =8.5 , label = "r=0.15")
...
ggplot(FB_stat,aes(value_mwtl,median_sens,ymin=low_sens,ymax=high_sens))+
  ...
  facet_wrap(~location)+
  labs(title="pH median FB")+
  stat_smooth(method = "lm",se=FALSE,size=.75) #,linetype="dashed")

SF_stat<-ddply(data_tot_SF2,"time_mwtl", summarise,
               n_sens=length(unique(value_sens)),
               value_mwtl=unique(value_mwtl),
               location=unique(location),
               year=unique(year),
...
ggplot(SF_stat,aes(value_mwtl,median_sens,ymin=low_sens,ymax=high_sens))+
  geom_linerange(col="grey")+
  geom_point()+
  labs(title=" pH median SF")+
  stat_smooth(aes(value_mwtl,median_sens),method = "lm",se=FALSE)+
  annotate("text", x =7.8, y = 8.5, label = "r=0.96")
...
CTD_stat<-ddply(data_tot_CTD2,"time_mwtl", summarise,
                n_sens=length(unique(value_sens)),
                value_mwtl=unique(value_mwtl),
                location=unique(location),
                year=unique(year),
...
ggplot(CTD_stat,aes(value_mwtl,median_sens,ymin=low_sens,ymax=high_sens))+
  geom_linerange(col="grey")+
  geom_point()+
  labs(title=" pH median CTD")+
  stat_smooth(aes(value_mwtl,median_sens),method = "lm",se=FALSE)+
  annotate("text", x =7.8, y = 8.5, label = "r=0.78")
