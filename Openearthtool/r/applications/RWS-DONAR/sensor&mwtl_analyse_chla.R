...
p + geom_point() + scale_y_log10(limit=c(0.001,100)) + scale_x_log10()+ 
  labs(xlab="values MWTL", ylab="values SF", title=" Chla and Fluorescence SF") + facet_wrap(~location)

p<-ggplot(data_tot_FB,aes(value_mwtl, value_sens,color=year))
p + geom_point() + scale_y_log10() + scale_x_log10() + scale_colour_brewer(palette="Accent") +
  labs(xlab="values MWTL", ylab="values FB", title=" Chla and Fluorescence FB") + facet_wrap(~location)

p<-ggplot(data_tot_CTD_a,aes(value_mwtl, value_sens,color=year))
p + geom_point() + scale_y_log10(limit=c(0.001,100)) + scale_x_log10() + 
  labs(xlab="values MWTL", ylab="values CTD", title=" Chla and Fluorescence CTD") + facet_wrap(~location)
...
p + geom_point() + scale_y_log10(limit=c(0.001,100)) + scale_x_log10(limit=c(0.01,100))+ 
  labs(xlab="values MWTL", ylab="values SF", title=" Chla and Fluorescence SF")+
  stat_smooth(aes(value_mwtl,value_sens),method = "lm") +
   annotate("text", x = 0.05, y = 50, label = "r=0.75")
   
cor.test(data_tot_FB$value_sens,data_tot_FB$value_mwtl)
p<-ggplot(data_tot_FB,aes(value_mwtl, value_sens))
p + geom_point() + scale_y_log10(limit=c(0.001,100)) + scale_x_log10(limit=c(0.01,100))+ 
  labs(xlab="values MWTL", ylab="values FB", title=" Chla and Fluorescence FB")+
  stat_smooth(aes(value_mwtl,value_sens),method = "lm")+
  annotate("text", x = 0.05, y = 50, label = "r=0.28")

cor.test(data_tot_CTD$value_sens,data_tot_CTD$value_mwtl)
p<-ggplot(data_tot_CTD,aes(value_mwtl, value_sens))
p + geom_point() + scale_y_log10(limit=c(0.001,100)) + scale_x_log10(limit=c(0.01,100))+ 
  labs(xlab="values MWTL", ylab="values sens", title=" Chla and Fluorescence CTD")+
  stat_smooth(aes(value_mwtl,value_sens),method = "lm")+
  annotate("text", x = 0.05, y = 50, label = "r=0.63")
...
ggplot(FB_stat,aes(value_mwtl,median_sens,ymin=low_sens,ymax=high_sens))+
  ...
  geom_point()+scale_y_log10(limit=c(0.005,100)) + scale_x_log10(limit=c(0.005,100))+
  labs(x=expression(MWTL~waarde~Chla~(mu*g/l)), y="Sensorwaarde Fluor(-)", title="Chla Ferrybox")+
  stat_smooth(aes(value_mwtl,median_sens),se=FALSE,method = "lm",size=1)+
  annotate("text", x =0.005, y = 100, label = "r=0.16")+
  geom_abline(slope=1,linetype="dashed",col="black")
  ggsave(file="Chla_FB_all_median.png",path=plotdir,width=8,height=6)

ggplot(FB_stat,aes(value_mwtl,median_sens,ymin=low_sens,ymax=high_sens))+
  geom_linerange(col="grey",size=.75)+
  geom_point(aes(colour=year))+
  scale_y_log10(limit=c(0.005,100)) + scale_x_log10(limit=c(0.005,100))+
  facet_wrap(~location)+
  labs(title=" Chla and Fluorescence median FB")+
  labs(x=expression(MWTL~waarde~Chla~(mu*g/l)), y="Sensorwaarde Fluor(-)", title="Chla Ferrybox")+
  geom_abline(slope=1,linetype="dashed",col="black")
  ggsave(file="Chla_FB_perloc&year_median.png",path=plotdir,width=8,height=6)     
...
ggplot(SF_stat,aes(value_mwtl,median_sens,ymin=low_sens,ymax=high_sens))+
  geom_linerange(col="grey")+
  geom_point()+scale_y_log10(limit=c(0.005,100)) + scale_x_log10(limit=c(0.005,100))+
  labs(title=" Chla and Fluorescence median SF")+
  labs(x=expression(MWTL~waarde~Chla~(mu*g/l)), y="Sensorwaarde Fluor(-)", title="Chla Meetvis")+
  stat_smooth(aes(value_mwtl,median_sens),method = "lm",se=FALSE,size=1)+
  annotate("text", x =0.005, y = 100, label = "r=0.77")+
  geom_abline(slope=1,linetype="dashed",col="black")
  ggsave(file="Chla_SF_all_median.png",path=plotdir,width=8,height=6)      
...
ggplot(SF_stat,aes(value_mwtl,median_sens,ymin=low_sens,ymax=high_sens))+
  ...
  labs(x=expression(MWTL~waarde~Chla~(mu*g/l)), y="Sensorwaarde Fluor(-)", title="Chla Meetvis")+
  geom_abline(slope=1,linetype="dashed",col="black")
  ggsave(file="Chla_SF_perloc&year_median.png",path=plotdir,width=8,height=6) 

CTD_stat<-ddply(data_tot_CTD,"time_mwtl", summarise,
               n_sens=length(unique(value_sens)),
               value_mwtl=unique(value_mwtl),
               location=unique(location),
               year=unique(year),
...
ggplot(CTD_stat,aes(value_mwtl,median_sens,ymin=low_sens,ymax=high_sens))+
  geom_linerange(col="grey")+
  geom_point()+scale_y_log10(limit=c(0.005,100)) + scale_x_log10(limit=c(0.005,100))+
  labs(x=expression(MWTL~waarde~Chla~(mu*g/l)), y="Sensorwaarde Fluor(-)", title="Chla CTD")+
  stat_smooth(aes(value_mwtl,median_sens),method = "lm",se=FALSE,size=1)+
  annotate("text", x =0.005, y = 100, label = "r=0.64")+
  geom_abline(slope=1,linetype="dashed",col="black")
  ggsave(file="Chla_CTD_all_median.png",path=plotdir,width=8,height=6)     
...
ggplot(CTD_stat,aes(value_mwtl,median_sens,ymin=low_sens,ymax=high_sens))+
  ...
  labs(x=expression(MWTL~waarde~Chla~(mu*g/l)), y="Sensorwaarde Fluor(-)", title="Chla CTD")+
  geom_abline(slope=1,linetype="dashed",col="black")
  ggsave(file="Chla_CTD_perloc&year_median.png",path=plotdir,width=8,height=6)                   
