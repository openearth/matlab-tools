...
subset <- subset(SPMSoele, as.Date(Date) >= "2004-07-01" & as.Date(Date)<="2014-01-01")

#write subset to Excel
write.xlsx(x = subset, file = "spmsoelepermonthafterkatse.xlsx", sheetName = "TestSheet", row.names = FALSE)


#save plot in pdf
pdf("SPMveerseafterkatsepermonth.pdf", width=10, height=8)
print(ggplot(subset)+
