...
subset <- subset(ChlorASoele, as.Date(Date) >= "1972-01-01" & as.Date(Date)<="2004-07-01")

#write subset to Excel
write.xlsx(x = subset, file = "chlorasoelepermonthbeforekatse.xlsx", sheetName = "TestSheet", row.names = FALSE)


#save plot in pdf
pdf("Chloraveersebeforekatsepermonth.pdf", width=10, height=8)
print(ggplot(subset)+
