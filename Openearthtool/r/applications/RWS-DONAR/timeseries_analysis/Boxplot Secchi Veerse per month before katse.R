...
subset <- subset(SecchiSoele, as.Date(Date) >= "1970-01-01" & as.Date(Date)<="2004-07-01")

#write subset to Excel
write.xlsx(x = subset, file = "secchisoelepermonthbeforekatse.xlsx", sheetName = "TestSheet", row.names = FALSE)
...
#save plot in pdf
pdf("Secchiveersebeforekatsepermonth.pdf", width=10, height=8)
print(ggplot(subset)+
