#Read in raw data
secchivolkerak<-file.path("N:/My Documents/Data from Donar/SecchiVolkerak.xlsx")
...
df <- data.frame(SecchiVolkerak$Date, SecchiVolkerak$Location, SecchiVolkerak$Value)
df2 <-subset(df,SecchiVolkerak.Location == "Oesterdam" | SecchiVolkerak.Location == "Molenplaat midden" | SecchiVolkerak.Location == "Nieuw-Vossemeer" | SecchiVolkerak.Location == "Steenbergen (Roosendaalsevliet)" | SecchiVolkerak.Location == "Volkerak, meetplaats 02" | SecchiVolkerak.Location == "Hellegat")
data <- dcast(df2, SecchiVolkerak.Date ~ SecchiVolkerak.Location, value.var="SecchiVolkerak.Value", mean)
subset <- subset(data, as.Date(SecchiVolkerak.Date) >= "1987-01-01" & as.Date(SecchiVolkerak.Date)<="2014-01-01")


#reorder columns according to location on map
data2 <- subset[c(1,4,3,5,6,7,2)]
data3 <-log10(data2[,-1])
...
# plot the data
#save plot in pdf
pdf("Loglogcorrelationsecchivolkerakafterenclosure.pdf", width=12, height=8)
print(chart.Correlation(DM2,histogram = FALSE,xlim=c(0.6,1.8), ylim=c(0.6,1.8))+ 
