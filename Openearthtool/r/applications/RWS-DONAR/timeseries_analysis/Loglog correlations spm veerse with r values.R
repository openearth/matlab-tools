#Read in raw data
spmveerse<-file.path("N:/My Documents/Data from Donar/SPMVeerse.xlsx")
...
df <- data.frame(SPMVeerse$Date, SPMVeerse$Location, SPMVeerse$Value)
data <- dcast(df, SPMVeerse.Date ~ SPMVeerse.Location, value.var="SPMVeerse.Value", mean)
subset <- subset(data, as.Date(SPMVeerse.Date) >= "1972-01-01" & as.Date(SPMVeerse.Date)<="1976-01-01")
...
# plot the data
#save plot in pdf
pdf("Loglogcorrelationspmveersebeforekatse.pdf", width=12, height=8)
